...
function formatPublishedDate(d) { return new Date(d+"T12:00:00").toLocaleDateString("en-US",{month:"long",day:"numeric",year:"numeric"}); }
    ...
    <div style={{animation:"fadeSlideIn 0.3s ease"}}>
      ...
      <div style={{background:th.surface,borderRadius:20,border:`1.5px solid ${th.border}`,overflow:"hidden",boxShadow:th.shadowLg}}>
        ...
        <div style={{padding:"20px 20px 24px",display:"flex",flexDirection:"column",gap:22}}>
          ...
          {event.details.eggs&&<DetailSection title="7 km Eggs" emoji="🥚" items={event.details.eggs} color="#E67E22"/>}
          {event.details.routes&&(<div><h4 style={{margin:"0 0 10px 0",fontSize:13,fontWeight:700,color:th.text,display:"flex",alignItems:"center",gap:8}}><span>🛤️</span> Route Spawns</h4><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{event.details.routes.phases.map((p,i)=>(<div key={i} style={{flex:1,minWidth:200,border:`1.5px solid ${th.border}`,borderRadius:14,overflow:"hidden"}}><div style={{padding:"8px 14px",background:th.accentBgSubtle?.("#27AE60")||"#27AE6015",borderBottom:`1.5px solid ${th.border}`}}><div style={{fontSize:12,fontWeight:700,color:th.text,letterSpacing:0.5}}>{p.dates}</div></div><div style={{padding:"10px 14px",display:"flex",flexDirection:"column",gap:6}}>{p.pokemon.map((pk,j)=>(<div key={j} style={{fontSize:13,fontWeight:600,color:th.text}}>{pk}</div>))}</div></div>))}</div>{event.details.routes.notes&&<div style={{display:"flex",flexDirection:"column",gap:6,marginTop:8}}>{event.details.routes.notes.map((n,i)=>(<div key={i} style={{padding:"10px 14px",borderRadius:10,background:th.accentBgSubtle?.("#27AE60")||"#27AE6015",fontSize:13,color:th.textSecondary,lineHeight:1.5,display:"flex",alignItems:"center",gap:8}}><span style={{color:"#27AE60",fontWeight:700}}>•</span> {n}</div>))}</div>}</div>)}
      ...
      <div style={{background:th.surface,borderRadius:20,border:`1.5px solid ${th.border}`,overflow:"hidden",boxShadow:th.shadowLg}}>
        <div style={{padding:"24px 20px",borderBottom:`1.5px solid ${th.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <span style={{fontSize:12,color:th.textSecondary,fontWeight:600}}>{announcement.published ? "Published " + formatPublishedDate(announcement.published) : formatDate(announcement.date)}</span>
          </div>
  ...
  return (
    <button onClick={()=>onSelect(announcement)} style={{padding:"14px 18px",background:th.surface,border:`1.5px solid ${th.border}`,borderRadius:14,display:"flex",flexDirection:"column",gap:7,boxShadow:th.shadow,cursor:"pointer",textAlign:"left",width:"100%",fontFamily:"inherit",transition:"all 0.25s cubic-bezier(0.25,0.46,0.45,0.94)",animation:`fadeSlideUp 0.4s cubic-bezier(0.25,0.46,0.45,0.94) ${index*0.06}s both`}}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%"}}>
        ...
        <span style={{fontSize:11,color:th.textSecondary,fontWeight:600}}>{announcement.published ? "Published " + formatPublishedDate(announcement.published) : formatDate(announcement.date)}</span>
      </div>
