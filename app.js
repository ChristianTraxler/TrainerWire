...
function renderBossItem(item, color, th, cardLayout, noSparkles) {
  ...
  const isRaidTier = /\(\d★\s*(?:Raid|Shadow Raid)?\)|\(Mega\)|\(\d★\s*Max Battle|\(Shadow Raid\)/.test(item);
  ...
  return `<div style="animation:fadeSlideIn 0.3s ease">
    ...
    <div style="background:${th.surface};border-radius:20px;border:1.5px solid ${th.border};overflow:hidden;box-shadow:${th.shadowLg}">
      <div style="background:${th.detailHeaderBg(event.color)};padding:24px 20px;border-bottom:2px solid ${th.detailHeaderBorder(event.color)};position:relative">
        ${event.published || event.updated ? `<div style="position:absolute;top:14px;right:16px;display:flex;flex-direction:column;align-items:flex-end;gap:2px;font-size:12px;color:${th.textSecondary};font-weight:600">${event.published ? `<span>Published ${formatPublishedDate(event.published)}</span>` : ""}${event.updated ? `<span>Updated ${formatPublishedDate(event.updated)}</span>` : ""}</div>` : ""}
        <div style="display:flex;align-items:center;gap:14px;margin-bottom:12px;flex-wrap:wrap">
          ${(() => {
            ...
            if (event.iconImg) {
              ...
              return `<div style="width:70px;height:70px;border-radius:16px;background:${th.accentBg(event.color)};border:2px solid ${th.countdownBorder(event.color)};display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;overflow:hidden">${shadowLayer}<img src="${event.iconImg}" style="width:62px;height:62px;object-fit:contain;position:relative;z-index:1" onerror="this.parentElement.innerHTML='${event.icon}'" />${shinySparkleDetail}</div>`;
            }
      ...
      <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0">
        ${(() => {
          ...
          if (event.iconImg) {
            ...
            return `<div style="width:48px;height:48px;border-radius:12px;background:${th.accentBg(event.color)};display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;overflow:hidden">${shadowLayer}<img src="${event.iconImg}" style="${imgStyle}" onerror="this.parentElement.innerHTML='${event.icon}'" />${shinySparkleCard}</div>`;
          }
