...
function rewardIcon(text) {
  ...
  if (t.includes("rare candy xl")) return `${IMG_BASE}/Items/RareXLCandy_PSD.png`;
  if (t.includes("rare candy")) return `${IMG_BASE}/Items/Rare-candy.png`;
  if (/candy(?!\s*xl)/i.test(text)) return `${IMG_BASE}/Items/pokemon_details_candy.png`; // a regular Candy reward wins over a secondary "Candy XL" mention
  if (t.includes("candy xl")) return `${IMG_BASE}/Items/RareXLCandy_PSD.png`;
  if (t.includes("stardust")) return `${IMG_BASE}/Items/stardust_painted.png`;
  if (t.includes("golden razz")) return `${IMG_BASE}/Items/Golden-razz-berry.png`;
  if (t.includes("silver pinap")) return `${IMG_BASE}/Items/Silver-pinap-berry.png`;
  if (t.includes("pinap")) return `${IMG_BASE}/Items/Pinap-berry.png`;
  if (t.includes("nanab")) return `${IMG_BASE}/Items/Nanab-berry.png`;
...
// PokeAPI lookup is unavailable (offline/blocked). Keyed by the base species'
// dex number; each stage needs { name, dexNum, depth }. Sprites/types come from
// local assets + pokemon-data.json, so no network is required to render.
const EVO_CHAIN_FALLBACK = {
  816: [
    { name: "Sobble", dexNum: 816, depth: 0 },
    { name: "Drizzile", dexNum: 817, depth: 1 },
    { name: "Inteleon", dexNum: 818, depth: 2 }
  ],
  827: [
    { name: "Nickit", dexNum: 827, depth: 0 },
    { name: "Thievul", dexNum: 828, depth: 1 }
  ]
};
...
function getPokemonImg(name) {
  ...
  if (lower.includes("pikachu") && lower.includes("willow")) {
    return { url: costumeDexImg(25, "anniversary2026"), shadow: false }; // Willow's assistant Pikachu — official Anniversary 2026 sprite (replaces earlier cut-promo-art placeholder)
  }
  // Special Anniversary Pikachu Celebration costumes (raids.nl form IDs)
  if (lower.includes("pikachu") && lower.includes("dapper")) {
    if (lower.includes("red")) return { url: costumeDexImg(25, "f2867"), shadow: false };
    if (lower.includes("yellow")) return { url: costumeDexImg(25, "f2869"), shadow: false };
    if (lower.includes("blue")) return { url: costumeDexImg(25, "f2868"), shadow: false };
  }
  if (lower.includes("pikachu") && lower.includes("varsity")) {
    return { url: costumeDexImg(25, "f2863"), shadow: false };
  }
  if (lower.includes("pikachu") && lower.includes("safari")) {
    return { url: costumeDexImg(25, "c10"), shadow: false };
  }
  if (lower.includes("pikachu") && lower.includes("tricks")) {
    return { url: costumeDexImg(25, "c71"), shadow: false }; // Tricks & Treats — confirmed by user (raids.nl 25_c71)
  }
  if (lower.includes("pikachu") && lower.includes("winter carnival")) {
    return { url: costumeDexImg(25, "f2670"), shadow: false }; // Winter Carnival — confirmed by user image (bowler + scarf + striped ring)
  }
  if (lower.includes("summer flair")) {
    return { url: costumeDexImg(25, "c5"), shadow: false }; // summer flair — confirmed by user image (straw hat + sunglasses)
  }
  if (lower.includes("amethyst")) {
    return { url: costumeDexImg(25, "f3004"), shadow: false }; // amethyst crown — confirmed by user (raids.nl 25_f3004)
  }
  if (lower.includes("championship")) {
    if (lower.includes("2022")) return { url: costumeDexImg(25, "f2806"), shadow: false }; // Worlds 2022 — confirmed by user image (blue top hat + red/blue robe)
    if (lower.includes("2023")) return { url: costumeDexImg(25, "f3014"), shadow: false }; // Worlds 2023 — confirmed by user (raids.nl 25_f3014)
    if (lower.includes("2024")) return { url: costumeDexImg(25, "f3015"), shadow: false }; // Worlds 2024 — confirmed by user (raids.nl 25_f3015)
    if (lower.includes("2025")) return { url: costumeDexImg(25, "f3017"), shadow: false }; // Worlds 2025 — confirmed by user (raids.nl 25_f3017); not in this event's article
  }
  if (lower.includes("pikachu") && lower.includes("cake")) {
    return { url: eventDexImg(25, "cake-male"), shadow: false };
  }
  if (lower.includes("party hat")) {
    const PARTY_HAT = { Bulbasaur: [1, "party"], Charmander: [4, "party"], Squirtle: [7, "party"], Raticate: [20, "party-male"], Nidorino: [33, "party-male"], Grimer: [88, "party"], Gengar: [94, "party"], Wobbuffet: [202, "party-male"], Wurmple: [265, "party"], Eevee: [133, "party-male"] };
  ...
  if (lower.includes("psyduck") && lower.includes("swim ring")) {
    return { url: eventDexImg(54, "swimring"), shadow: false }; // Ultra Unlock: Water Festival — swim-ring Psyduck raid
  }
  if (lower.includes("tatsugiri")) {
    if (lower.includes("curly")) return { url: natDexImg(978, "_curly"), shadow: false };
    if (lower.includes("droopy")) return { url: natDexImg(978, "_droopy"), shadow: false };
    if (lower.includes("stretchy")) return { url: natDexImg(978, "_stretchy"), shadow: false };
  }
  if (lower.includes("lapras") && lower.includes("scarf")) {
    return { url: costumeDexImg(131, "scarf"), shadow: false }; // Ultra Unlock: Water Festival — scarf-wearing Lapras raid
  }
  ...
  if (lower.includes("plumage")) {
    if (lower.includes("yellow plumage")) return { url: natDexImg(931, "_yellow_plumage"), shadow: false };
    if (lower.includes("blue plumage"))   return { url: natDexImg(931, "_blue_plumage"),   shadow: false };
    if (lower.includes("white plumage"))  return { url: natDexImg(931, "_white_plumage"),  shadow: false };
    if (lower.includes("green plumage"))  return { url: natDexImg(931, ""),                shadow: false };
  }
  ...
  if (lower.includes("black kyurem")) {
    return { url: natDexImg(646, "_black"), shadow: lower.startsWith("shadow ") };
  }
  if (lower.includes("white kyurem")) {
    return { url: natDexImg(646, "_white"), shadow: lower.startsWith("shadow ") };
  }
...
const SHINY_AVAILABLE = new Set([
  ...
  "Regidrago","Kyogre","Groudon","Mewtwo","Solgaleo",
...
// --- GO Fest 2026 Zeraora Special Research ("A Thunderous Discovery") ---
// Source: https://pokemongohub.net/post/research/zeraora-special-research-a-thunderous-discovery-tokyo-copenhagen-chicago/
// Step 2 is venue-specific — the Pikachu hat matches the host Team Leader's team.
function buildThunderousDiscovery(teamHat) {
  const teamPikachu = `Pikachu wearing a ${teamHat} hat`;
  return {
    title: "A Thunderous Discovery",
    subtitle: "GO Fest 2026 Special Research — Zeraora",
    encounter: "Zeraora",
    source: { label: "Pokémon GO Hub", url: "https://pokemongohub.net/post/research/zeraora-special-research-a-thunderous-discovery-tokyo-copenhagen-chicago/" },
    note: "Exclusive to in-person GO Fest 2026 (Tokyo / Chicago / Copenhagen). One Zeraora encounter per Trainer; extra completions award Zeraora Candy. Step 2 is venue-specific.",
    stages: [
      {
        label: "Page 1 of 8",
        tasks: [
          { task: "Spin 5 PokéStops or Gyms", reward: "2,026 × XP" },
          { task: "Make a Nice Throw", reward: "20 × Poké Ball" },
          { task: "Catch 10 Pokémon", reward: "2,026 × Stardust" }
        ],
        rewards: ["1 × Star Piece", "1 × Lucky Egg", "Caterpie wearing a Poké Ball hat"]
      },
      {
        label: "Page 2 of 8",
        venueSpecific: true,
        tasks: [
          { task: "Catch 10 Pokémon in the Recruitment Zone", reward: teamPikachu },
          { task: "Catch 10 Pokémon in the Conservatory Zone", reward: teamPikachu },
          { task: "Catch 10 Pokémon in the Cultivation Zone", reward: teamPikachu },
          { task: "Catch 10 Pokémon in the Team GO Rocket Hideout", reward: "Sandile" }
        ],
        rewards: ["2,026 × XP", "26 × Poké Ball", "Wash Rotom"]
      },
      {
        label: "Page 3 of 8",
        tasks: [
          { task: "Catch 3 Electric-type Pokémon", reward: "26 × Poké Ball" },
          { task: "Power up Electric-type Pokémon 6 times", reward: "1 × Magnetic Lure Module" },
          { task: "Transfer 3 Electric-type Pokémon", reward: "26 × Great Ball" }
        ],
        rewards: ["2,026 × XP", "Dhelmise", "2,026 × Stardust"]
      },
      {
        label: "Page 4 of 8",
        tasks: [
          { task: "Explore 2 km", reward: "26 × Poké Ball" },
          { task: "Make 6 Nice Throws", reward: "6 × Pinap Berry" },
          { task: "Make 3 Curveball Throws", reward: "1 × Lucky Egg" }
        ],
        rewards: ["2,026 × XP", "Mudbray", "3 × Premium Raid Pass"]
      },
      {
        label: "Page 5 of 8",
        tasks: [
          { task: "Battle in 2 Raids", reward: "6 × Max Revive" },
          { task: "Win a Raid", reward: "6 × Max Potion" },
          { task: "Use 3 Max Potions", reward: "26 × Ultra Ball" }
        ],
        rewards: ["2,026 × XP", "Comfey", "2 × Super Incubator"]
      },
      {
        label: "Page 6 of 8",
        tasks: [
          { task: "Explore 2 km", reward: "2,026 × Stardust" },
          { task: "Make 3 Great Throws", reward: "3 × Rare Candy" },
          { task: "Make an Excellent Throw", reward: "1 × Rare Candy XL" }
        ],
        rewards: ["6 × Zeraora Sticker", "Zeraora encounter", "26 × Zeraora Candy"]
      },
      {
        label: "Page 7 of 8",
        tasks: [
          { task: "Take a GO Snapshot of Zeraora", reward: "1 × Poffin" },
          { task: "Earn 1 Candy with Zeraora as your Buddy", reward: "26 × Ultra Ball" },
          { task: "Make Zeraora your Buddy and earn 6 Hearts", reward: "3 × Rare Candy" }
        ],
        rewards: ["Zeraora T-shirt avatar item", "6 × GO Fest 2026 Sticker", "2 × Lure Module"]
      },
      {
        label: "Page 8 of 8",
        tasks: [
          { task: "Catch a Pokémon on 7 different days", reward: "2,026 × XP" },
          { task: "Hatch 2 Eggs", reward: "2,026 × Stardust" },
          { task: "Send a Gift to a friend", reward: "2,026 × XP" }
        ],
        rewards: ["GO Fest 2026 Glasses avatar item", "6 × Rare Candy XL", "6 × Zeraora Rare Candy XL"]
      }
    ]
  };
}
...
const EVENTS = [
  ...
  { id: 121, title: "Ultra Unlock: Starmie Super Mega Raid Day", type: "Raid", url: "https://pokemongo.com/en/news/starmie-super-mega-raid-day-2026", date: "2026-08-22", endDate: null, time: "11:00 AM – 5:00 PM", whenText: "Saturday, August 22, 2026, from 11:00 a.m. to 5:00 p.m. local time", color: "#3498DB", icon: "🌊", published: "2026-07-15", updated: "2026-07-15", lastUpdated: "July 15, 2026 at 11:31 AM", iconImg: "assets/pokemon-images/Mega/regular/Gen-1_Kanto/0121_mega.webp", featured: true, summary: "Mega Starmie debuts in Super Mega Raids during Ultra Unlock on Saturday, August 22, from 11:00 a.m. to 5:00 p.m. local time! Shiny Starmie encounter rates are boosted, and Starmie caught from these raids will have Mega Level 1 unlocked. Free Timed Research rewards a Premium Battle Pass and a Scizor encounter.", details: { bosses: ["★Mega Starmie (Super Mega Raid)"], groupSize: [{ bossName: "Mega Starmie", tier: "Super Mega Raid", minimum: 3, optimalMin: 4, optimalMax: 6, easyAt: 7, greenAt: 8 }], bonuses: [
    ...
    { text: "Remote Raid Pass limit increased to 20 (Aug 21 2:00 PM – Aug 22 8:00 PM PDT)", img: "assets/pokemon-images/Items/remote-raid-pass.png" },
    { text: "Up to 6 free Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" },
    { text: "Increased chance of encountering Shiny Starmie from Super Mega Raids", img: "assets/pokemon-images/icons/shiny-sparkles.webp" },
    { text: "Starmie caught from Super Mega Raids will have Mega Level 1 unlocked", img: "assets/pokemon-images/icons/dna-helix-icon.webp" },
    { text: "Super Mega Raids may only appear at certain Gyms — locate them via pokemongo.com/map", img: "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png" },
    { text: "Free Timed Research available during event hours — rewards a Premium Battle Pass and a Scizor encounter", img: "assets/pokemon-images/icons/green-research.png" }
  ], timedResearch: { free: { rewards: ["Premium Battle Pass", "Scizor encounter"], note: "Timed Research expires — tasks must be completed and rewards claimed before the Timed Research ends." } }, ticketPrice: "$4.99", ticketNote: "These bonuses will be effective on Saturday, August 22, 2026, from 11:00 a.m. to 5:00 p.m. local time.", ticketBonuses: [
    { text: "An additional 5,000 XP from all raids", img: "assets/pokemon-images/icons/xp.png" },
    { text: "Up to 14 Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" },
    { text: "An even greater chance to get Rare Candy XL from Raid Battles", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png" },
    { text: "Guaranteed Rare Candy XL for successfully catching Starmie from raids", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png" }
  ], tips: [
    "Starmie caught from Super Mega Raids will have Mega Level 1 unlocked — catch at least one to start Mega Evolving it.",
    "Shiny Starmie encounter rates are boosted for the event — check every catch. ✨",
    "Free Timed Research rewards a Premium Battle Pass and a Scizor encounter — complete it during event hours.",
    "Super Mega Raids may only appear at certain Gyms — use pokemongo.com/map to find them."
  ], counters: { label: "Mega Starmie (Water / Psychic)", bosses: ["Mega Starmie"], pokemon: [
    { name: "Dawn Wings Necrozma", fast: "Shadow Claw", charged: "Moongeist Beam" },
    { name: "Mega Sceptile", fast: "Fury Cutter", charged: "Frenzy Plant" },
    { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Shadow Ball" },
    { name: "Mega Gengar", fast: "Lick", charged: "Shadow Ball" },
    { name: "Mega Tyranitar", fast: "Bite", charged: "Brutal Swing" },
    { name: "Shadow Hydreigon", fast: "Bite", charged: "Brutal Swing" }
  ] }, relatedNews: [{ id: 33, label: "Ultra Unlock: Starmie Super Mega Raid Day", icon: "🌊" }] } },
  { id: 122, title: "Gigantamax Rillaboom Max Battle Day", type: "Max Battle", url: "https://pokemongo.com/en/news/gigantamax-rillaboom-max-battle-day-2026", date: "2026-08-01", endDate: null, time: "2:00 PM – 5:00 PM", whenText: "Saturday, August 1, 2026, from 2:00 p.m. to 5:00 p.m. local time", color: "#27AE60", icon: "🥁", published: "2026-07-15", updated: "2026-07-15", lastUpdated: "July 15, 2026 at 12:06 PM", iconImg: "assets/pokemon-images/Gigantamax/regular/Gen-8_Galar/0812_gigamax.webp", featured: true, summary: "Gigantamax Rillaboom drums into six-star Max Battles on Saturday, August 1, from 2:00 p.m. to 5:00 p.m. local time — and Shiny Gigantamax Rillaboom debuts! All Power Spots host Gigantamax Battles, the Max Particle cap rises to 1,600, and Power Spots grant 8× Max Particles.", details: { bosses: ["Gigantamax Rillaboom (6★ Max Battle) ✨"], bonuses: [
    { text: "Max Particle collection limit increased to 1,600", img: "assets/pokemon-images/Items/max-particles.png" },
    { text: "All Power Spots will host Gigantamax Battles", img: "assets/pokemon-images/icons/power-stop.png" },
    { text: "Power Spots will refresh more frequently", img: "assets/pokemon-images/icons/power-stop.png" },
    { text: "Additional Power Spots will be active", img: "assets/pokemon-images/icons/power-stop.png" },
    { text: "8× Max Particles from Power Spots", img: "assets/pokemon-images/Items/max-particles.png" },
    { text: "Up to three Special Trades for the day", img: "assets/pokemon-images/icons/pogo_trade_icon.png" },
    { text: "Remote Raid limit increased to 20 (July 31 5:00 PM – August 1 8:00 PM PDT)", img: "assets/pokemon-images/Items/remote-raid-pass.png" },
    { text: "2× Max Particles from exploring", img: "assets/pokemon-images/Items/max-particles.png" },
    { text: "1/4 Adventuring distance to receive Max Particles", img: "assets/pokemon-images/Items/max-particles.png" }
  ], paidResearch: { price: "$4.99", rewards: [{ text: "1 Max Mushroom", img: "assets/pokemon-images/Items/max_mushrooms.webp" }, { text: "25,000 XP", img: "assets/pokemon-images/icons/xp.png" }, { text: "6,400 Max Particles", img: "assets/pokemon-images/Items/max-particles.png" }, { text: "2× XP from Max Battles", img: "assets/pokemon-images/icons/xp.png" }] }, tips: [
    "Gigantamax Rillaboom appears in six-star Max Battles — coordinate a full group at a Power Spot.",
    "Shiny Gigantamax Rillaboom is available for the first time in Pokémon GO — check every encounter. ✨",
    "Max Battles only appear at Power Spots that sporadically appear in different locations — use the in-game Map to find them and join community meetups.",
    "The Gigantamax Rillaboom Max Battle Day Ultra Ticket Box ($4.99) on the Web Store includes an event ticket and a bonus Premium Battle Pass.",
    "Tickets cannot be purchased with PokéCoins."
  ], counters: { label: "Gigantamax Rillaboom (Grass) — best Fire-type Max attackers", bosses: ["Gigantamax Rillaboom"], pokemon: [
    { name: "Gigantamax Cinderace", fast: "Tackle", charged: "G-Max Fireball" },
    { name: "Gigantamax Charizard", fast: "Wing Attack", charged: "G-Max Wildfire" },
    { name: "Dynamax Darmanitan", fast: "Incinerate", charged: "Max Flare" },
    { name: "Dynamax Moltres", fast: "Wing Attack", charged: "Max Airstream" },
    { name: "Dynamax Flareon", fast: "Fire Spin", charged: "Max Flare" },
    { name: "Dynamax Ho-Oh", fast: "Incinerate", charged: "Max Flare" }
  ] }, relatedNews: [{ id: 34, label: "Gigantamax Rillaboom Max Battle Day", icon: "🥁" }] } },
  { id: 118, title: "Ozone Ascent", type: "Event", url: "https://pokemongo.com/en/news/ozone-ascent-2026", date: "2026-07-25", endDate: "2026-07-26", startsAt: "2026-07-25T10:00:00", endsAt: "2026-07-26T20:00:00", published: "2026-06-26", updated: "2026-06-26", lastUpdated: "June 26, 2026 at 10:46 AM", time: "Jul 25 – Jul 26", whenText: "Saturday, July 25, at 10:00 a.m. to Sunday, July 26, 2026, at 8:00 p.m. local time", color: "#27AE60", icon: "☄️", iconImg: "assets/pokemon-images/Mega/regular/Gen-3_Hoenn/0384_mega.webp", featured: true, summary: "Rayquaza ascends! From July 25–26, complete free Timed Research for a Meteorite and a Rayquaza encounter, or pick up the US$4.99 paid Timed Research to also earn Rayquaza Mega Energy. Use Meteorites to teach Rayquaza the Charged Attack Dragon Ascent — the only way the Sky High Pokémon can Mega Evolve. Runs 10:00 a.m. July 25 to 8:00 p.m. July 26 local time.", details: { bosses: ["Rayquaza (Timed Research encounter) (no shiny)"], bonuses: ["Free Timed Research: complete tasks to earn a Meteorite, an encounter with Rayquaza, and more.", "Paid Timed Research (US$4.99, or the equivalent pricing tier in your local currency): a Meteorite, Rayquaza Mega Energy, an encounter with Rayquaza, and more.", "Unlike other Mega-Evolving Pokémon, Rayquaza can't Mega Evolve with Mega Energy alone — teach it the Charged Attack Dragon Ascent using Meteorites.", "Dragon Ascent can only be learned by using Meteorites.", "Timed Research must be completed and its rewards claimed by Sunday, July 26, 2026, at 8:00 p.m. local time.", "Paid tickets — including those purchased for other Trainers — are non-refundable (subject to applicable law) and can't be purchased with PokéCoins.", "Tickets can be gifted to friends at Great Friends level or higher."], tips: ["Mega Rayquaza is one of the best Dragon-type (and Flying-type) attackers in the game — well worth powering up.", "Save your Meteorites: they're the only way to teach Rayquaza Dragon Ascent and unlock its Mega Evolution.", "The paid Timed Research stacks Rayquaza Mega Energy on top of the free rewards — handy for Mega Evolving right away.", "Don't let your Timed Research expire — finish it before Sunday, July 26 at 8:00 p.m. local time.", "Tickets can be gifted to Great Friends or higher, but purchases are non-refundable and can't be bought with PokéCoins.", "All listed times are local."], relatedNews: [{ id: 29, label: "Ozone Ascent — Rayquaza & Dragon Ascent", icon: "☄️" }] } },
  { id: 106, title: "Special Anniversary Pikachu Celebration", type: "Event", url: "https://pokemongo.com/en/news/special-anniversary-pikachu-celebration", date: "2026-07-13", endDate: "2026-07-20", startsAt: "2026-07-13T10:00:00", endsAt: "2026-07-20T20:00:00", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:30 PM", time: "Jul 13 – Jul 20", whenText: "Monday, July 13, at 10:00 a.m. to Monday, July 20, 2026, at 8:00 p.m. local time", color: "#F4C20D", icon: "⚡", iconImg: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_f2867.webp", featured: true, summary: "Three back-to-back celebrations of costumed Pikachu keep the anniversary going! Tricks & Treats, Winter Carnival, and summer-flair Pikachu fill the wild (with standard Pikachu appearing more often and easier to catch), while nine costumed Pikachu — Dapper red/yellow/blue, safari hat, varsity jacket, amethyst crown, and World Championships 2022–2024 — headline one-star raids. A free event GO Pass stacks 2×/3× transfer Candy and catch Stardust, plus event-themed Field Research. Runs July 13–20.", details: { bosses: ["Pikachu wearing a Tricks & Treats costume (Wild) ✨", "Pikachu wearing a Winter Carnival outfit (Wild) ✨", "Pikachu with summer flair (Wild) ✨", "Pikachu (appears more frequently in the wild and easier to catch)", "…and more event-themed Pokémon (Wild)", "Dapper Pikachu with red accents (1★ Raid) ✨", "Dapper Pikachu with yellow accents (1★ Raid) ✨", "Dapper Pikachu with blue accents (1★ Raid) ✨", "Pikachu wearing a safari hat (1★ Raid) ✨", "Pikachu wearing a varsity jacket (1★ Raid) ✨", "Pikachu wearing an amethyst crown (1★ Raid) ✨", "World Championships 2022 Pikachu (1★ Raid) ✨", "World Championships 2023 Pikachu (1★ Raid) ✨", "World Championships 2024 Pikachu (1★ Raid) ✨"], bonuses: ["This event features three celebrations of different costumed Pikachu — find as many as you can in the wild and in Field Research tasks", "Celebration 1: July 13, 10:00 a.m. to July 15, 10:00 a.m. local time", "Celebration 2: July 15, 10:00 a.m. to July 18, 10:00 a.m. local time", "Celebration 3: July 18, 10:00 a.m. to July 20, 8:00 p.m. local time", "Pikachu appears more frequently in the wild and is easier to catch during the event", "Event-themed Field Research tasks reward encounters with event-themed Pokémon", "Reach Major Milestones on your event GO Pass to unlock the bonus multipliers below", "Keep an eye out for additional bonus tasks for this GO Pass during the event", "If you're lucky, every costumed Pikachu (wild and raid) can be Shiny"], milestones: [{ tier: "Tier 1 – Rank 10", bonus: "2× Candy for transferring Pokémon; increased chance for Trainers level 31 and up to receive Candy XL for transferring Pokémon", deluxe: "3× Candy for transferring Pokémon; even greater chance for Trainers level 31 and up to receive Candy XL for transferring Pokémon" }, { tier: "Tier 2 – Rank 20", bonus: "2× Stardust for catching Pokémon", deluxe: "3× Stardust for catching Pokémon" }], goPass: { free: ["Automatically granted to all Trainers on Monday, July 13 at 10:00 a.m. local time", "Encounters with event-themed Pokémon", "More than 150,000 XP", "Lucky Egg", "Golden Razz Berries", "And even more goodies", "Tier 1 (Rank 10): 2× transfer Candy + boosted Candy XL chance (L31+)", "Tier 2 (Rank 20): 2× catch Stardust", "Rewards expire Wednesday, July 22, 2026 at 8:00 p.m. local time"], deluxe: { price: "$4.99", rewards: ["Everything in the free track, with upgraded rewards and faster progression", "Additional encounters with even more event-themed Pokémon", "More than 350,000 XP", "Lucky Eggs", "Silver Pinap Berries", "Lure Module", "And even more goodies", "Tier 1 (Rank 10): 3× transfer Candy + even greater Candy XL chance (L31+)", "Tier 2 (Rank 20): 3× catch Stardust"] }, deluxePlus: { price: "$6.99", rewards: ["Everything in GO Pass Deluxe", "Automatically earn enough GO Points to reach Rank 7 (+6 Ranks)"] } }, tips: ["The GO Pass is automatic and free — claim your rewards before they expire on Wednesday, July 22 at 8:00 p.m. local time.", "Three celebrations rotate different costumed Pikachu — check the wild and Field Research across all three windows to collect them all.", "Every costumed Pikachu can be Shiny — check each wild spawn and one-star raid.", "Standard Pikachu spawns more often and is easier to catch — a good window to farm Pikachu Candy.", "Pokémon GO Web Store: buying GO Pass Deluxe (or Deluxe + 6 Ranks) there includes a gift of 10 Ultra Balls, 5 Max Revives, 1 Premium Battle Pass, and 5 Max Potions.", "Web-exclusive GO Pass Deluxe + 6 Ranks Ultra Box adds 20 Ultra Balls, 10 Max Revives, 10 Max Potions, 2 Premium Battle Passes, 1 Incubator, and 1 Super Incubator.", "All listed times are local."], relatedNews: [{ id: 26, label: "Special Anniversary Pikachu Celebration — Every Costumed Pikachu", icon: "⚡" }] } },
  { id: 119, title: "Ultra Unlock: 10th Anniversary Edition", type: "Event", url: "https://pokemongo.com/en/news/ultraunlock-10th-anniversary-edition", date: "2026-07-21", endDate: "2026-07-27", startsAt: "2026-07-21T10:00:00", endsAt: "2026-07-27T20:00:00", published: "2026-07-14", updated: "2026-07-14", lastUpdated: "July 14, 2026 at 11:07 AM", time: "Jul 21 – Jul 27", whenText: "Tuesday, July 21, 2026, at 10:00 a.m. to Monday, July 27, 2026 at 8:00 p.m. local time", color: "#D4AF37", icon: "🎉", iconImg: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_willow.webp", featured: true, summary: "Pokémon GO's 10th Anniversary caps off Ultra Unlock with escalating Catch XP (6× → 8× → 10×), all 27 regional first-partner Pokémon spawning across three phases, party-hat costumed raids, and the debut of Professor Willow's Assistant Pikachu. Paid Masterwork Research 'Glimmers of Gratitude' (US$4.99) rewards a Shiny Shaymin. Runs July 21 (10 a.m.) to July 27 (8 p.m.) local time.", details: { bosses: ["Bulbasaur (Wild) ✨", "Charmander (Wild) ✨", "Squirtle (Wild) ✨", "Chikorita (Wild) ✨", "Cyndaquil (Wild) ✨", "Totodile (Wild) ✨", "Treecko (Wild) ✨", "Torchic (Wild) ✨", "Mudkip (Wild) ✨", "Turtwig (Wild) ✨", "Chimchar (Wild) ✨", "Piplup (Wild) ✨", "Snivy (Wild) ✨", "Tepig (Wild) ✨", "Oshawott (Wild) ✨", "Chespin (Wild) ✨", "Fennekin (Wild) ✨", "Froakie (Wild) ✨", "Rowlet (Wild) ✨", "Litten (Wild) ✨", "Popplio (Wild) ✨", "Grookey (Wild) ✨", "Scorbunny (Wild) ✨", "Sobble (Wild) ✨", "Sprigatito (Wild) ✨", "Fuecoco (Wild) ✨", "Quaxly (Wild) ✨", "Beldum (Lure) ✨", "Gible (Lure) ✨", "Dreepy (Lure)", "Grimer wearing a party hat (1★ Raid) ✨", "Raticate wearing a party hat (3★ Raid) ✨", "Nidorino wearing a party hat (3★ Raid) ✨", "Gengar wearing a party hat (3★ Raid) ✨", "Wobbuffet wearing a party hat (3★ Raid) ✨", "Professor Willow's Assistant Pikachu (1★ Raid) ✨"], bonuses: ["6× XP for catching Pokémon from July 21 (10:00 a.m.) to July 23 (10:00 a.m.)", "8× XP for catching Pokémon from July 23 (10:00 a.m.) to July 25 (10:00 a.m.)", "10× XP for catching Pokémon from July 25 (10:00 a.m.) to July 27 (8:00 p.m.)", "Your first partner Pokémon may have a team-specific Special Background, determined by your team when the encounter begins"], milestones: [{ tier: "Tier 1 – Rank 10", bonus: "2× Candy for catching Pokémon", deluxe: "3× Candy for catching Pokémon" }, { tier: "Tier 2 – Rank 20", bonus: "Increased chance of Candy XL from catching Pokémon", deluxe: "Increased chance of Candy XL from catching Pokémon" }], goPass: { free: ["Auto-granted July 21 at 10:00 a.m. local time; runs through July 27 at 8:00 p.m.", "10th Anniversary Tee (Yellow)", "Golden Lure Module", "Event-themed encounters", "Bonus XP window with no daily point limit: July 25 (12:00 a.m.) to July 27 (7:59 p.m.)", "Rewards expire July 29 at 7:59 p.m. local time"], deluxe: { price: "$4.99", rewards: ["Additional Golden Lure Module", "Additional event-themed encounters", "3× Candy for catching Pokémon at Rank 10"] }, deluxePlus: { price: "$6.99", rewards: ["Everything in GO Pass Deluxe", "Auto-grants Rank 7"] } }, tips: ["Featured Charged Attacks unlock during the event — Frenzy Plant for Grass-type starter final evolutions (Venusaur, Meganium, Sceptile, Torterra, Serperior, Chesnaught, Decidueye, Rillaboom, Meowscarada), Blast Burn for Fire-types (Charizard, Typhlosion, Blaziken, Infernape, Emboar, Delphox, Incineroar, Cinderace, Skeledirge), and Hydro Cannon for Water-types (Blastoise, Feraligatr, Swampert, Empoleon, Samurott, Greninja, Primarina, Inteleon, Quaquaval).", "Event-themed Field Research awards Poké Balls and encounters with event-themed Pokémon.", "Masterwork Research: Glimmers of Gratitude (US$4.99) rewards an encounter with Shiny Shaymin. Trainers who previously purchased this Masterwork story cannot buy it again.", "Free avatar items: Professor Willow–Style Lab Coat, Pants, Gloves, Shoes, and Backpack, plus Professor Willow Glasses. Event stickers available from PokéStops, Gifts, and the shop.", "Web Store — GO Pass Deluxe purchase gift: 10 Ultra Balls, 5 Max Revives, 1 Premium Battle Pass, 5 Max Potions.", "Web Store — GO Pass Deluxe + 6 Ranks Ultra Box (web exclusive): 20 Ultra Balls, 10 Max Revives, 10 Max Potions, 2 Premium Battle Passes, 1 Incubator, 1 Super Incubator.", "Professor Willow's Assistant Pikachu is available during Willow's Birthday Research (Jul 21, 10 a.m.–8 p.m.); the encounter itself is available through Jul 27, 8 p.m.", "All listed times are local."], relatedNews: [{ id: 31, label: "Ultra Unlock: 10th Anniversary Edition — Full Guide", icon: "🎉" }] } },
  { id: 120, title: "Ultra Unlock: Water Festival", type: "Event", url: "https://pokemongo.com/en/news/water-festival-2026", date: "2026-08-18", endDate: "2026-08-24", startsAt: "2026-08-18T10:00:00", endsAt: "2026-08-24T20:00:00", published: "2026-07-14", updated: "2026-07-14", lastUpdated: "July 14, 2026 at 11:07 AM", time: "Aug 18 – Aug 24", whenText: "Tuesday, August 18, at 10:00 a.m. to Monday, August 24, 2026, at 8:00 p.m. local time", color: "#2E86DE", icon: "🌊", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0845.webp", featured: true, summary: "Ultra Unlock: Water Festival brings Galar's Arrokuda, Barraskewda, and Cramorant to Pokémon GO, with escalating Stardust and XP catch bonuses, Water-type wild spawns, regional Tatsugiri forms, costumed raids (swim-ring Psyduck, scarf Lapras), and boosted Shiny Ducklett and Dewpider. Runs August 18 (10 a.m.) to August 24 (8 p.m.) local time.", details: { bosses: ["Psyduck (Wild)", "Goldeen (Wild) ✨", "Mareanie (Wild) ✨", "Wimpod (Wild)", "Feebas (Wild)", "Clamperl (Wild)", "Wiglett (Wild) (appears more frequently)", "Ducklett (Wild) ✨", "Dewpider (Wild) ✨", "Arrokuda (Wild) (Debut)", "Cramorant (Egg hatch) (Debut)", "Tatsugiri (Curly Form) (1★ Raid) — Europe, Middle East, Africa", "Tatsugiri (Droopy Form) (1★ Raid) — Americas", "Tatsugiri (Stretchy Form) (1★ Raid) — Asia-Pacific", "Psyduck wearing a swim ring (1★ Raid)", "Lapras wearing a scarf (3★ Raid)", "Hisuian Samurott (3★ Raid)", "Dondozo (3★ Raid)"], bonuses: ["2× XP and 3× Stardust for catching Pokémon from August 18 (10:00 a.m.) to August 20 (10:00 a.m.)", "3× XP and 4× Stardust for catching Pokémon from August 20 (10:00 a.m.) to August 22 (10:00 a.m.)", "4× XP and 5× Stardust for catching Pokémon from August 22 (10:00 a.m.) to August 24 (8:00 p.m.)", "Rainy Lure Modules attract Arrokuda", "Increased chance to encounter Shiny Ducklett and Shiny Dewpider"], milestones: [{ tier: "Rank 10", bonus: "2× Candy for catching Pokémon", deluxe: "2× Candy for catching Pokémon" }, { tier: "Rank 20", bonus: "1/2 Egg Hatch Distance for Eggs placed in an Incubator during the event", deluxe: "1/2 Egg Hatch Distance for Eggs placed in an Incubator during the event" }], goPass: { free: ["Auto-granted during the event", "Milestone bonuses (see above)"], deluxe: { price: "$4.99", rewards: ["GO Pass Deluxe rewards", "Web Store gift with purchase: Ultra Balls, Max Revives, Premium Battle Passes, Potions"] }, deluxePlus: { price: "$6.99", rewards: ["Everything in GO Pass Deluxe", "6 additional Ranks"] } }, tips: ["Arrokuda, Barraskewda, and Cramorant — first discovered in the Galar region — make their Pokémon GO debuts.", "Barraskewda debuts via evolution: evolve Arrokuda with 50 Candy.", "Cramorant has form-changing battle mechanics, is excluded from the Competitors Cup, and appears as a GO Battle League reward starting at Rank 16 (more frequently in the premium reward track).", "Shiny variants are available for select event Pokémon (check in-game shiny markers); Shiny Ducklett and Shiny Dewpider have increased encounter rates.", "Web Store bundles during the event include Ultra Balls, Max Revives, Premium Battle Passes, and Potions with purchase.", "Wiglett appears more frequently in the wild during this event.", "All listed times are local."], relatedNews: [{ id: 32, label: "Ultra Unlock: Water Festival — Full Guide", icon: "🌊" }] } },
  { id: 105, title: "The Road of Legends", type: "Raid", url: "https://pokemongo.com/en/news/road-of-legends-2026", date: "2026-07-06", endDate: "2026-07-10", startsAt: "2026-07-06T00:01:00", endsAt: "2026-07-10T23:59:00", published: "2026-06-18", updated: "2026-06-18", lastUpdated: "June 18, 2026 at 8:23 PM", time: "Jul 6 – Jul 10", whenText: "Monday, July 6, 2026, at 12:01 a.m. to Friday, July 10, 2026, at 11:59 p.m. local time", color: "#7C4DFF", icon: "⚔️", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0150.webp", featured: true, summary: "The five-day raid road to Pokémon GO Fest 2026: Global. Monday floods five-star raids with nearly every Legendary Pokémon and Ultra Beast, then each day spotlights marquee bosses — White and Black Kyurem, the Necrozma fusions, Crowned Sword Zacian and Crowned Shield Zamazenta, Origin Forme Dialga and Palkia — closing Friday with Primal Kyogre and Primal Groudon. No Remote Raid Pass limit, up to 2 free Raid Passes a day, souvenir Special Backgrounds, Elite TM Adventure Effects, and the GO Pass: Road of Legends.", details: { bossesTitle: "Marquee Raid Bosses", bosses: ["White Kyurem (5★ Raid)", "Black Kyurem (5★ Raid)", "Crowned Sword Zacian (5★ Raid)", "Crowned Shield Zamazenta (5★ Raid)", "Dawn Wings Necrozma (5★ Raid)", "Dusk Mane Necrozma (5★ Raid)", "Dialga (Origin Forme) (5★ Raid)", "Palkia (Origin Forme) (5★ Raid)", "Primal Kyogre (Primal Raid)", "Primal Groudon (Primal Raid)"], dailyRaids: [ { label: "Monday · July 6", accent: "#8E44AD", fiveStarRaids: ["Articuno (5★ Raid)", "Zapdos (5★ Raid)", "Moltres (5★ Raid)", "Raikou (5★ Raid)", "Entei (5★ Raid)", "Suicune (5★ Raid)", "Lugia (5★ Raid)", "Ho-Oh (5★ Raid)", "Regirock (5★ Raid)", "Regice (5★ Raid)", "Registeel (5★ Raid)", "Latias (5★ Raid)", "Latios (5★ Raid)", "Kyogre (5★ Raid)", "Groudon (5★ Raid)", "Rayquaza (5★ Raid)", "Normal Forme Deoxys (5★ Raid)", "Attack Forme Deoxys (5★ Raid)", "Defense Forme Deoxys (5★ Raid)", "Speed Forme Deoxys (5★ Raid)", "Uxie (5★ Raid)", "Mesprit (5★ Raid)", "Azelf (5★ Raid)", "Dialga (5★ Raid)", "Dialga (Origin Forme) (5★ Raid)", "Palkia (5★ Raid)", "Palkia (Origin Forme) (5★ Raid)", "Giratina (Altered Forme) (5★ Raid)", "Giratina (Origin Forme) (5★ Raid)", "Heatran (5★ Raid)", "Regigigas (5★ Raid)", "Cresselia (5★ Raid)", "Darkrai (5★ Raid)", "Cobalion (5★ Raid)", "Terrakion (5★ Raid)", "Virizion (5★ Raid)", "Tornadus (Incarnate Forme) (5★ Raid)", "Tornadus (Therian Forme) (5★ Raid)", "Thundurus (Incarnate Forme) (5★ Raid)", "Thundurus (Therian Forme) (5★ Raid)", "Reshiram (5★ Raid)", "Zekrom (5★ Raid)", "Kyurem (5★ Raid)", "Landorus (Incarnate Forme) (5★ Raid)", "Landorus (Therian Forme) (5★ Raid)", "Genesect (5★ Raid)", "Douse Drive Genesect (5★ Raid)", "Shock Drive Genesect (5★ Raid)", "Burn Drive Genesect (5★ Raid)", "Chill Drive Genesect (5★ Raid)", "Xerneas (5★ Raid)", "Yveltal (5★ Raid)", "Tapu Koko ✨ (5★ Raid)", "Tapu Lele ✨ (5★ Raid)", "Tapu Bulu ✨ (5★ Raid)", "Tapu Fini ✨ (5★ Raid)", "Solgaleo (5★ Raid)", "Lunala (5★ Raid)", "Necrozma (5★ Raid)", "Nihilego (5★ Raid)", "Buzzwole (5★ Raid)", "Pheromosa (5★ Raid)", "Xurkitree (5★ Raid)", "Celesteela (5★ Raid)", "Kartana (5★ Raid)", "Guzzlord (5★ Raid)", "Stakataka (5★ Raid)", "Blacephalon (5★ Raid)", "Zacian (Hero of Many Battles) (5★ Raid)", "Zamazenta (Hero of Many Battles) (5★ Raid)", "Regieleki (5★ Raid)", "Regidrago (5★ Raid)", "Enamorus (Incarnate Forme) (5★ Raid)", "Enamorus (Therian Forme) (5★ Raid)"], megaRaids: ["Mega Salamence (Mega)"], note: "Legendary Pokémon and Ultra Beasts appear in five-star raids all day. Uxie, Mesprit, Azelf and several Ultra Beasts are region-exclusive. Five-star Raid Hour runs 6:00–8:00 p.m.; Mega Raid Hour 7:00–8:00 p.m. local time." }, { label: "Tuesday · July 7", accent: "#3498DB", fiveStarRaids: ["White Kyurem (5★ Raid)", "Zekrom (5★ Raid)", "Dawn Wings Necrozma (5★ Raid)"], megaRaids: ["Mega Tyranitar (Mega)"], note: "White Kyurem caught from raids may know the Charged Attack Glaciate. Five-star Raid Hour 6:00–7:00 p.m.; Mega Raid Hour 7:00–8:00 p.m. local time." }, { label: "Wednesday · July 8", accent: "#16A085", fiveStarRaids: ["Black Kyurem (5★ Raid)", "Reshiram (5★ Raid)", "Dusk Mane Necrozma (5★ Raid)"], megaRaids: ["Mega Gardevoir (Mega)"], note: "Black Kyurem caught from raids may know the Charged Attack Glaciate. Five-star Raid Hour 6:00–7:00 p.m.; Mega Raid Hour 7:00–8:00 p.m. local time." }, { label: "Thursday · July 9", accent: "#E67E22", fiveStarRaids: ["Crowned Sword Zacian (5★ Raid)", "Crowned Shield Zamazenta (5★ Raid)"], megaRaids: ["Mega Gengar (Mega)"], note: "Five-star Raid Hour 6:00–7:00 p.m.; Mega Raid Hour 7:00–8:00 p.m. local time." }, { label: "Friday · July 10", accent: "#E74C3C", fiveStarRaids: ["Dialga (Origin Forme) (5★ Raid)", "Palkia (Origin Forme) (5★ Raid)"], primalRaids: ["Primal Kyogre (Primal Raid)", "Primal Groudon (Primal Raid)"], note: "Origin Forme Dialga may know Roar of Time and Origin Forme Palkia may know Spacial Rend — Adventure Effects available through Elite TM for the first time in Pokémon GO. Five-star Raid Hour 6:00–7:00 p.m.; Primal Raid Hour 7:00–8:00 p.m. local time." } ], eggLabel: "2 km Eggs", eggs: ["Weedle ✨", "Pidgey ✨", "Abra ✨", "Gastly ✨", "Pinsir ✨", "Aerodactyl ✨", "Mareep ✨", "Larvitar ✨", "Treecko ✨", "Torchic ✨", "Mudkip ✨", "Ralts ✨", "Bagon ✨", "Beldum ✨", "Gible ✨", "Riolu ✨", "Snover ✨", "Audino ✨", "Larvesta ✨"], seasonEggPools: [{ label: "5 km Eggs", items: ["Bulbasaur wearing Pikachu visor ✨", "Charmander wearing Pikachu visor ✨", "Squirtle wearing Pikachu visor ✨", "Heracross ✨", "Mime Jr. ✨", "Larvesta ✨"] }, { label: "10 km Eggs", items: ["Tropius ✨", "Axew ✨", "Bouffalant ✨", "Goomy", "Jangmo-o ✨", "Toxel ✨", "Frigibax ✨", "Larvesta ✨"] }], bonuses: ["No Remote Raid Pass limit (Monday, July 6 – Sunday, July 12, 2026)", "Up to 2 free Raid Passes per day from spinning Photo Discs at Gyms (Road of Legends only)", "Trainers level 31 and up are guaranteed to receive Candy XL for trading Pokémon", "Premier Balls are more effective at catching Pokémon after raids", "Party Power charges faster in raids", "Special event stickers from spinning PokéStops and opening Gifts", "Special Backgrounds: Pokémon caught from five-star, Mega, and Primal raids July 6–12 have a chance at a souvenir Special Background", "Featured Charged Attacks: Kyurem may know Glaciate; Origin Forme Dialga may know Roar of Time; Origin Forme Palkia may know Spacial Rend", "Roar of Time and Spacial Rend are available through Elite TM for the first time in Pokémon GO", "Raid Hours: five-star feature 6:00–7:00 p.m. daily (Monday 6:00–8:00 p.m.); Mega Raid Hour 7:00–8:00 p.m.; Friday Primal Raid Hour 7:00–8:00 p.m.", "Daily Discoveries will not take place during Road of Legends or GO Fest 2026: Global (July 6 – July 12)"], goPass: { free: ["Team-hat Pikachu encounters", "Candy from Legendary raid Pokémon", "GO Fest Tee avatar item", "Link Charges, Incubators, and Incense", "Rank 100: Lucky Trinket", "Increases the Link Charge cap to 2,000 through GO Fest 2026: Global"], deluxe: { price: "Paid", rewards: ["Everything in the free track", "Team Leader Pose (avatar)", "Rank 100: Gold Bottle Cap + Mewtwo with a Special Background", "Candy XL", "2× Hatch Stardust and 2× Hatch Candy", "An additional 5,000 XP from raids", "An additional 3 Candy from five-star and higher raids", "An additional 1 Candy XL from five-star and higher raids", "1/2 Egg Hatch Distance"] } }, tips: ["Monday is the 'everything' day — nearly every Legendary and Ultra Beast is in five-star raids; with no Remote Raid Pass limit, this is the day to clear your dex.", "GO Pass: Road of Legends runs Monday, July 6 (12:01 a.m.) to Sunday, July 12 (11:59 p.m.) local time — it carries through GO Fest 2026: Global.", "Elite TM Roar of Time onto Origin Forme Dialga or Spacial Rend onto Origin Forme Palkia — the first time these Adventure Effects are available, and only for a limited time.", "Kyurem caught from raids may know Glaciate — grab one on Monday, and White/Black Kyurem on their featured days.", "Up to 2 free Raid Passes per day from Gyms, plus no Remote Raid Pass limit — stack daily raids efficiently.", "Friday closes with Primal Kyogre and Primal Groudon — bring Primal counters and a full lobby.", "All listed times are local. Five-star Raid Hour is 6–7 p.m. (Monday 6–8 p.m.); Mega and Primal Raid Hours are 7–8 p.m.", "Pikachu has a few playful surprises lined up starting July 13 at 10:00 a.m. local time — more to be announced."], relatedNews: [{ id: 25, label: "The Road of Legends — Five Days of Legendary Raids", icon: "⚔️" }] } },
  { id: 116, title: "5★ Raid: Legendary Birds", type: "Raid", date: "2026-07-01", endDate: "2026-07-05", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 5:08 PM", time: "Raid Hour: Wed Jul 1, 6–7 PM", whenText: "Wednesday, July 1 to Sunday, July 5, 2026 — Raid Hour Wednesday, July 1, from 6:00 to 7:00 p.m. local time", color: "#5499C7", icon: "🦅", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0144.webp", featured: true, summary: "The Kanto Legendary Birds open July's 5-Star Raid rotation — Articuno, Zapdos, and Moltres, all with Shiny available. Mega Lucario headlines Mega Raids and Shadow Palkia runs in 5★ Shadow Raids. Five-star Raid Hour is Wednesday, July 1, 6–7 PM.", details: { bosses: ["Articuno ✨ (5★ Raid)", "Zapdos ✨ (5★ Raid)", "Moltres ✨ (5★ Raid)", "Mega Lucario ✨ (Mega)", "Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Articuno", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Zapdos", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Moltres", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Lucario", tier: "Mega Raid", minimum: 1, optimalMin: 2, optimalMax: 3, easyAt: 4, greenAt: 4 }, { bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday July 1, 6–7 PM local", "Articuno, Zapdos, and Moltres share 5-Star Raids July 1–5", "Mega Lucario in Mega Raids", "Shadow Palkia in 5★ Shadow Raids (July 1 – August 4)"], tips: ["Articuno (Ice/Flying) is weak to Rock, Fire, Electric, and Steel — Rock attackers shine.", "Zapdos (Electric/Flying) is weak to Rock and Ice.", "Moltres (Fire/Flying) is double weak to Rock — bring your best Rock throwers.", "All three birds have Shiny available — check every catch.", "Mega Lucario (Fighting/Steel) is weak to Fire, Fighting, and Ground.", "Shadow Palkia (Water/Dragon) runs all month in 5★ Shadow Raids — bring Purified Gems."], counters: [{ label: "Articuno (Ice/Flying)", pokemon: [{ name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Shadow Rampardos", fast: "Smack Down", charged: "Rock Slide" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Mega Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }, { name: "Shadow Gigalith", fast: "Lock-On", charged: "Meteor Beam", chargedNote: "Elite TM" }, { name: "Shadow Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }] }, { label: "Zapdos (Electric/Flying)", pokemon: [{ name: "White Kyurem", fast: "Ice Fang", charged: "Ice Burn", chargedNote: "Signature" }, { name: "Black Kyurem", fast: "Dragon Tail", charged: "Freeze Shock", chargedNote: "Signature" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Mega Mewtwo X", fast: "Psycho Cut", charged: "Ice Beam" }] }, { label: "Moltres (Fire/Flying)", pokemon: [{ name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Shadow Rampardos", fast: "Smack Down", charged: "Rock Slide" }, { name: "Shadow Tyrantrum", fast: "Rock Throw", charged: "Meteor Beam", chargedNote: "Elite TM" }, { name: "Mega Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }, { name: "Shadow Gigalith", fast: "Lock-On", charged: "Meteor Beam", chargedNote: "Elite TM" }] }, { label: "Mega Lucario (Fighting/Steel)", pokemon: [{ name: "Mega Blaziken", fast: "Fire Spin", charged: "Aura Sphere" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Lucario", fast: "Force Palm", charged: "Aura Sphere", fastNote: "Legacy" }, { name: "Shadow Reshiram", fast: "Fire Fang", charged: "Fusion Flare", chargedNote: "Signature" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword", chargedNote: "Signature" }] }, { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] }] } },
  { id: 107, title: "5★ Raid: Legendary Birds (Return)", type: "Raid", date: "2026-07-13", endDate: "2026-07-14", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 5:08 PM", time: "Jul 13 – Jul 14", whenText: "Monday, July 13 to Tuesday, July 14, 2026, local time", color: "#5499C7", icon: "🦅", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0145.webp", featured: false, summary: "Articuno, Zapdos, and Moltres make a brief return to 5-Star Raids on July 13–14 between GO Fest and the Kyogre rotation. Mega Lucario continues in Mega Raids; Shadow Palkia continues in 5★ Shadow Raids.", details: { bosses: ["Articuno ✨ (5★ Raid)", "Zapdos ✨ (5★ Raid)", "Moltres ✨ (5★ Raid)", "Mega Lucario ✨ (Mega)", "Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Articuno", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Zapdos", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Moltres", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Lucario", tier: "Mega Raid", minimum: 1, optimalMin: 2, optimalMax: 3, easyAt: 4, greenAt: 4 }, { bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Articuno, Zapdos, and Moltres return to 5-Star Raids July 13–14", "Mega Lucario continues in Mega Raids", "Shadow Palkia continues in 5★ Shadow Raids"], tips: ["A short two-day window — grab any birds you still need (and Shiny checks).", "Same counters as the July 1–5 birds rotation.", "Mega Lucario stays in Mega Raids through July 14."], counters: [{ label: "Articuno (Ice/Flying)", pokemon: [{ name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Shadow Rampardos", fast: "Smack Down", charged: "Rock Slide" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Mega Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }, { name: "Shadow Gigalith", fast: "Lock-On", charged: "Meteor Beam", chargedNote: "Elite TM" }, { name: "Shadow Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }] }, { label: "Zapdos (Electric/Flying)", pokemon: [{ name: "White Kyurem", fast: "Ice Fang", charged: "Ice Burn", chargedNote: "Signature" }, { name: "Black Kyurem", fast: "Dragon Tail", charged: "Freeze Shock", chargedNote: "Signature" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Mega Mewtwo X", fast: "Psycho Cut", charged: "Ice Beam" }] }, { label: "Moltres (Fire/Flying)", pokemon: [{ name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Move" }, { name: "Shadow Rampardos", fast: "Smack Down", charged: "Rock Slide" }, { name: "Shadow Tyrantrum", fast: "Rock Throw", charged: "Meteor Beam", chargedNote: "Elite TM" }, { name: "Mega Tyranitar", fast: "Smack Down", charged: "Stone Edge", fastNote: "Legacy" }, { name: "Shadow Gigalith", fast: "Lock-On", charged: "Meteor Beam", chargedNote: "Elite TM" }] }, { label: "Mega Lucario (Fighting/Steel)", pokemon: [{ name: "Mega Blaziken", fast: "Fire Spin", charged: "Aura Sphere" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Lucario", fast: "Force Palm", charged: "Aura Sphere", fastNote: "Legacy" }, { name: "Shadow Reshiram", fast: "Fire Fang", charged: "Fusion Flare", chargedNote: "Signature" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword", chargedNote: "Signature" }] }, { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] }] } },
  { id: 108, title: "5★ Raid: Kyogre", type: "Raid", date: "2026-07-15", endDate: "2026-07-21", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 4:55 PM", time: "Raid Hour: Wed Jul 15, 6–7 PM", whenText: "Wednesday, July 15 to Tuesday, July 21, 2026 — Raid Hour Wednesday, July 15, from 6:00 to 7:00 p.m. local time", color: "#2E86C1", icon: "🌊", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-3_Hoenn/0382.webp", featured: true, summary: "Kyogre, the Sea Basin Pokémon, takes over 5-Star Raids — a top Water-type attacker with Shiny available. Mega Sceptile headlines Mega Raids and Shadow Palkia continues in 5★ Shadow Raids. Five-star Raid Hour is Wednesday, July 15, 6–7 PM.", details: { bosses: ["Kyogre ✨ (5★ Raid)", "Mega Sceptile ✨ (Mega)", "Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Kyogre", tier: "5-Star Raid", minimum: 3, optimalMin: 5, optimalMax: 8, easyAt: 9, greenAt: 9 }, { bossName: "Mega Sceptile", tier: "Mega Raid", minimum: 1, optimalMin: 2, optimalMax: 3, easyAt: 4, greenAt: 4 }, { bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday July 15, 6–7 PM local", "Mega Sceptile in Mega Raids", "Shadow Palkia continues in 5★ Shadow Raids"], tips: ["Kyogre (Water) is weak to Grass and Electric.", "Shiny Kyogre is available — one of the best Water attackers in the game.", "Power up Kyogre for Origin Dialga, Mega Aggron, and other future raids.", "Mega Sceptile (Grass/Dragon) is weak to Ice, Dragon, Fairy, Bug, Flying, and Poison.", "Ice attackers (White/Black Kyurem) cover both Kyogre's grass counters and Mega Sceptile."], counters: [{ label: "Kyogre (Water)", pokemon: [{ name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Thunderbolt" }, { name: "Mega Mewtwo X", fast: "Psycho Cut", charged: "Thunderbolt" }, { name: "Mega Sceptile", fast: "Fury Cutter", charged: "Frenzy Plant", chargedNote: "CD Move" }, { name: "Zeraora", fast: "Volt Switch", charged: "Plasma Fists", chargedNote: "Signature" }, { name: "Kartana", fast: "Razor Leaf", charged: "Leaf Blade" }, { name: "Shadow Raikou", fast: "Thunder Shock", charged: "Wild Charge" }] }, { label: "Mega Sceptile (Grass/Dragon)", pokemon: [{ name: "White Kyurem", fast: "Ice Fang", charged: "Ice Burn", chargedNote: "Signature" }, { name: "Black Kyurem", fast: "Dragon Tail", charged: "Freeze Shock", chargedNote: "Signature" }, { name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Dragon Ascent", chargedNote: "Signature" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Mega Mewtwo X", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Eternatus", fast: "Poison Jab", charged: "Dynamax Cannon", chargedNote: "Signature" }] }, { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] }] } },
  { id: 109, title: "5★ Raid: Solgaleo", type: "Raid", date: "2026-07-22", endDate: "2026-07-28", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "Raid Hour: Wed Jul 22, 6–7 PM", whenText: "Wednesday, July 22 to Tuesday, July 28, 2026 — Raid Hour Wednesday, July 22, from 6:00 to 7:00 p.m. local time", color: "#F5B041", icon: "🌞", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-7_Alola/0791.webp", featured: true, summary: "Shiny Solgaleo DEBUTS! The Sunne Pokémon (Psychic/Steel) headlines 5-Star Raids July 22–28 with its Shiny release. Mega Salamence takes Mega Raids and Shadow Palkia continues in 5★ Shadow Raids. Five-star Raid Hour is Wednesday, July 22, 6–7 PM.", details: { bosses: ["Solgaleo ✨ (5★ Raid)", "Mega Salamence ✨ (Mega)", "Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Solgaleo", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Salamence", tier: "Mega Raid", minimum: 2, optimalMin: 2, optimalMax: 4, easyAt: 5, greenAt: 5 }, { bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday July 22, 6–7 PM local", "Shiny Solgaleo debut — first chance at a Shiny Solgaleo", "Mega Salamence in Mega Raids", "Shadow Palkia continues in 5★ Shadow Raids"], tips: ["Shiny Solgaleo debuts here — top priority, check every encounter.", "Solgaleo (Psychic/Steel) is weak to Fire, Ground, Ghost, and Dark.", "Fire attackers (Shadow Reshiram, Mega Blaziken, Mega Charizard Y) hit both of Solgaleo's typings.", "Mega Salamence (Dragon/Flying) is double weak to Ice — White Kyurem and Mega Mewtwo Y with Ice Beam excel.", "Solgaleo Candy banks toward fusing Necrozma into Dusk Mane Necrozma."], counters: [{ label: "Solgaleo (Psychic/Steel)", pokemon: [{ name: "Shadow Reshiram", fast: "Fire Fang", charged: "Fusion Flare", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Blacephalon", fast: "Astonish", charged: "Mind Blown", chargedNote: "Signature" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Dawn Wings Necrozma", fast: "Shadow Claw", charged: "Moongeist Beam", chargedNote: "Signature" }] }, { label: "Mega Salamence (Dragon/Flying)", pokemon: [{ name: "White Kyurem", fast: "Ice Fang", charged: "Ice Burn", chargedNote: "Signature" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Mega Mewtwo X", fast: "Psycho Cut", charged: "Ice Beam" }, { name: "Black Kyurem", fast: "Dragon Tail", charged: "Freeze Shock", chargedNote: "Signature" }, { name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }] }, { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] }] } },
  { id: 110, title: "5★ Raid: Kyurem", type: "Raid", date: "2026-07-29", endDate: "2026-08-04", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 5:02 PM", time: "Raid Hour: Wed Jul 29, 6–7 PM", whenText: "Wednesday, July 29 to Tuesday, August 4, 2026 — Raid Hour Wednesday, July 29, from 6:00 to 7:00 p.m. local time", color: "#85C1E9", icon: "❄️", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0646.webp", featured: true, summary: "Kyurem, the Boundary Pokémon (Dragon/Ice), closes out July's 5-Star Raid rotation with Shiny available. Mega Aggron headlines Mega Raids and Shadow Palkia wraps its Shadow Raid run. Five-star Raid Hour is Wednesday, July 29, 6–7 PM.", details: { bosses: ["Kyurem ✨ (5★ Raid)", "Mega Aggron ✨ (Mega)", "Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Kyurem", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Aggron", tier: "Mega Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday July 29, 6–7 PM local", "Mega Aggron in Mega Raids", "Shadow Palkia continues in 5★ Shadow Raids (through August 4)"], tips: ["Kyurem (Dragon/Ice) is weak to Dragon, Fairy, Rock, Steel, and Fighting.", "Steel attackers (Crowned Sword Zacian, Crowned Shield Zamazenta, Dusk Mane Necrozma) are top counters.", "Shiny Kyurem is available — and Kyurem fuses with Reshiram/Zekrom into White/Black Kyurem.", "Mega Aggron (Steel) is weak to Fire, Fighting, and Ground.", "Fighting attackers (Mega Lucario, Mega Blaziken, Keldeo) double up against Kyurem and Mega Aggron."], counters: [{ label: "Kyurem (Dragon/Ice)", pokemon: [{ name: "Crowned Sword Zacian", fast: "Metal Claw", charged: "Behemoth Blade", chargedNote: "Signature" }, { name: "Crowned Shield Zamazenta", fast: "Metal Claw", charged: "Behemoth Bash", chargedNote: "Signature" }, { name: "Mega Lucario", fast: "Force Palm", charged: "Aura Sphere", fastNote: "Legacy" }, { name: "Dusk Mane Necrozma", fast: "Metal Claw", charged: "Sunsteel Strike", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword", chargedNote: "Signature" }] }, { label: "Mega Aggron (Steel)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", charged: "Aura Sphere", fastNote: "Legacy" }, { name: "Mega Blaziken", fast: "Fire Spin", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }, { name: "Shadow Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }] }, { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] }] } },
  { id: 111, title: "Shadow Palkia in Shadow Raids", type: "Raid", date: "2026-07-01", endDate: "2026-08-04", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "All month", whenText: "Wednesday, July 1 to Tuesday, August 4, 2026, local time", color: "#7B2FBE", icon: "🌑", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-4_Sinnoh/0484.webp", shadowBg: true, featured: true, summary: "Shadow Palkia (Water/Dragon) appears in 5★ Shadow Raids from July 1 through August 4. Shiny Shadow Palkia is available — bring Purified Gems and Dragon-busting counters.", details: { bosses: ["Shadow Palkia ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Shadow Palkia", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Available in 5★ Shadow Raids from July 1 to August 4", "Purified Gems weaken Shadow Palkia during the raid", "Shiny Shadow Palkia is available"], tips: ["Shadow Palkia is Water/Dragon — weak to Dragon and Fairy.", "Top counters: Mega Rayquaza, Eternatus, Mega Garchomp, Shadow Palkia, Mega Salamence, and Origin Forme Palkia.", "Bring Purified Gems early in the raid to reduce damage taken.", "Shiny Shadow Palkia is a top priority catch.", "Fairy attackers like Mega Gardevoir and Xerneas are safe answers that resist Dragon."], counters: { label: "Shadow Palkia (Water/Dragon)", pokemon: [{ name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Mega Salamence", fast: "Dragon Tail", charged: "Draco Meteor" }, { name: "Palkia (Origin Forme)", fast: "Dragon Tail", charged: "Spacial Rend", chargedNote: "Signature" }] } } },
  { id: 112, title: "Dynamax Chansey", type: "Max Battle", date: "2026-07-06", endDate: "2026-07-12", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "Max Battles: Jul 6 – Jul 12", whenText: "Monday, July 6 to Sunday, July 12, 2026, local time", color: "#F5B7B1", icon: "💗", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0113-female.webp", featured: false, summary: "Dynamax Chansey appears in Max Battles at Power Spots throughout Forever Forward, July 6–12. Shiny Chansey available.", details: { bosses: ["Dynamax Chansey ✨"], bonuses: ["Dynamax Chansey in Max Battles July 6–12", "Power Spots all week", "Shiny Chansey available"], tips: ["Chansey (Normal) is weak only to Fighting — bring Max Strike/Fighting Dynamax attackers.", "Chansey gives huge Stardust on catch and evolves into Blissey, a top defender.", "Stockpile Max Particles at Power Spots before the battle."] } },
  { id: 113, title: "Dynamax Deino", type: "Max Battle", date: "2026-07-13", endDate: "2026-07-19", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "Max Battles: Jul 13 – Jul 19", whenText: "Monday, July 13 to Sunday, July 19, 2026, local time", color: "#5D4037", icon: "🐉", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0633.webp", featured: false, summary: "Dynamax Deino appears in Max Battles at Power Spots, July 13–19. Shiny Deino available.", details: { bosses: ["Dynamax Deino ✨"], bonuses: ["Dynamax Deino in Max Battles July 13–19", "Power Spots all week", "Shiny Deino available"], tips: ["Deino (Dark/Dragon) is weak to Fairy, Ice, Dragon, Bug, and Fighting.", "Bank Deino Candy — Hydreigon is a strong Dark/Dragon attacker.", "Shiny Deino is available; check every encounter."] } },
  { id: 114, title: "Dynamax Trubbish", type: "Max Battle", date: "2026-07-20", endDate: "2026-07-26", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "Max Battles: Jul 20 – Jul 26", whenText: "Monday, July 20 to Sunday, July 26, 2026, local time", color: "#8E44AD", icon: "🗑️", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0568.webp", featured: false, summary: "Dynamax Trubbish appears in Max Battles at Power Spots, July 20–26. Shiny Trubbish available.", details: { bosses: ["Dynamax Trubbish ✨"], bonuses: ["Dynamax Trubbish in Max Battles July 20–26", "Power Spots all week", "Shiny Trubbish available"], tips: ["Trubbish (Poison) is weak to Ground and Psychic.", "Evolves into Garbodor; a budget-friendly Max Battle target.", "Shiny Trubbish is available."] } },
  { id: 115, title: "Dynamax Feebas", type: "Max Battle", date: "2026-07-27", endDate: "2026-08-02", published: "2026-06-25", updated: "2026-06-25", lastUpdated: "June 25, 2026 at 3:42 PM", time: "Max Battles: Jul 27 – Aug 2", whenText: "Monday, July 27 to Sunday, August 2, 2026, local time", color: "#5DADE2", icon: "🐟", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-3_Hoenn/0349.webp", featured: false, summary: "Dynamax Feebas appears in Max Battles at Power Spots, July 27 – August 2. Shiny Feebas available.", details: { bosses: ["Dynamax Feebas ✨"], bonuses: ["Dynamax Feebas in Max Battles July 27 – August 2", "Power Spots all week", "Shiny Feebas available"], tips: ["Feebas (Water) is weak to Grass and Electric.", "Evolves into Milotic — bank Candy for a great PvP pick.", "Shiny Feebas is available; this is a great chance to grab one."] } },
  { id: 117, title: "Happy Birthday, Professor Willow!", type: "Event", url: "https://pokemongo.com/en/news/happy-birthday-professor-willow-2026", date: "2026-07-21", endDate: null, startsAt: "2026-07-21T10:00:00", endsAt: "2026-07-21T20:00:00", published: "2026-06-26", updated: "2026-06-26", lastUpdated: "June 26, 2026 at 10:39 AM", time: "Jul 21", whenText: "Tuesday, July 21, 2026, from 10:00 a.m. to 8:00 p.m. local time", color: "#F4C20D", icon: "🎂", iconImg: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_willow.webp", featured: false, summary: "Celebrate Professor Willow's birthday on July 21! A brand-new costumed Pokémon — Professor Willow's assistant Pikachu — debuts in Field Research, Timed Research, snapshot surprises, and raids, and it can be Shiny (or sport a team leader–specific Special Background). A free Timed Research line rewards Pikachu encounters, a Lucky Egg, and a Star Piece (complete it by July 27), while event-themed Field Research gives PokéCoins, Lucky Eggs, and more Pikachu encounters. Runs 10:00 a.m. to 8:00 p.m. local time.", details: { bosses: ["Professor Willow's assistant Pikachu (Research, snapshots & raids) ✨"], bonuses: ["New costumed Pokémon debut: Professor Willow's assistant Pikachu appears in Field Research, Timed Research, snapshot surprises, and raids.", "If you're lucky, you might encounter a Shiny one — or one with a team leader–specific Special Background.", "Free Timed Research: encounters with Professor Willow's assistant Pikachu, 1 Lucky Egg, 1 Star Piece, and more (you might encounter a Shiny one).", "Timed Research tasks must be completed and their rewards claimed before Monday, July 27, 2026, at 8:00 p.m. local time.", "Event-themed Field Research: PokéCoins, Lucky Eggs, additional Professor Willow's assistant Pikachu encounters, and more."], tips: ["The costumed Pikachu can be Shiny — check every research and snapshot encounter.", "Take snapshots during the event for surprise Professor Willow's assistant Pikachu appearances.", "Don't let the free Timed Research expire — finish it and claim rewards by July 27 at 8:00 p.m. local time.", "All listed times are local."], relatedNews: [{ id: 28, label: "Happy Birthday, Professor Willow! — Willow's Birthday Research", icon: "🎂" }] } },
  { id: 104, title: "Raichu Super Mega Raid Day", type: "Raid", url: "https://pokemongo.com/news/raichu-super-mega-raid-day-2026", date: "2026-07-18", endDate: null, time: "2:00 PM – 5:00 PM", whenText: "Saturday, July 18, 2026, from 2:00 p.m. to 5:00 p.m. local time", color: "#F1C40F", icon: "⚡", published: "2026-06-15", updated: "2026-06-15", lastUpdated: "June 15, 2026 at 1:10 PM", iconImg: "assets/pokemon-images/Mega/regular/Gen-1_Kanto/0026_megay.webp", featured: true, summary: "Mega Raichu X and Mega Raichu Y both debut in Super Mega Raids on Saturday, July 18! Raichu caught from these raids come with Mega Level 1 unlocked, plus increased Shiny Raichu odds, up to 6 free Raid Passes, the Remote Raid limit raised to 20, and a free Timed Research line rewarding a Premium Battle Pass and a Steelix encounter.", details: { bosses: ["★Mega Raichu X (Super Mega Raid)", "★Mega Raichu Y (Super Mega Raid)"], groupSize: [{ bossName: "Mega Raichu X", tier: "Super Mega Raid", minimum: 8, optimalMin: 8, optimalMax: 20, easyAt: 8, greenAt: 8 }, { bossName: "Mega Raichu Y", tier: "Super Mega Raid", minimum: 8, optimalMin: 8, optimalMax: 20, easyAt: 8, greenAt: 8 }], bonuses: [{ text: "Up to 6 free Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" }, { text: "Raichu caught from Super Mega Raids will have Mega Level 1 unlocked", img: "assets/pokemon-images/icons/dna-helix-icon.webp" }, { text: "Increased chance of encountering Shiny Raichu from Super Mega Raids", img: "assets/pokemon-images/icons/shiny-sparkles.webp" }, { text: "Remote Raid Pass limit increased to 20 (Fri, July 17, 5:00 PM – Sat, July 18, 8:00 PM PDT)", img: "assets/pokemon-images/Items/remote-raid-pass.png" }, { text: "Super Mega Raids may only appear at certain Gyms — locate them via pokemongo.com/map", img: "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png" }, { text: "Free Timed Research available during event hours — rewards a Premium Battle Pass and a Steelix encounter", img: "assets/pokemon-images/icons/green-research.png" }], timedResearch: { free: { rewards: ["Premium Battle Pass", "Steelix encounter"], note: "Timed Research expires — tasks must be completed and rewards claimed before the Timed Research ends." } }, ticketPrice: "$4.99", ticketNote: "These bonuses will be effective on Saturday, July 18, 2026, from 2:00 p.m. to 5:00 p.m. local time.", ticketBonuses: [{ text: "Up to 14 Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" }, { text: "Increased chance to get Rare Candy XL from Raid Battles", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png" }, { text: "An additional 5,000 XP from Super Mega Raids", img: "assets/pokemon-images/icons/xp.png" }, { text: "An additional 5,000 Stardust from Super Mega Raids", img: "assets/pokemon-images/Items/stardust_painted.png" }, "Tickets giftable to Great Friends or higher", "Purchases are non-refundable — cannot be bought with PokéCoins", "Ticket available in the in-game shop until Saturday, July 18 at 5:00 PM local time", "Ultra Ticket Box $4.99 on Web Store (event ticket + Premium Battle Pass at no additional cost)"], tips: ["Mega Raichu X and Mega Raichu Y both make their Pokémon GO debut — top priority for collectors.", "Mega Raichu Y is set to be the best Electric-type attacker in the game — power it up for future raids.", "Both Mega Raichu X and Y are pure Electric — they are weak only to Ground-type attacks.", "Super Mega Raids require at least 8 Trainers because of the Shield phases — coordinate a full lobby (lobby cap 20).", "Raichu caught from these raids come with Mega Level 1 already unlocked — no extra Mega Energy needed for the first Mega Evolution.", "Shiny Raichu has boosted odds from Super Mega Raids — check every encounter.", "Free Timed Research includes a Steelix encounter — complete and claim before it expires.", "Ultra Ticket Box is Web Store exclusive — grab it before the event for a free Premium Battle Pass.", "Ticket only available in the in-game shop until 5:00 PM local on July 18 — buy early.", "Find nearby Super Mega Raids and Community Ambassador meetups via pokemongo.com/map."], counters: { label: "Mega Raichu X & Y (Electric)", bosses: ["Mega Raichu X", "Mega Raichu Y"], pokemon: [{ name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power" }, { name: "Shadow Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Shadow Garchomp", fast: "Mud Shot", charged: "Earth Power" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Landorus (Therian Forme)", fast: "Mud Shot", charged: "Sandsear Storm", chargedNote: "Signature" }] } } },
  { id: 103, title: "10th Anniversary Party", type: "Event", url: "https://pokemongo.com/news/10th-anniversary-party", date: "2026-07-04", endDate: "2026-07-06", startsAt: "2026-07-04T10:00:00", endsAt: "2026-07-06T20:00:00", published: "2026-06-11", updated: "2026-06-11", lastUpdated: "June 11, 2026 at 7:47 PM", time: "10:00 AM – 8:00 PM", whenText: "Saturday, July 4, at 10:00 a.m. to Monday, July 6, 2026, at 8:00 p.m. local time", color: "#FFB300", icon: "🎉", iconImg: "assets/pokemon-images/Event-Dex/regular/Gen-9_Paldea/0999_anniversary_badge.webp", featured: true, summary: "Pokémon GO turns 10! Gimmighoul holding a 10th anniversary coin debuts via Golden Lure Modules and Coin Bags (with a Shiny chance), party-hat Kanto starters and a cake-hat Pikachu fill the wild, and 4× Catch XP + 4× Catch Stardust run all weekend.", details: { bosses: ["Pikachu wearing a cake hat ✨", "Eevee wearing a party hat ✨", "Wurmple wearing a party hat ✨", "Bulbasaur wearing a party hat ✨", "Charmander wearing a party hat ✨", "Squirtle wearing a party hat ✨", "Gimmighoul holding a 10th anniversary coin (debut) ✨", "Other event-themed Pokémon in the wild"], bonuses: [{ text: "4× XP for catching Pokémon", img: "assets/pokemon-images/icons/xp.png" }, { text: "4× Stardust for catching Pokémon", img: "assets/pokemon-images/Items/stardust_painted.png" }, { text: "Event-themed Pokémon appearing in the wild are more likely to be Shiny", img: "assets/pokemon-images/icons/shiny-sparkles.webp" }, { text: "Gimmighoul holding a 10th anniversary coin debuts via Golden Lure Modules and Coin Bags (available July 4 – July 31)", img: "assets/pokemon-images/Items/TroyKey_sparkly.png" }, { text: "Gimmighoul holding a 10th anniversary coin may have a Special Background", img: "assets/pokemon-images/Items/tx_ar_photo_camera.png" }, { text: "Use a Coin Bag during the event for a chance at Shiny Gimmighoul", img: "assets/pokemon-images/Items/sparkly_incense.png", imgSize: 32 }, { text: "Evolve Bulbasaur or Ivysaur during the event for a Venusaur that knows Frenzy Plant", img: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0003-male.webp", imgSize: 34 }, { text: "Evolve Charmander or Charmeleon during the event for a Charizard that knows Blast Burn", img: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0006.webp", imgSize: 32 }, { text: "Evolve Squirtle or Wartortle during the event for a Blastoise that knows Hydro Cannon", img: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0009.webp", imgSize: 30 }, { text: "Event-themed Field Research available", img: "assets/pokemon-images/icons/green-research.png" }, { text: "New avatar item, plus event-themed stickers from spinning PokéStops, opening Gifts, and the in-game shop", img: "assets/pokemon-images/icons/QuestGiftIcon.png" }], paidResearch: { price: "$1.99", rewards: [{ text: "1 Super Incubator", img: "assets/pokemon-images/Items/super_incubator.webp" }, { text: "1 Premium Battle Pass", img: "assets/pokemon-images/Items/premium-raid-pass.png" }, { text: "An encounter with Gimmighoul holding a 10th anniversary coin (with a Special Background)", img: "assets/pokemon-images/Event-Dex/regular/Gen-9_Paldea/0999_anniversary_badge.webp", imgSize: 34 }, { text: "And more", glyph: "+" }], note: "Paid Timed Research, also available as a $1.99 Web Store bundle. Must be completed by Monday, July 6, 2026 at 8:00 PM local time. Giftable to Great Friends or higher." }, tips: ["Stack the 4× Catch XP with a Lucky Egg for a huge XP weekend.", "Shiny Gimmighoul only comes from Coin Bags — open them during the event window.", "Gimmighoul is obtainable through July 31 via Golden Lure Modules and Coin Bags.", "Evolve your party-hat Kanto starters during the event for the exclusive Frenzy Plant / Blast Burn / Hydro Cannon moves.", "All listed times are local."] } },
  ...
  { id: 96, title: "Choose Your Path: Venom and Vines", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-08-11", endDate: "2026-08-17", startsAt: "2026-08-11T10:00:00", endsAt: "2026-08-17T20:00:00", published: "2026-06-10", time: "Aug 11, 10 AM – Aug 17, 8 PM", color: "#6B8E23", icon: "🧭", featured: false, summary: "Choose Your Path Timed Research — pick Explore, Catch, or Battle to shape the tasks and bonuses for this non-event week.", details: { bosses: ["Explore Path", "Catch Path", "Battle Path"], bonuses: ["Select one of three paths: Explore, Catch, or Battle", "Your choice shapes the Timed Research tasks and bonuses for the week", "Available during this non-event week only"], tips: ["You may get different research opportunities than other Trainers.", "All listed times are local.", "Part of the Forever Forward Choose Your Path Timed Research series."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 97, title: "Spotlight Hour: Swinub", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-06-18", endDate: "2026-06-18", startsAt: "2026-06-18T18:00:00", endsAt: "2026-06-18T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Swinub in the spotlight — 2× Candy for transferring Pokémon. Shiny Swinub may appear.", details: { bosses: ["Swinub ✨"], bonuses: ["2× Candy for transferring Pokémon", "Shiny Swinub may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 98, title: "Spotlight Hour: Wingull", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-06-25", endDate: "2026-06-25", startsAt: "2026-06-25T18:00:00", endsAt: "2026-06-25T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Wingull in the spotlight — 2× Stardust for catching Pokémon. Shiny Wingull may appear.", details: { bosses: ["Wingull ✨"], bonuses: ["2× Stardust for catching Pokémon", "Shiny Wingull may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 99, title: "Spotlight Hour: Pidgey", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-07-02", endDate: "2026-07-02", startsAt: "2026-07-02T18:00:00", endsAt: "2026-07-02T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Pidgey in the spotlight — 2× XP for evolving Pokémon. Shiny Pidgey may appear.", details: { bosses: ["Pidgey ✨"], bonuses: ["2× XP for evolving Pokémon", "Shiny Pidgey may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 100, title: "Spotlight Hour: Zubat", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-07-16", endDate: "2026-07-16", startsAt: "2026-07-16T18:00:00", endsAt: "2026-07-16T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Zubat in the spotlight — 2× XP for catching Pokémon. Shiny Zubat may appear.", details: { bosses: ["Zubat ✨"], bonuses: ["2× XP for catching Pokémon", "Shiny Zubat may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 101, title: "Spotlight Hour: Eevee", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-07-23", endDate: "2026-07-23", startsAt: "2026-07-23T18:00:00", endsAt: "2026-07-23T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Eevee in the spotlight — 2× Candy for catching Pokémon. Shiny Eevee may appear.", details: { bosses: ["Eevee ✨"], bonuses: ["2× Candy for catching Pokémon", "Shiny Eevee may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  { id: 102, title: "Spotlight Hour: Bidoof", type: "Event", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", date: "2026-07-30", endDate: "2026-07-30", startsAt: "2026-07-30T18:00:00", endsAt: "2026-07-30T19:00:00", published: "2026-06-10", time: "Spotlight Hour: 6–7 PM", color: "#7E57C2", icon: "✨", iconImg: "assets/pokemon-images/icons/QuestPokemonReward.png", featured: false, summary: "Bidoof in the spotlight — 2× Candy for transferring Pokémon. Shiny Bidoof may appear.", details: { bosses: ["Bidoof ✨"], bonuses: ["2× Candy for transferring Pokémon", "Shiny Bidoof may appear if you are lucky"], tips: ["Spotlight Hour runs Thursday, 6–7 PM local time.", "Time-box your session to the 6–7 PM hour for the bonus."], relatedNews: [{ id: 24, label: "Choose Your Path & Daily Discoveries Update", icon: "🧭" }] } },
  ...
  { id: 66, title: "Skarmory Super Mega Raid Day", type: "Raid", url: "https://pokemongo.com/news/skarmory-super-mega-raid-day-2026", date: "2026-06-27", endDate: null, time: "2:00 PM – 5:00 PM", color: "#95A5A6", icon: "\uD83D\uDEE1\uFE0F", published: "2026-06-04", updated: "2026-06-04", lastUpdated: "June 4, 2026", iconImg: "assets/pokemon-images/Mega/regular/Gen-2_Johto/0227_mega.webp", featured: true, summary: "Mega Skarmory debuts in Super Mega Raids on Saturday June 27! Increased Shiny Skarmory odds, up to 6 free Raid Passes, Remote Raid limit raised to 20, and a Timed Research line with a Houndoom encounter.", details: { bosses: ["★Mega Skarmory (Super Mega Raid)"], groupSize: [{ bossName: "Mega Skarmory", tier: "Super Mega Raid", minimum: 8, optimalMin: 8, optimalMax: 20, easyAt: 8, greenAt: 8 }], bonuses: [{ text: "Up to 6 free Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" }, { text: "Increased chance of encountering Shiny Skarmory from Super Mega Raids", img: "assets/pokemon-images/icons/shiny-sparkles.webp" }, { text: "Remote Raid Pass limit increased to 20 (Fri, June 26, 5:00 PM – Sat, June 27, 8:00 PM PDT)", img: "assets/pokemon-images/Items/remote-raid-pass.png" }, { text: "Super Mega Raids appear only at select Gyms — locate them via pokemongo.com/map", img: "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png" }, { text: "Free Timed Research available during event hours — rewards a Premium Battle Pass and a Houndoom encounter", img: "assets/pokemon-images/icons/green-research.png" }], timedResearch: { free: { rewards: ["Premium Battle Pass", "Houndoom encounter"], note: "Timed Research expires — tasks must be completed and rewards claimed before the Timed Research ends." } }, ticketPrice: "$4.99", ticketNote: "These bonuses will be effective on Saturday, June 27, 2026, from 2:00 p.m. to 5:00 p.m. local time.", ticketBonuses: [{ text: "Up to 14 Raid Passes from spinning Gym Photo Discs", img: "assets/pokemon-images/Items/Raid-pass(free).png" }, { text: "Increased chance to get Rare Candy XL from Raid Battles", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png" }, { text: "An additional 5,000 XP per Super Mega Raid Battle", img: "assets/pokemon-images/icons/xp.png" }, { text: "An additional 5,000 Stardust per Super Mega Raid Battle", img: "assets/pokemon-images/Items/stardust_painted.png" }, "Tickets giftable to Great Friends or higher", "Purchases are non-refundable — cannot be bought with PokéCoins", "Ticket available in the in-game shop until Saturday, June 27 at 5:00 PM local time", "Ultra Ticket Box $4.99 on Web Store (event ticket + Premium Battle Pass at no additional cost)"], tips: ["Mega Skarmory makes its Pokémon GO debut — top priority for collectors.", "Mega Skarmory is Steel/Flying — 4× weak to Fire, also weak to Electric.", "Super Mega Raids require at least 8 Trainers because of the Shield phases — coordinate a full lobby (lobby cap 20).", "Plan a raid route with your group — the 3-hour window goes fast.", "Shiny Skarmory has boosted odds from Super Mega Raids — check every encounter.", "Free Timed Research includes a Houndoom encounter — complete and claim before it expires.", "Ultra Ticket Box is Web Store exclusive — grab it before the event for a free Premium Battle Pass.", "Ticket only available in-game shop until 5:00 PM local on June 27 — buy early.", "Find nearby Super Mega Raids and Community Ambassador meetups via pokemongo.com/map."], counters: { label: "Mega Skarmory (Steel/Flying)", pokemon: [{ name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Shadow Thundurus (Therian Forme)", fast: "Volt Switch", charged: "Wildbolt Storm", chargedNote: "Signature" }, { name: "Shadow Heatran", fast: "Fire Spin", charged: "Magma Storm", chargedNote: "Signature" }, { name: "Shadow Raikou", fast: "Thunder Shock", charged: "Wild Charge" }, { name: "Regieleki", fast: "Lock-On", charged: "Thunder Cage", chargedNote: "Signature" }, { name: "Shadow Magnezone", fast: "Volt Switch", charged: "Wild Charge" }] } } },
  { id: 67, title: "Community Day: Sobble", type: "Community Day", url: "https://pokemongo.com/news/communityday-july-2026-sobble", date: "2026-07-04", endDate: null, time: "2:00 PM – 5:00 PM", whenText: "Saturday, July 4, 2026, from 2:00 p.m. to 5:00 p.m. local time", color: "#3498DB", icon: "💧", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0816.webp", featured: true, published: "2026-06-16", updated: "2026-06-16", lastUpdated: "June 16, 2026 at 2:11 PM", summary: "Sobble takes the spotlight on July 4! Evolve Drizzile to Inteleon for the exclusive Charged Attack Hydro Cannon, with 1/4 Egg Hatch Distance, 2× Catch Candy, and 3-hour Incense.", details: { bosses: ["Inteleon with Hydro Cannon (exclusive Charged Move)"], bonuses: [{ text: "1/4 Egg Hatch Distance for Eggs placed in an Incubator (2–5 PM)", img: "assets/pokemon-images/Items/EggIncubatorIAP_Activated.png" }, { text: "2× Candy for catching Pokémon (2–5 PM)", img: "assets/pokemon-images/Items/pokemon_details_candy.png", imgSize: 22 }, { text: "2× Candy XL chance for level 31+ (2–5 PM)", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png", imgSize: 22 }, { text: "3-hour Incense — excludes Daily Adventure Incense (2–5 PM)", img: "assets/pokemon-images/Items/Incense_1.png" }, { text: "Snapshot surprise available (2–5 PM)", img: "assets/pokemon-images/Items/tx_ar_photo_camera.png" }, { text: "1-hour Lure Modules (2–9 PM)", img: "assets/pokemon-images/Items/lure-module.png" }, { text: "Sobble has a very high chance at PokéStops with active regular Lure Modules (2–9 PM)", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0816.webp", imgSize: 56, imgNudgeY: 10 }, { text: "1 extra Special Trade, up to 2 daily (2–9 PM)", img: "assets/pokemon-images/icons/pogo_trade_icon.png", imgSize: 22 }, { text: "50% less Trade Stardust cost (2–9 PM)", img: "assets/pokemon-images/Items/stardust_painted.png" }, { text: "Possible Shiny Sobble encounters", img: "assets/pokemon-images/icons/shiny-sparkles.webp" }, { text: "Sobble with a Special Background from Field Research and Lure Modules (new yearly Special Background since January 2026)", img: "assets/pokemon-images/Items/tx_ar_photo_camera.png" }], paidResearch: { price: "$1.99", rewards: [{ text: "3 Sobble encounters with a Special Background", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0816.webp", imgSize: 56, imgNudgeY: 10 }, { text: "Additional Sobble encounters", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0816.webp", imgSize: 56, imgNudgeY: 10 }, { text: "1 Premium Battle Pass", img: "assets/pokemon-images/Items/premium-raid-pass.png" }, { text: "1 Rare Candy XL", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png", imgSize: 22 }, { text: "And even more goodies", glyph: "+" }], note: "Tickets are giftable to friends at Great Friends level or higher." }, tips: ["Evolve Drizzile during the event or up to 4 hours after (by 9 PM) for an Inteleon that knows Hydro Cannon.", "After the event, Inteleon can learn the Charged Attack Snipe Shot — 65 power in Trainer Battles (chance to raise its Attack by 2 stages), 100 power in Gyms & Raids.", "July Community Day–themed Field Research: catch Sobble for Stardust, Ultra Balls, and additional Sobble encounters.", "Check in to a Community Ambassador event for Timed Research that rewards extra Special Background Sobble encounters plus premium item rewards.", "Take a few snapshots during Community Day for a surprise.", "Sobble caught during the event can be Shiny — check every encounter."] } },
  /* HIDDEN 2026-07-14: "Mystery Event: July 2026" (id 68, 2026-07-18) removed from display per request — reversible, uncomment to restore.
  ...
  */
  ...
  { id: 70, title: "Community Day: Nickit", type: "Community Day", url: "https://pokemongo.com/en/news/communityday-august-2026-nickit", date: "2026-08-16", endDate: null, time: "2:00 PM – 5:00 PM", whenText: "Sunday, August 16, 2026, from 2:00 p.m. to 5:00 p.m. local time", color: "#E67E22", icon: "🦊", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0827.webp", featured: true, published: "2026-07-14", updated: "2026-07-14", lastUpdated: "July 14, 2026 at 10:53 AM", summary: "Nickit takes the spotlight on August 16! Evolve Nickit to Thievul for the exclusive Charged Attack Icy Wind, with 3× Catch Stardust, 2× Catch Candy, and 3-hour Incense.", details: { bosses: ["Thievul with Icy Wind (exclusive Charged Move)"], bonuses: [{ text: "3× Stardust for catching Pokémon (2–5 PM)", img: "assets/pokemon-images/Items/stardust_painted.png" }, { text: "2× Candy for catching Pokémon (2–5 PM)", img: "assets/pokemon-images/Items/pokemon_details_candy.png", imgSize: 22 }, { text: "2× Candy XL chance for level 31+ (2–5 PM)", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png", imgSize: 22 }, { text: "3-hour Incense — excludes Daily Adventure Incense (2–5 PM)", img: "assets/pokemon-images/Items/Incense_1.png" }, { text: "Snapshot surprise available (2–5 PM)", img: "assets/pokemon-images/Items/tx_ar_photo_camera.png" }, { text: "1-hour Lure Modules (2–9 PM)", img: "assets/pokemon-images/Items/lure-module.png" }, { text: "Nickit has a very high chance at PokéStops with active regular Lure Modules; Lure-attracted Nickit keep boosted Shiny odds (2–9 PM)", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0827.webp", imgSize: 56, imgNudgeY: 10 }, { text: "1 extra Special Trade, up to 2 daily (2–9 PM)", img: "assets/pokemon-images/icons/pogo_trade_icon.png", imgSize: 22 }, { text: "50% less Trade Stardust cost (2–9 PM)", img: "assets/pokemon-images/Items/stardust_painted.png" }, { text: "Possible Shiny Nickit encounters", img: "assets/pokemon-images/icons/shiny-sparkles.webp" }, { text: "Nickit with a Special Background from Field Research and Lure Modules (yearly Special Background since January 2026)", img: "assets/pokemon-images/Items/tx_ar_photo_camera.png" }], paidResearch: { price: "$1.99", rewards: [{ text: "3 Nickit encounters with a Special Background", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0827.webp", imgSize: 56, imgNudgeY: 10 }, { text: "Additional Nickit encounters", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0827.webp", imgSize: 56, imgNudgeY: 10 }, { text: "1 Premium Battle Pass", img: "assets/pokemon-images/Items/premium-raid-pass.png" }, { text: "1 Rare Candy XL", img: "assets/pokemon-images/Items/RareXLCandy_PSD.png", imgSize: 22 }, { text: "And even more goodies", glyph: "+" }], note: "Tickets are giftable to friends at Great Friends level or higher. This Special Research has no in-game medal and no story." }, tips: ["Evolve Nickit during the event or up to 4 hours after (by 9 PM) for a Thievul that knows Icy Wind.", "August Community Day–themed Field Research: catch Nickit for Stardust, Ultra Balls, and additional Nickit encounters (some with a Special Background).", "Check in to a Community Ambassador event for Timed Research that rewards extra Special Background Nickit encounters plus premium item rewards.", "Nickit attracted to regular Lure Modules have boosted Shiny rates.", "Take a few snapshots during Community Day for a surprise.", "Nickit caught during the event can be Shiny — check every encounter.", "Community Day Ultra Box ($1.99) includes the event ticket + 5 Ultra Balls."] } },
  ...
  { id: 81, title: "5★ Raid: Reshiram", type: "Raid", date: "2026-06-03", endDate: "2026-06-09", published: "2026-05-27", updated: "2026-05-27", time: "Raid Hour: Wed Jun 3, 6–7 PM", color: "#E74C3C", icon: "🔥", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0643.webp", featured: false, summary: "Reshiram opens June's 5-Star Raid rotation. Dragon/Fire-type Vast White Pokémon. Mega Audino in Mega Raids. Shadow Dialga in Shadow Raids all month.", details: { bosses: ["Reshiram ✨ (5★ Raid)", "Mega Audino ✨ (Mega)", "Shadow Dialga ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Reshiram", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Audino", tier: "Mega Raid", minimum: 3, optimalMin: 4, optimalMax: 7, easyAt: 8, greenAt: 8 }, { bossName: "Shadow Dialga", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday June 3, 6–7 PM local", "Mega Audino in Mega Raids", "Shadow Dialga in 5★ Shadow Raids throughout June"], tips: ["Reshiram is weak to Rock, Dragon, and Ground.", "Reshiram top counters: Eternatus, Mega Rayquaza, Mega Garchomp, Primal Groudon, Shadow Garchomp, Shadow Palkia.", "Shiny Reshiram is available — check every encounter.", "Mega Audino is Normal/Fairy — a strong defensive Mega; weak to Steel and Poison.", "Mega Audino top counters: Crowned Sword Zacian, Dusk Mane Necrozma, Crowned Shield Zamazenta, Shadow Metagross, Mega Lucario, Mega Metagross.", "Shadow Dialga (Steel/Dragon) runs all month — bring Purified Gems."], counters: [{ label: "Reshiram (Dragon/Fire)", pokemon: [{ name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Shadow Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Shadow Palkia", fast: "Dragon Tail", charged: "Draco Meteor" }] }, { label: "Shadow Dialga (Steel/Dragon)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }] }, { label: "Mega Audino (Normal/Fairy)", pokemon: [{ name: "Crowned Sword Zacian", fast: "Metal Claw", charged: "Behemoth Blade" }, { name: "Dusk Mane Necrozma", fast: "Metal Claw", charged: "Sunsteel Strike" }, { name: "Crowned Shield Zamazenta", fast: "Metal Claw", charged: "Behemoth Bash" }, { name: "Shadow Metagross", fast: "Bullet Punch", charged: "Meteor Mash", chargedNote: "CD Exclusive" }, { name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Meteor Mash" }, { name: "Mega Metagross", fast: "Bullet Punch", charged: "Meteor Mash", chargedNote: "CD Exclusive" }] }] } },
  { id: 82, title: "5★ Raid: Zekrom", type: "Raid", date: "2026-06-10", endDate: "2026-06-16", published: "2026-05-27", updated: "2026-05-27", time: "Raid Hour: Wed Jun 10, 6–7 PM", color: "#34495E", icon: "⚡", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0644.webp", featured: false, summary: "Zekrom takes over 5-Star Raids — Dragon/Electric Deep Black Pokémon. Mega Lopunny in Mega Raids. Shadow Dialga continues all month.", details: { bosses: ["Zekrom ✨ (5★ Raid)", "Mega Lopunny ✨ (Mega)", "Shadow Dialga ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Zekrom", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Lopunny", tier: "Mega Raid", minimum: 2, optimalMin: 4, optimalMax: 6, easyAt: 7, greenAt: 7 }, { bossName: "Shadow Dialga", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday June 10, 6–7 PM local", "Mega Lopunny in Mega Raids", "Shadow Dialga continues in 5★ Shadow Raids"], tips: ["Zekrom is weak to Dragon, Ground, Fairy, and Ice.", "Zekrom top counters: Eternatus, Mega Rayquaza, Mega Garchomp, Black Kyurem, Primal Groudon, Shadow Garchomp.", "Shiny Zekrom is available.", "Mega Lopunny is Normal/Fighting — a top Fighting attacker for raids; weak to Flying, Psychic, Fairy.", "Mega Lopunny top counters: Mega Mewtwo Y, Mega Rayquaza, Mega Mewtwo X, Shadow Mewtwo, Mega Blaziken, Mega Lucario."], counters: [{ label: "Zekrom (Electric/Dragon)", pokemon: [{ name: "Eternatus", fast: "Dragon Tail", charged: "Dynamax Cannon", chargedNote: "Signature" }, { name: "Mega Rayquaza", fast: "Dragon Tail", charged: "Breaking Swipe", chargedNote: "Legacy" }, { name: "Mega Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }, { name: "Black Kyurem", fast: "Dragon Tail", charged: "Freeze Shock" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Shadow Garchomp", fast: "Dragon Tail", charged: "Breaking Swipe" }] }, { label: "Mega Lopunny (Normal/Fighting)", pokemon: [{ name: "Mega Mewtwo Y", fast: "Confusion", charged: "Psystrike", chargedNote: "Signature" }, { name: "Mega Rayquaza", fast: "Air Slash", charged: "Dragon Ascent", chargedNote: "Signature" }, { name: "Mega Mewtwo X", fast: "Confusion", charged: "Psystrike", chargedNote: "Signature" }, { name: "Shadow Mewtwo", fast: "Psycho Cut", charged: "Psystrike", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }] }, { label: "Shadow Dialga (Steel/Dragon)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }] }] } },
  { id: 83, title: "5★ Raid: Necrozma", type: "Raid", date: "2026-06-17", endDate: "2026-06-23", published: "2026-05-27", updated: "2026-05-27", time: "Raid Hour: Wed Jun 17, 6–7 PM", color: "#2C3E50", icon: "🌌", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-7_Alola/0800.webp", featured: true, summary: "Necrozma debuts in 5-Star Raids — the Prism Pokémon. Catch enough now to fuse with Solgaleo (Dusk Mane) or Lunala (Dawn Wings). Mega Scizor in Mega Raids.", details: { bosses: ["Necrozma ✨ (5★ Raid)", "Mega Scizor ✨ (Mega)", "Shadow Dialga ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Necrozma", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Scizor", tier: "Mega Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Shadow Dialga", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday June 17, 6–7 PM local", "Mega Scizor in Mega Raids", "Shadow Dialga continues in 5★ Shadow Raids"], tips: ["Necrozma is Psychic-type — weak to Bug, Ghost, and Dark.", "Necrozma top counters: Dawn Wings Necrozma, Mega Tyranitar, Mega Mewtwo Y, Shadow Darkrai, Mega Absol, Mega Gengar.", "Shiny Necrozma is available — top priority catch this month.", "Bank Necrozma candy to fuse with Solgaleo (Dusk Mane) or Lunala (Dawn Wings) later.", "Mega Scizor (Bug/Steel) is 4× weak to Fire — load up Fire attackers.", "Mega Scizor top counters: Mega Blaziken, Blacephalon, Mega Charizard Y, Shadow Heatran, Shadow Blaziken, Standard Mode Shadow Darmanitan."], counters: [{ label: "Necrozma (Psychic)", pokemon: [{ name: "Dawn Wings Necrozma", fast: "Shadow Claw", charged: "Moongeist Beam", chargedNote: "Signature" }, { name: "Mega Tyranitar", fast: "Bite", charged: "Brutal Swing" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Shadow Ball", chargedNote: "Legacy" }, { name: "Shadow Darkrai", fast: "Snarl", charged: "Shadow Ball" }, { name: "Mega Absol", fast: "Snarl", charged: "Brutal Swing", chargedNote: "Legacy" }, { name: "Mega Gengar", fast: "Lick", fastNote: "Legacy", charged: "Shadow Ball" }] }, { label: "Mega Scizor (Bug/Steel)", pokemon: [{ name: "Mega Blaziken", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Blacephalon", fast: "Incinerate", charged: "Mind Blown", chargedNote: "Signature" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Shadow Heatran", fast: "Fire Spin", charged: "Magma Storm", chargedNote: "Signature" }, { name: "Shadow Blaziken", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Standard Mode Shadow Darmanitan", fast: "Fire Fang", charged: "Overheat" }] }, { label: "Shadow Dialga (Steel/Dragon)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }] }] } },
  { id: 84, title: "5★ Raid: Celesteela & Kartana", type: "Raid", date: "2026-06-24", endDate: "2026-06-30", published: "2026-05-27", updated: "2026-06-04", lastUpdated: "June 4, 2026", time: "Raid Hour: Wed Jun 24, 6–7 PM", color: "#8E44AD", icon: "🌐", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-7_Alola/0798.webp", featured: true, summary: "Hemisphere split! Celesteela (Steel/Flying) headlines 5-Star Raids in the Southern Hemisphere while Kartana (Grass/Steel) takes the Northern Hemisphere. Mega Pidgeot in Mega Raids. Skarmory Super Mega Raid Day on Saturday June 27 — Mega Skarmory debuts in Super Mega Raids.", details: { bosses: ["Celesteela (Southern Hemisphere) ✨ (5★ Raid)", "Kartana (Northern Hemisphere) ✨ (5★ Raid)", "Mega Pidgeot ✨ (Mega)", "★Mega Skarmory (Super Mega Raid — June 27, 2–5 PM)", "Shadow Dialga ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Celesteela", tier: "5-Star Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Kartana", tier: "5-Star Raid", minimum: 2, optimalMin: 2, optimalMax: 4, easyAt: 5, greenAt: 5 }, { bossName: "Mega Pidgeot", tier: "Mega Raid", minimum: 2, optimalMin: 3, optimalMax: 5, easyAt: 6, greenAt: 6 }, { bossName: "Mega Skarmory", tier: "Super Mega Raid", minimum: 8, optimalMin: 8, optimalMax: 20, easyAt: 8, greenAt: 8 }, { bossName: "Shadow Dialga", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Raid Hour: Wednesday June 24, 6–7 PM local", "Celesteela appears in 5-Star Raids in the Southern Hemisphere", "Kartana appears in 5-Star Raids in the Northern Hemisphere", "Mega Pidgeot in Mega Raids", "Skarmory Super Mega Raid Day: Saturday June 27, 2:00–5:00 PM local — Mega Skarmory ✨ debut in Super Mega Raids", "Remote Raid Pass limit increased to 20 (Fri, June 26, 5:00 PM – Sat, June 27, 8:00 PM PDT)", "Shadow Dialga continues in 5★ Shadow Raids"], tips: ["Hemisphere-locked — coordinate remote raids with friends in the opposite hemisphere to grab both.", "Celesteela (Steel/Flying) is weak to Fire and Electric.", "Celesteela top counters: Shadow Thundurus (Therian Forme), Mega Blaziken, Mega Charizard Y, Blacephalon, Shadow Heatran, Regieleki.", "Kartana (Grass/Steel) is 4× weak to Fire.", "Kartana top counters: Mega Blaziken, Mega Charizard Y, Blacephalon, Shadow Heatran, Shadow Moltres, Mega Charizard X.", "Both Celesteela and Kartana have Shiny releases available.", "Mega Pidgeot is Normal/Flying — strong Flying attacker; weak to Electric, Ice, Rock.", "Mega Pidgeot top counters: Shadow Thundurus (Therian Forme), White Kyurem, Regieleki, Mega Diancie, Mega Mewtwo Y, Shadow Rhyperior.", "Skarmory Super Mega Raid Day on June 27 — Mega Skarmory (Steel/Flying) is 4× weak to Fire. Top counters: Mega Charizard Y, Shadow Thundurus (Therian Forme), Shadow Heatran, Shadow Raikou, Regieleki, Shadow Magnezone.", "Super Mega Raids need at least 8 Trainers — coordinate a full lobby (cap 20).", "Free Timed Research during Skarmory Super Mega Raid Day rewards a Premium Battle Pass and a Houndoom encounter."], counters: [{ label: "Celesteela (Steel/Flying)", pokemon: [{ name: "Shadow Thundurus (Therian Forme)", fast: "Volt Switch", charged: "Wildbolt Storm", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Blacephalon", fast: "Incinerate", charged: "Mind Blown", chargedNote: "Signature" }, { name: "Shadow Heatran", fast: "Fire Spin", charged: "Magma Storm", chargedNote: "Signature" }, { name: "Regieleki", fast: "Lock-On", charged: "Thunder Cage", chargedNote: "Signature" }] }, { label: "Kartana (Grass/Steel)", pokemon: [{ name: "Mega Blaziken", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Blacephalon", fast: "Incinerate", charged: "Mind Blown", chargedNote: "Signature" }, { name: "Shadow Heatran", fast: "Fire Spin", charged: "Magma Storm", chargedNote: "Signature" }, { name: "Shadow Moltres", fast: "Fire Spin", charged: "Overheat" }, { name: "Mega Charizard X", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }] }, { label: "Mega Pidgeot (Normal/Flying)", pokemon: [{ name: "Shadow Thundurus (Therian Forme)", fast: "Volt Switch", charged: "Wildbolt Storm", chargedNote: "Signature" }, { name: "White Kyurem", fast: "Ice Fang", charged: "Ice Burn" }, { name: "Regieleki", fast: "Lock-On", charged: "Thunder Cage", chargedNote: "Signature" }, { name: "Mega Diancie", fast: "Rock Throw", charged: "Rock Slide" }, { name: "Mega Mewtwo Y", fast: "Psycho Cut", charged: "Thunderbolt" }, { name: "Shadow Rhyperior", fast: "Smack Down", charged: "Rock Wrecker", chargedNote: "CD Exclusive" }] }, { label: "Mega Skarmory (Steel/Flying — Super Mega Raid)", pokemon: [{ name: "Mega Charizard Y", fast: "Fire Spin", charged: "Blast Burn", chargedNote: "CD Move" }, { name: "Shadow Thundurus (Therian Forme)", fast: "Volt Switch", charged: "Wildbolt Storm", chargedNote: "Signature" }, { name: "Shadow Heatran", fast: "Fire Spin", charged: "Magma Storm", chargedNote: "Signature" }, { name: "Shadow Raikou", fast: "Thunder Shock", charged: "Wild Charge" }, { name: "Regieleki", fast: "Lock-On", charged: "Thunder Cage", chargedNote: "Signature" }, { name: "Shadow Magnezone", fast: "Volt Switch", charged: "Wild Charge" }] }, { label: "Shadow Dialga (Steel/Dragon)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }] }] } },
  ...
  { id: 91, title: "Flying Taxi", type: "Event", url: "https://pokemongo.com/news/flying-taxi-2026", date: "2026-06-23", endDate: "2026-06-29", published: "2026-05-27", updated: "2026-06-09", lastUpdated: "June 9, 2026", time: "10:00 AM – 8:00 PM", color: "#1F8AC0", icon: "🚕", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-9_Paldea/0931.webp", featured: true, summary: "Take flight as Squawkabilly's debut leads the Flying Taxi Charge! The Parrot Pokémon arrives with regional plumage forms — Yellow & White Plumage worldwide, Green in the Eastern Hemisphere, Blue in the Western Hemisphere. GO Pass: Flying Taxi runs June 23–29 with Pidgeotto, Hoothoot, Vullaby, and Rookidee encounters, a Super Rocket Radar, plus Mysterious Components and Shadow Shards. Unlimited GO Points June 27–29.", details: { alert: "GO Pass: Flying Taxi rewards expire Wednesday, July 1 at 7:59 PM local — claim before then!", bosses: ["Squawkabilly ✨ (Debut — Field Research and Wild)", "Yellow Plumage Squawkabilly ✨ (Global)", "White Plumage Squawkabilly ✨ (Global — Wild)", "Green Plumage Squawkabilly ✨ (Eastern Hemisphere)", "Blue Plumage Squawkabilly ✨ (Western Hemisphere)", "Hoothoot ✨ (Wild)", "Rookidee ✨ (Wild)", "Spearow ✨ (Field Research)", "Wattrel (Field Research)", "Pidgeotto ✨ (GO Pass encounter)", "Vullaby ✨ (GO Pass encounter)", "Swablu ✨ (1★ Raid)", "Starly ✨ (1★ Raid)", "Vullaby ✨ (1★ Raid)", "Hisuian Braviary ✨ (3★ Raid)", "Corvisquire ✨ (3★ Raid)", "Bombirdier ✨ (3★ Raid)"], wildSpawns: ["Hoothoot ✨", "Rookidee ✨", "White Plumage Squawkabilly", "…and more!"], fieldResearch: ["Spearow ✨", "Hoothoot ✨", "Rookidee ✨", "Squawkabilly ✨ (form based on your location)", "Wattrel"], raids: { oneStar: ["Swablu ✨", "Starly ✨", "Vullaby ✨"], threeStar: ["Hisuian Braviary ✨", "Corvisquire ✨", "Bombirdier ✨"] }, bonuses: ["Event window: Tuesday, June 23 at 10:00 AM – Monday, June 29 at 8:00 PM local time", "GO Pass: Flying Taxi auto-delivered Tuesday, June 23 at 10:00 AM local time", "Unlimited GO Points: Saturday, June 27 at 12:00 AM – Monday, June 29 at 7:59 PM local time (no daily cap)", "Free GO Pass rewards include a Super Rocket Radar, Mysterious Components, and Shadow Shards", "GO Pass Deluxe adds extra Flying-type encounters and an additional Super Rocket Radar", "Watch for additional bonus tasks for your GO Pass: Flying Taxi during the event", "Rewards expire Wednesday, July 1 at 7:59 PM local time"], milestones: [{ tier: "Tier 1", bonus: "2× Catch Candy" }, { tier: "Tier 2", bonus: "Earn more Stardust from defeating Team GO Rocket Grunts and Leaders" }], goPass: { free: ["Encounters with Pidgeotto ✨, Hoothoot ✨, Vullaby ✨, Rookidee ✨, and more!", "One Super Rocket Radar", "Mysterious Components and Shadow Shards", "Tier 1: 2× Catch Candy (Major Milestone)", "Tier 2: Increased Stardust from Team GO Rocket Grunts & Leaders (Major Milestone)"], deluxe: { price: "$4.99", rewards: ["Everything in the free GO Pass", "Additional encounters with even more Flying-type Pokémon", "An additional Super Rocket Radar", "Upgraded milestone rewards", "Faster progression through ranks", "And even more goodies!"] }, deluxePlus: { price: "$6.99", rewards: ["Everything in GO Pass Deluxe", "Instantly earn enough GO Points to reach Rank 7 (skip 6 ranks)"] }, webStore: { giftWithPurchase: ["10 Ultra Balls", "5 Max Revives", "1 Premium Battle Pass", "5 Max Potions"], ultraBox: { name: "GO Pass Deluxe: Flying Taxi + 6 Ranks Ultra Box (Web Store Exclusive)", contents: ["20 Ultra Balls", "10 Max Revives", "10 Max Potions", "2 Premium Battle Passes", "1 Incubator", "1 Super Incubator"] } } }, tips: ["Squawkabilly is a brand-new debut — the Parrot Pokémon, Normal/Flying. Catch from Field Research OR in the wild.", "Yellow and White Plumage appear globally; Green is Eastern Hemisphere only and Blue is Western Hemisphere only — trade with friends overseas to complete the set.", "White Plumage Squawkabilly is the only plumage confirmed as a wild spawn — Field Research Squawkabilly matches your location's regional form.", "All event spawns, research encounters, and raid bosses can be Shiny — except Wattrel (no shiny mark on the article list).", "Hisuian Braviary debut in 3★ Raids — Psychic/Flying, weak to Rock/Electric/Ice/Dark/Ghost. Good Mega Manectric / Electric raid week.", "Use your free GO Pass Super Rocket Radar to face the current Giovanni line-up before it rotates.", "Hit the Unlimited GO Points weekend (June 27 12 AM – June 29 7:59 PM) hard — no daily cap means stack as many Pass Tasks as you can.", "Web Store Deluxe purchase activates as soon as the GO Pass begins (June 23, 10 AM) and includes a gift bundle on top.", "Rewards expire Wednesday, July 1 at 7:59 PM local — claim everything before then.", "Skarmory Super Mega Raid Day overlaps Saturday June 27 (2–5 PM) — pair Flying Taxi field activity with raid grinding.", "Pairs with the Mega Pidgeot rotation that same week (June 24–30)."], relatedNews: [{ id: 21, label: "June Content Update — Forever Forward", icon: "📅" }, { id: 92, label: "Flying Taxi: Taken Over (June 25–29)", icon: "🚀", iconImg: "assets/pokemon-images/icons/RocketTakeover.png" }] } },
  { id: 92, title: "Flying Taxi: Taken Over", type: "Event", url: "https://pokemongo.com/news/flying-taxi-taken-over-2026", date: "2026-06-25", endDate: "2026-06-29", published: "2026-05-27", updated: "2026-06-09", lastUpdated: "June 9, 2026", time: "12:00 AM – 8:00 PM", color: "#7B68EE", icon: "🚀", iconImg: "assets/pokemon-images/icons/RocketTakeover.png", featured: true, summary: "Rescue Shadow Reshiram! Team GO Rocket takes over the Flying Taxi event — progress through GO Pass: Flying Taxi to earn a Super Rocket Radar and chase Giovanni (now only found at PokéStops). Grunt rosters add Shadow Seel, Shadow Hoothoot, Shadow Noibat, and Shadow Rookidee (all can be Shiny). Sierra, Cliff, and Arlo rotate to new Shadow lineups. Field Research rewards Fast TMs, Charged TMs, and Mysterious Components. Charged TMs can remove Frustration during the event, and 1★/3★ Shadow Raid catches have wider IV variance.", details: { alert: "Claim all GO Pass: Flying Taxi rewards by Wednesday, July 1 at 7:59 PM local — anything unclaimed disappears!", bosses: ["Shadow Reshiram ✨ (Giovanni via Super Rocket Radar)", "Shadow Seel ✨ (Grunt encounter)", "Shadow Hoothoot ✨ (Grunt encounter)", "Shadow Noibat ✨ (Grunt encounter)", "Shadow Rookidee ✨ (Grunt encounter)", "Pidgeotto ✨ (GO Pass encounter)", "Hoothoot ✨ (GO Pass encounter)", "Vullaby ✨ (GO Pass encounter)", "Rookidee ✨ (GO Pass encounter)"], rocketLeaders: { title: "Team GO Rocket Leaders", subtitle: "Sierra, Cliff, and Arlo rotate to new Shadow lineups during the event. Defeat them to earn Mysterious Components toward your next Super Rocket Radar.", leaders: [{ name: "Sierra", img: "assets/pokemon-images/icons/leader-sierra.png.webp" }, { name: "Cliff", img: "assets/pokemon-images/icons/leader-cliff.png.webp" }, { name: "Arlo", img: "assets/pokemon-images/icons/leader-arlo.png.webp" }] }, shadowPokemon: ["Shadow Seel ✨", "Shadow Hoothoot ✨", "Shadow Noibat ✨", "Shadow Rookidee ✨", "Shadow Reshiram ✨ (Giovanni reward — Super Rocket Radar required)"], fieldResearch: ["Event-themed Field Research tasks award Fast TMs, Charged TMs, and Mysterious Components"], bonuses: ["Event window: Thursday, June 25 at 12:00 AM – Monday, June 29 at 8:00 PM local time", "Team GO Rocket appears more frequently at PokéStops and in balloons", "Charged TM can be used to help a Shadow Pokémon forget the Charged Attack Frustration", "Pokémon caught from 1★ and 3★ Shadow Raids during the event have a wider variance of Attack, Defense, and HP", "Progress GO Pass: Flying Taxi to receive a Super Rocket Radar and chase Giovanni (only found at PokéStops during this event)", "Unlimited GO Points: Saturday, June 27 at 12:00 AM – Monday, June 29 at 7:59 PM local time (no daily cap)", "Watch for additional bonus tasks for your GO Pass: Flying Taxi during the event", "GO Pass rewards expire Wednesday, July 1 at 7:59 PM local time", "Shadow Pokémon and Purified Pokémon cannot fuse with other Pokémon"], milestones: [{ tier: "Tier 1 — Rank 10", bonus: "2× Catch Candy" }, { tier: "Tier 2 — Rank 20", bonus: "Earn more Stardust from defeating Team GO Rocket Grunts and Leaders" }], goPass: { free: ["Encounters with Pidgeotto ✨, Hoothoot ✨, Vullaby ✨, Rookidee ✨, and more!", "One Super Rocket Radar", "Mysterious Components and Shadow Shards", "Tier 1 (Rank 10): 2× Catch Candy", "Tier 2 (Rank 20): Increased Stardust from Team GO Rocket Grunts & Leaders"], deluxe: { price: "$4.99", rewards: ["Everything in the free GO Pass", "Additional encounters with even more Flying-type Pokémon", "An additional Super Rocket Radar", "Upgraded milestone rewards", "Faster progression through ranks", "And even more goodies!"] }, deluxePlus: { price: "$6.99", rewards: ["Everything in GO Pass Deluxe", "Instantly earn enough GO Points to reach Rank 7 (skip 6 ranks)"] }, webStore: { giftWithPurchase: ["10 Ultra Balls", "5 Max Revives", "1 Premium Battle Pass", "5 Max Potions"], ultraBox: { name: "GO Pass Deluxe: Flying Taxi + 6 Ranks Ultra Box (Web Store Exclusive)", contents: ["20 Ultra Balls", "10 Max Revives", "10 Max Potions", "2 Premium Battle Passes", "1 Incubator", "1 Super Incubator"] } } }, tips: ["Headline encounter: Shadow Reshiram! Rank up GO Pass: Flying Taxi to claim the free Super Rocket Radar, then hunt Giovanni at PokéStops (he is NOT in balloons during this event).", "Charged TMs can remove Frustration during the event — bank a stack and use them on your top Shadow Pokémon now.", "Shadow Seel, Shadow Hoothoot, Shadow Noibat, and Shadow Rookidee all have shiny chances from Grunts.", "1★ and 3★ Shadow Raids during the event have a wider Attack/Defense/HP IV variance — chase high rolls (and dump low ones).", "Sierra, Cliff, and Arlo all use new Shadow rosters this event — defeating each rewards Mysterious Components toward the next Super Rocket Radar.", "GO Pass Deluxe ($4.99) adds a SECOND Super Rocket Radar plus extra Flying-type encounters and upgraded milestones.", "Deluxe + 6 Ranks ($6.99) jumps you straight to Rank 7 — fastest way to the milestone bonuses.", "Hit the Unlimited GO Points weekend hard (June 27 12 AM – June 29 7:59 PM local) — there's no daily cap, so stack every Pass Task you can.", "Claim ALL unclaimed rewards before Wednesday, July 1 at 7:59 PM local — they vanish after that.", "Web Store Deluxe purchase gifts 10 Ultra Balls, 5 Max Revives, 1 Premium Battle Pass, and 5 Max Potions on top of the in-game rewards.", "Web-exclusive Ultra Box (Deluxe + 6 Ranks) packs 20 Ultra Balls, 10 Max Revives, 10 Max Potions, 2 Premium Battle Passes, 1 Incubator, and 1 Super Incubator.", "Shadow & Purified Pokémon cannot fuse with other Pokémon — keep that in mind before purifying your Shadow Reshiram."], relatedNews: [{ id: 91, label: "Flying Taxi — Full Details (June 23–29)", icon: "🚕", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-9_Paldea/0931.webp" }, { id: 21, label: "June Content Update — Forever Forward", icon: "📅" }] } },
  { id: 90, title: "Shadow Dialga in Shadow Raids", type: "Raid", date: "2026-06-02", endDate: "2026-06-30", published: "2026-05-27", updated: "2026-05-27", time: "All month", color: "#7B2FBE", icon: "🌑", iconImg: "assets/pokemon-images/National-Dex/regular/Gen-4_Sinnoh/0483.webp", shadowBg: true, featured: true, summary: "Shadow Dialga appears in 5★ Shadow Raids all of June. Bring Purified Gems and Steel-busting counters.", details: { bosses: ["Shadow Dialga ✨ (5★ Shadow Raid)"], groupSize: [{ bossName: "Shadow Dialga", tier: "5★ Shadow Raid", minimum: 4, optimalMin: 5, optimalMax: 9, easyAt: 10, greenAt: 10 }], bonuses: ["Available in 5★ Shadow Raids from June 2 to June 30", "Purified Gems weaken Shadow Dialga during the raid"], tips: ["Shadow Dialga is Steel/Dragon — weak to Fighting and Ground.", "Top counters: Mega Lucario, Primal Groudon, Mega Blaziken, Keldeo (Resolute Forme), Shadow Excadrill, Mega Garchomp.", "Bring Purified Gems early in the raid to reduce damage taken.", "Shiny Shadow Dialga is available — top priority catch.", "Defeats Mewtwo-tier raids when paired with a Dragon-typing Dawn Wings Necrozma in the future."], counters: { label: "Shadow Dialga (Steel/Dragon)", pokemon: [{ name: "Mega Lucario", fast: "Force Palm", fastNote: "Legacy", charged: "Aura Sphere" }, { name: "Primal Groudon", fast: "Mud Shot", charged: "Precipice Blades", chargedNote: "Signature" }, { name: "Mega Blaziken", fast: "Counter", charged: "Aura Sphere" }, { name: "Keldeo (Resolute Forme)", fast: "Low Kick", charged: "Secret Sword" }, { name: "Shadow Excadrill", fast: "Mud-Slap", charged: "Scorching Sands" }, { name: "Mega Garchomp", fast: "Mud Shot", charged: "Earth Power", chargedNote: "CD Exclusive" }] } } }
];

const ANNOUNCEMENTS = [
  { id: 35, date: "2026-07-15", published: "2026-07-15", updated: "2026-07-15", lastUpdated: "July 15, 2026 at 11:31 AM", title: "GO Fest 2026: Global — Sunday Raid Battles", tag: "News", url: "https://pokemongo.com/en/news/gofest2026-global-update", icon: "assets/pokemon-images/Mega/regular/Gen-1_Kanto/0150_megay.webp", body: "A special raid event replays dozens of Pokémon from the Sunday of GO Fest 2026: Global on Sunday, July 26, 10:00 a.m.–7:00 p.m. local time — making up for technical issues that blocked raids in some time zones. Three habitats rotate every three hours, Mega Mewtwo Y headlines Super Mega Raids, team-hat Pikachu fill one-star raids, and there's no Remote Raid limit during the window.", fullBody: "GO Fest 2026: Global — Sunday Raid Battles runs Sunday, July 26, 2026, from 10:00 a.m. to 7:00 p.m. local time (for Trainers in time zones UTC+14 through UTC+0), replaying dozens of Pokémon from the Sunday of GO Fest 2026: Global to make up for technical issues that prevented raid participation without consuming passes in certain time zones. There's no Remote Raid limit from Saturday, July 25, at 1:00 p.m. PDT to Sunday, July 26, at 12:00 p.m. PDT. Three habitats rotate every three hours — Earthforged Domain, Verdant Anomaly, and Twilight Battlefield — while Mega Mewtwo Y headlines Super Mega Raids and Pikachu wearing Team Instinct, Mystic, and Valor hats fill one-star raids, all day. Event bonuses include up to nine free Raid Passes per day from spinning Photo Discs at Gyms, more effective Premier Balls after raids, Deluxe Pass bonus XP and Candy from five-star and Mega Raids, and a Link Charge cap increased to 2,000.", sections: [
  { heading: "When (all times local)", items: [
    "Sunday, July 26, 2026, 10:00 a.m. to 7:00 p.m. local time (for Trainers in time zones UTC+14 through UTC+0).",
    "No Remote Raid limit from Saturday, July 25 at 1:00 p.m. PDT to Sunday, July 26 at 12:00 p.m. PDT."
  ] },
  { heading: "Why this event", items: [
    "A special raid event featuring dozens of Pokémon that appeared on the Sunday of GO Fest 2026: Global.",
    "It makes up for technical issues in certain time zones during the original event that prevented raid participation without consuming passes."
  ] },
  { heading: "Earthforged Domain — 10 a.m.–1 p.m. (Five-Star)", items: ["Regirock", "Regice", "Registeel", "Dialga (Origin Forme)", "Palkia (Origin Forme)", "Heatran", "Regigigas", "Tornadus (Incarnate Forme)", "Tornadus (Therian Forme)", "Thundurus (Incarnate Forme)", "Thundurus (Therian Forme)", "Landorus (Incarnate Forme)", "Landorus (Therian Forme)", "Regieleki", "Regidrago", "Enamorus (Incarnate Forme) (no shiny)", "Enamorus (Therian Forme) (no shiny)"] },
  { heading: "Earthforged Domain — Mega Raids", items: ["Mega Metagross", "Mega Garchomp", "Mega Audino"] },
  { heading: "Verdant Anomaly — 1–4 p.m. (Five-Star)", items: ["Attack Forme Deoxys", "Defense Forme Deoxys", "Normal Forme Deoxys", "Speed Forme Deoxys", "Genesect", "Tapu Koko", "Tapu Lele", "Tapu Bulu", "Tapu Fini", "Nihilego", "Pheromosa (Europe/Middle East/Africa/India)", "Xurkitree (Asia-Pacific)", "Celesteela (Southern Hemisphere)", "Kartana (Northern Hemisphere)", "Guzzlord", "Necrozma", "Stakataka (Eastern Hemisphere)", "Blacephalon (Western Hemisphere)"] },
  { heading: "Verdant Anomaly — Mega Raids", items: ["Mega Beedrill", "Mega Pinsir", "Mega Sceptile"] },
  { heading: "Twilight Battlefield — 4–7 p.m. (Five-Star)", items: ["Latias", "Latios", "Cresselia", "Darkrai", "Cobalion", "Terrakion", "Virizion", "Zacian (Hero of Many Battles)", "Zamazenta (Hero of Many Battles)"] },
  { heading: "Twilight Battlefield — Mega Raids", items: ["Mega Tyranitar", "Mega Gardevoir", "Mega Lucario"] },
  { heading: "One-Star Raids (all day)", icon: "✨", showImages: true, items: [{ name: "Pikachu wearing Team Instinct hat" }, { name: "Pikachu wearing Team Mystic hat" }, { name: "Pikachu wearing Team Valor hat" }] },
  { heading: "Super Mega Raids (all day)", icon: "✨", showImages: true, items: [{ name: "Mega Mewtwo Y" }] },
  { heading: "Event Bonuses", items: [
    "Up to nine free Raid Passes per day by spinning Photo Discs at Gyms.",
    "Pokémon encountered after five-star and Mega Raids may have a special background.",
    "Mewtwo encountered after Super Mega Raids may have a special background — with enhanced visual effects when it Mega Evolves.",
    "Premier Balls will be more effective at catching Pokémon after raids.",
    "Deluxe Pass holders receive 5,000 bonus XP, 3 bonus Candy, and 1 bonus Candy XL from five-star and Mega Raids.",
    "No Remote Raid limit from Saturday, July 25 at 1:00 p.m. PDT to Sunday, July 26 at 12:00 p.m. PDT.",
    "The Link Charge cap will increase to 2,000."
  ] },
  { heading: "Good to Know", items: [
    "Enamorus (Incarnate Forme) and Enamorus (Therian Forme) can NOT be Shiny; every other five-star boss can.",
  ...
  { heading: "When (all times local)", items: [
    "Saturday, August 22, 2026, 11:00 a.m. to 5:00 p.m. local time.",
    "Remote Raid Pass limit increased to 20 from Friday, August 21, at 2:00 p.m. to Saturday, August 22, 2026, at 8:00 p.m. PDT."
  ] },
  { heading: "Featured Pokémon", icon: "✨", showImages: true, items: [{ name: "Mega Starmie" }] },
  { heading: "Free Event Bonuses", items: [
    "An additional 5,000 XP from all raids.",
    "Increased chance to get Rare Candy XL from Raid Battles.",
    "Up to six free Raid Passes from spinning Gym Photo Discs.",
    "Increased chance of encountering Shiny Starmie from Super Mega Raids.",
    "Starmie caught from Super Mega Raids will have Mega Level 1 unlocked."
  ] },
  { heading: "Ticket ($4.99)", items: [
    "An additional 5,000 XP from all raids.",
    "Up to 14 Raid Passes from spinning Gym Photo Discs.",
    "An even greater chance to get Rare Candy XL from Raid Battles.",
    "Guaranteed Rare Candy XL for successfully catching Starmie from raids."
  ] },
  { heading: "Timed Research (free)", items: [
    "Complete free Timed Research during event hours.",
    "Rewards include a Premium Battle Pass and an encounter with Scizor."
  ] },
  { heading: "Good to Know", items: [
    "Super Mega Raids may only appear at certain Gyms — use the web-based raid map at pokemongo.com/map to find them.",
    "Be aware of your surroundings and follow guidelines from local health authorities."
  ] }
] },
  { id: 34, date: "2026-07-15", published: "2026-07-15", updated: "2026-07-15", lastUpdated: "July 15, 2026 at 11:31 AM", title: "Gigantamax Rillaboom Max Battle Day", tag: "News", url: "https://pokemongo.com/en/news/gigantamax-rillaboom-max-battle-day-2026", icon: "assets/pokemon-images/Gigantamax/regular/Gen-8_Galar/0812_gigamax.webp", body: "Gigantamax Rillaboom drums into six-star Max Battles on Saturday, August 1, from 2:00 p.m. to 5:00 p.m. local time — with Shiny Gigantamax Rillaboom debuting for the first time. All Power Spots host Gigantamax Battles, the Max Particle cap rises to 1,600, and Power Spots grant 8× Max Particles. A US$4.99 ticket adds event Timed Research and 2× XP from Max Battles.", fullBody: "Gigantamax Rillaboom Max Battle Day runs Saturday, August 1, 2026, from 2:00 p.m. to 5:00 p.m. local time. Gigantamax Rillaboom will appear in six-star Max Battles, and for the first time in Pokémon GO you'll be able to encounter Shiny Gigantamax Rillaboom—if you're lucky! Free event bonuses include the Max Particle collection limit increased to 1,600, all Power Spots hosting Gigantamax Battles, Power Spots refreshing more frequently, additional Power Spots being active, 8× Max Particles from Power Spots, up to three Special Trades for the day, 2× Max Particles from exploring, and 1/4 Adventuring distance to receive Max Particles. From July 31 at 5:00 p.m. to August 1 at 8:00 p.m. PDT (GMT −7), the daily Remote Raid limit increases from 10 to 20. A US$4.99 ticket (or the equivalent pricing tier in your local currency) unlocks event-exclusive Timed Research rewarding 1 Max Mushroom, 25,000 XP, and 6,400 Max Particles, plus 2× XP from Max Battles. The Gigantamax Rillaboom Max Battle Day Ultra Ticket Box (US$4.99) includes an event ticket and a bonus Premium Battle Pass. Tickets cannot be purchased with PokéCoins. Max Battles only appear at Power Spots that sporadically appear in different locations — use the Map to find them and coordinate with other Trainers.", sections: [
  { heading: "When (all times local)", items: [
    "Saturday, August 1, 2026, 2:00 p.m. to 5:00 p.m. local time.",
    "Remote Raid limit increased from 10 to 20 from July 31 at 5:00 p.m. to August 1 at 8:00 p.m. PDT (GMT −7)."
  ] },
  { heading: "Featured Pokémon", icon: "✨", showImages: true, items: [{ name: "Gigantamax Rillaboom" }] },
  { heading: "Free Event Bonuses", items: [
  ...
  { heading: "Event Ticket ($4.99)", items: [
    "Event-exclusive Timed Research rewarding 1 Max Mushroom, 25,000 XP, and 6,400 Max Particles.",
    "2× XP from Max Battles.",
    "Gigantamax Rillaboom Max Battle Day Ultra Ticket Box (US$4.99) includes an event ticket and a bonus Premium Battle Pass.",
    "Tickets cannot be purchased with PokéCoins."
  ] },
  { heading: "Where can I find Max Battles?", items: [
    "Max Battles take great teamwork and only appear at Power Spots that sporadically appear in different locations.",
    "Use the Map to locate Max Battles and find community meetups.",
    "Be aware of your surroundings and follow guidelines from local health authorities."
  ] }
] },
  ...
  { id: 31, date: "2026-07-14", published: "2026-07-14", updated: "2026-07-14", lastUpdated: "July 14, 2026 at 11:07 AM", title: "Ultra Unlock: 10th Anniversary Edition — Full Guide", tag: "News", url: "https://pokemongo.com/en/news/ultraunlock-10th-anniversary-edition", icon: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_willow.webp", body: "Ultra Unlock's 10th Anniversary Edition stacks escalating Catch XP (6× → 8× → 10×), all 27 regional first-partner Pokémon across three wild-spawn phases, party-hat costumed raids, and the debut of Professor Willow's Assistant Pikachu. Paid Masterwork Research 'Glimmers of Gratitude' (US$4.99) rewards a Shiny Shaymin. Runs July 21 (10 a.m.) to July 27 (8 p.m.) local time.", fullBody: "Ultra Unlock: 10th Anniversary Edition runs Tuesday, July 21, 2026, at 10:00 a.m. to Monday, July 27, 2026, at 8:00 p.m. local time. Catch XP escalates in three windows — 6×, then 8×, then 10× — while all 27 regional first-partner Pokémon spawn in the wild across three matching phases. Beldum, Gible, and Dreepy show up near Lure Modules, party-hat Grimer, Raticate, Nidorino, Gengar, and Wobbuffet headline raids, and Professor Willow's Assistant Pikachu debuts in a 1★ raid tied to Willow's Birthday Research. Paid Masterwork Research 'Glimmers of Gratitude' (US$4.99) rewards an encounter with Shiny Shaymin. All times are local.", sections: [
    { heading: "Event Window", items: [
      "Starts: Tuesday, July 21 at 10:00 AM local time",
      "Ends: Monday, July 27 at 8:00 PM local time",
      "Bonus XP window with no daily point limit: July 25 (12:00 AM) to July 27 (7:59 PM)",
      "GO Pass rewards expire: July 29 at 7:59 PM local time"
    ] },
    { heading: "Catch Bonuses", items: [
      "6× XP for catching Pokémon: July 21 (10:00 AM) – July 23 (10:00 AM)",
      "8× XP for catching Pokémon: July 23 (10:00 AM) – July 25 (10:00 AM)",
      "10× XP for catching Pokémon: July 25 (10:00 AM) – July 27 (8:00 PM)",
      "Your first partner Pokémon may get a team-specific Special Background"
    ] },
    { heading: "Wild Encounters", items: [
      { text: "Phase 1 (Jul 21–23): Bulbasaur, Charmander, Squirtle, Chikorita, Cyndaquil, Totodile, Treecko, Torchic, Mudkip — all Shiny eligible ✨", img: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0001.webp" },
      { text: "Phase 2 (Jul 23–25): Turtwig, Chimchar, Piplup, Snivy, Tepig, Oshawott, Chespin, Fennekin, Froakie — all Shiny eligible ✨", img: "assets/pokemon-images/National-Dex/regular/Gen-4_Sinnoh/0387.webp" },
      { text: "Phase 3 (Jul 25–27): Rowlet, Litten, Popplio, Grookey, Scorbunny, Sobble, Sprigatito, Fuecoco, Quaxly — all Shiny eligible ✨", img: "assets/pokemon-images/National-Dex/regular/Gen-7_Alola/0722.webp" },
      { text: "Lure Modules attract Beldum ✨ and Gible ✨ (Shiny); Dreepy also appears near Lures (no confirmed Shiny boost)", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0885.webp" }
    ] },
    { heading: "Raids", items: [
      "1★: Grimer wearing a party hat ✨",
      "3★: Raticate wearing a party hat ✨, Nidorino wearing a party hat ✨, Gengar wearing a party hat ✨, Wobbuffet wearing a party hat ✨",
      "1★: Professor Willow's Assistant Pikachu ✨ — available during Willow's Birthday Research (Jul 21, 10 AM–8 PM); encounter available through Jul 27, 8 PM"
    ] },
    { heading: "Research & Rewards", items: [
      "Featured Charged Attacks unlock: Frenzy Plant (Grass starters), Blast Burn (Fire starters), Hydro Cannon (Water starters) for their fully evolved forms",
      "Event-themed Field Research rewards Poké Balls and event-themed encounters",
      "Masterwork Research: Glimmers of Gratitude (US$4.99) rewards an encounter with Shiny Shaymin ✨ — one-time purchase only",
      "Free avatar items: Professor Willow–Style Lab Coat, Pants, Gloves, Shoes, Backpack, and Glasses",
      "Event stickers available from PokéStops, Gifts, and the shop"
    ] },
    { heading: "GO Pass", items: [
      "Free: auto-granted July 21, 10:00 AM; 10th Anniversary Tee (Yellow), Golden Lure Module, event-themed encounters",
      "Tier 1 (Rank 10): 2× Candy for catching Pokémon (3× on Deluxe)",
      "Tier 2 (Rank 20): Increased chance of Candy XL from catching Pokémon",
      "Deluxe ($4.99): additional Golden Lure Module, additional event-themed encounters",
      "Deluxe + 6 Ranks ($6.99): everything in Deluxe, auto-grants Rank 7"
    ] },
    { heading: "Web Store", items: [
      { text: "GO Pass Deluxe gift with purchase: 10 Ultra Balls, 5 Max Revives, 1 Premium Battle Pass, 5 Max Potions", img: "assets/pokemon-images/Items/go_pass_deluxe.webp" },
      { text: "GO Pass Deluxe + 6 Ranks Ultra Box (web exclusive): 20 Ultra Balls, 10 Max Revives, 10 Max Potions, 2 Premium Battle Passes, 1 Incubator, 1 Super Incubator", img: "assets/pokemon-images/Items/ultraball_sprite.png" }
    ] }
  ] },
  { id: 32, date: "2026-07-14", published: "2026-07-14", updated: "2026-07-14", lastUpdated: "July 14, 2026 at 11:07 AM", title: "Ultra Unlock: Water Festival — Full Guide", tag: "News", url: "https://pokemongo.com/en/news/water-festival-2026", icon: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0845.webp", body: "Ultra Unlock: Water Festival brings Galar's Arrokuda, Barraskewda, and Cramorant to Pokémon GO, with escalating Stardust and XP catch bonuses, Water-type wild spawns, regional Tatsugiri forms, costumed raids (swim-ring Psyduck, scarf Lapras), and boosted Shiny Ducklett and Dewpider. Runs August 18 (10 a.m.) to August 24 (8 p.m.) local time.", fullBody: "Ultra Unlock: Water Festival runs Tuesday, August 18, at 10:00 a.m. to Monday, August 24, 2026, at 8:00 p.m. local time. Stardust and XP catch bonuses escalate across three windows, Water-type Pokémon fill the wild, and Rainy Lure Modules attract Arrokuda. Galar's Arrokuda, Barraskewda, and Cramorant make their Pokémon GO debuts, region-locked Tatsugiri forms headline 1★ raids alongside swim-ring Psyduck, and scarf-wearing Lapras, Hisuian Samurott, and Dondozo appear in 3★ raids. Shiny Ducklett and Shiny Dewpider have increased encounter rates. All times are local.", sections: [
    { heading: "Event Window", items: [
      "Starts: Tuesday, August 18 at 10:00 AM local time",
      "Ends: Monday, August 24 at 8:00 PM local time"
    ] },
    { heading: "Catch Bonuses", items: [
      "2× XP and 3× Stardust for catching Pokémon: August 18 (10:00 AM) – August 20 (10:00 AM)",
      "3× XP and 4× Stardust for catching Pokémon: August 20 (10:00 AM) – August 22 (10:00 AM)",
      "4× XP and 5× Stardust for catching Pokémon: August 22 (10:00 AM) – August 24 (8:00 PM)",
      "Rainy Lure Modules attract Arrokuda",
      "Increased chance to encounter Shiny Ducklett and Shiny Dewpider ✨"
    ] },
    { heading: "Wild Encounters", items: [
      { text: "Psyduck, Goldeen ✨, Mareanie ✨, Wimpod, Feebas, Clamperl, Wiglett (appears more frequently)", img: "assets/pokemon-images/National-Dex/regular/Gen-1_Kanto/0054.webp" },
      { text: "Ducklett (Shiny boosted) ✨", img: "assets/pokemon-images/National-Dex/regular/Gen-5_Unova/0580.webp" },
      { text: "Dewpider (Shiny boosted) ✨", img: "assets/pokemon-images/National-Dex/regular/Gen-7_Alola/0751.webp" },
      { text: "Arrokuda (Debut) — Field Research reward and Rainy Lure Modules", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0846.webp" },
      { text: "Cramorant (Debut) — hatches from Eggs; also a GO Pass and GO Battle League reward", img: "assets/pokemon-images/National-Dex/regular/Gen-8_Galar/0845.webp" }
    ] },
    { heading: "Raids", items: [
      "1★: Tatsugiri (Curly Form) — Europe, Middle East, Africa",
      "1★: Tatsugiri (Droopy Form) — Americas",
      "1★: Tatsugiri (Stretchy Form) — Asia-Pacific",
      "1★: Psyduck wearing a swim ring",
      "3★: Lapras wearing a scarf, Hisuian Samurott, Dondozo"
    ] },
    { heading: "Research & Debuts", items: [
      "Arrokuda, Barraskewda, and Cramorant — first discovered in the Galar region — make their Pokémon GO debuts",
      "Barraskewda debuts via evolution: evolve Arrokuda with 50 Candy",
      "Cramorant has form-changing battle mechanics, is excluded from the Competitors Cup, and appears as a GO Battle League reward starting at Rank 16 (more frequently on the premium reward track)"
    ] },
    { heading: "GO Pass", items: [
      "Free: auto-granted during the event",
      "Tier (Rank 10): 2× Candy for catching Pokémon",
      "Tier (Rank 20): 1/2 Egg Hatch Distance for Eggs placed in an Incubator during the event",
      "Deluxe ($4.99): GO Pass Deluxe rewards plus Web Store gift with purchase",
      "Deluxe + 6 Ranks ($6.99): everything in Deluxe, plus 6 additional Ranks"
    ] },
    { heading: "Web Store", items: [
      { text: "Web Store bundles during the event include Ultra Balls, Max Revives, Premium Battle Passes, and Potions with purchase", img: "assets/pokemon-images/Items/ultraball_sprite.png" }
    ] }
  ] },
  { id: 29, date: "2026-06-26", published: "2026-06-26", updated: "2026-06-26", lastUpdated: "June 26, 2026 at 10:46 AM", title: "Ozone Ascent — Rayquaza & Dragon Ascent", tag: "News", url: "https://pokemongo.com/en/news/ozone-ascent-2026", icon: "assets/pokemon-images/Mega/regular/Gen-3_Hoenn/0384_mega.webp", body: "Rayquaza ascends July 25–26! Complete free Timed Research for a Meteorite and a Rayquaza encounter, or grab the US$4.99 paid Timed Research to also earn Rayquaza Mega Energy. Use Meteorites to teach Rayquaza the Charged Attack Dragon Ascent — the only way the Sky High Pokémon can Mega Evolve.", fullBody: "Ozone Ascent runs Saturday, July 25, at 10:00 a.m. to Sunday, July 26, 2026, at 8:00 p.m. local time, with Rayquaza in the spotlight. Free Timed Research rewards a Meteorite, an encounter with Rayquaza, and more, while paid Timed Research (US$4.99, or the equivalent pricing tier in your local currency) adds Rayquaza Mega Energy on top. Unlike other Mega-Evolving Pokémon, Rayquaza can't Mega Evolve with Mega Energy alone — you'll need to teach it the Charged Attack Dragon Ascent using Meteorites. Timed Research must be completed and its rewards claimed before the event ends on Sunday, July 26, at 8:00 p.m. local time. All times are local.", sections: [
    { heading: "When (all times local)", items: [
      ...
      "Timed Research expires Sunday, July 26, 2026, at 8:00 p.m. local time."
    ] },
    { heading: "Featured Pokémon", icon: "✨", showImages: true, items: [{ name: "Rayquaza (no shiny)" }] },
    { heading: "Timed Research (free)", items: [
      "Complete Timed Research tasks during the event to earn rewards.",
      "Rewards include a Meteorite, an encounter with Rayquaza, and more.",
      "Tasks must be completed and rewards claimed before Sunday, July 26, 2026, at 8:00 p.m. local time."
    ] },
    { heading: "Paid Timed Research", items: [
      "Available for US$4.99 (or the equivalent pricing tier in your local currency).",
      "Rewards include a Meteorite, Rayquaza Mega Energy, an encounter with Rayquaza, and more.",
      "Purchases — including those made for other Trainers — are non-refundable (subject to applicable law).",
      "Cannot be purchased with PokéCoins.",
      "Can be gifted to friends at Great Friends level or higher."
    ] },
    { heading: "Mega Rayquaza & Dragon Ascent", items: [
      "Unlike other Mega-Evolving Pokémon, Rayquaza can't Mega Evolve with Mega Energy alone.",
      "Teach Rayquaza the Charged Attack Dragon Ascent using Meteorites.",
      "Dragon Ascent can only be learned by using Meteorites."
    ] },
    { heading: "Good to Know", items: [
      "Mega Rayquaza is one of the best Dragon- and Flying-type attackers in the game — well worth powering up.",
      "Save your Meteorites — they're the only way to teach Rayquaza Dragon Ascent.",
      "Be aware of your surroundings and follow guidelines from local health authorities."
    ] }
  ] },
  { id: 28, date: "2026-06-26", published: "2026-06-26", updated: "2026-06-26", lastUpdated: "June 26, 2026 at 10:39 AM", title: "Happy Birthday, Professor Willow! — Willow's Birthday Research", tag: "News", url: "https://pokemongo.com/en/news/happy-birthday-professor-willow-2026", icon: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_willow.webp", body: "Wish Professor Willow a happy birthday on Tuesday, July 21! A brand-new costumed Pokémon — Professor Willow's assistant Pikachu — debuts in Field Research, Timed Research, snapshot surprises, and raids, and it can be Shiny or sport a team leader–specific Special Background. A free Timed Research line rewards Pikachu encounters, a Lucky Egg, and a Star Piece — finish it before July 27.", fullBody: "To celebrate Professor Willow's birthday, a new costumed Pokémon — Professor Willow's assistant Pikachu — appears in Field Research, Timed Research, snapshot surprises, and raids on Tuesday, July 21, 2026, from 10:00 a.m. to 8:00 p.m. local time. A free Timed Research opportunity rewards encounters with the costumed Pikachu plus a Lucky Egg and a Star Piece, and event-themed Field Research adds PokéCoins, Lucky Eggs, and more. All times are local.", sections: [
    { heading: "When (all times local)", items: [
      "Tuesday, July 21, 2026, 10:00 a.m. to 8:00 p.m."
    ] },
    { heading: "Pokémon Debut", items: [
      "A new costumed Pokémon — Professor Willow's assistant Pikachu — appears in Field Research, Timed Research, snapshot surprises, and raids.",
      "You might even encounter a Shiny one, or one with a team leader–specific Special Background, if you're lucky."
    ] },
    { heading: "Timed Research (free)", items: [
      "A free Timed Research opportunity will be available.",
      "Rewards include encounters with Professor Willow's assistant Pikachu, 1 Lucky Egg, 1 Star Piece, and more.",
      "You might even encounter a Shiny Professor Willow's assistant Pikachu from these encounters.",
      "Tasks must be completed and their rewards must be claimed before Monday, July 27, 2026, at 8:00 p.m. local time."
    ] },
    { heading: "Field Research", items: [
      "Event-themed Field Research will be available.",
      "Complete Field Research tasks to earn PokéCoins, Lucky Eggs, additional encounters with Professor Willow's assistant Pikachu, and more."
    ] },
    { heading: "Good to Know", items: [
      "The costumed Pikachu can be Shiny — check every encounter.",
  ...
  { id: 26, date: "2026-06-25", published: "2026-06-25", title: "Special Anniversary Pikachu Celebration — Every Costumed Pikachu", tag: "News", url: "https://pokemongo.com/en/news/special-anniversary-pikachu-celebration", icon: "assets/pokemon-images/National-Event-Costume-Dex/regular/Gen-1_Kanto/0025_f2867.webp", body: "Keep the festivities going! From July 13–20, three back-to-back celebrations spotlight different costumed Pikachu. Tricks & Treats, Winter Carnival, and summer-flair Pikachu appear in the wild, while nine costumed Pikachu — Dapper red/yellow/blue, safari hat, varsity jacket, amethyst crown, and World Championships 2022–2024 — fill one-star raids. A free event GO Pass adds 2×/3× transfer Candy and catch Stardust bonuses.", fullBody: "The Special Anniversary Pikachu Celebration runs Monday, July 13, at 10:00 a.m. to Monday, July 20, 2026, at 8:00 p.m. local time. The event is split into three celebrations, each featuring different costumed Pikachu to find in the wild and in Field Research tasks. Standard Pikachu also appears more frequently in the wild and is easier to catch, and a free event GO Pass (with a paid GO Pass Deluxe upgrade) delivers milestone bonuses and rewards throughout.", sections: [
    { heading: "When (all times local)", items: [
      ...
      "Celebration 1: July 13, 10:00 a.m. to July 15, 10:00 a.m.",
      "Celebration 2: July 15, 10:00 a.m. to July 18, 10:00 a.m.",
      "Celebration 3: July 18, 10:00 a.m. to July 20, 8:00 p.m."
    ] },
    { heading: "Wild Encounters (all can be Shiny)", items: [
      "Pikachu wearing a Tricks & Treats costume.",
      "Pikachu wearing a Winter Carnival outfit.",
      "Pikachu with summer flair.",
      "Standard Pikachu appears more frequently and is easier to catch.",
      "…and more event-themed Pokémon."
    ] },
    { heading: "One-Star Raids (all can be Shiny)", items: [
    ...
    { heading: "GO Pass Milestone Bonuses", items: [
      "Tier 1 – Rank 10: 2× Candy for transferring Pokémon; increased chance for Trainers level 31 and up to receive Candy XL for transferring Pokémon.",
      "Tier 1 – Rank 10 (GO Pass Deluxe): 3× Candy for transferring Pokémon; even greater Candy XL chance for Trainers level 31 and up.",
      "Tier 2 – Rank 20: 2× Stardust for catching Pokémon.",
      "Tier 2 – Rank 20 (GO Pass Deluxe): 3× Stardust for catching Pokémon.",
      "Keep an eye out for additional bonus tasks for this GO Pass during the event."
    ] },
    { heading: "GO Pass: Special Anniversary Pikachu Celebration", items: [
      "Available July 13, 10:00 a.m. to July 20, 8:00 p.m. local time; granted automatically to all Trainers on July 13 at 10:00 a.m.",
      "Free track: encounters with event-themed Pokémon, more than 150,000 XP, a Lucky Egg, Golden Razz Berries, and even more goodies.",
      "GO Pass Deluxe (US$4.99): additional event-themed encounters, more than 350,000 XP, Lucky Eggs, Silver Pinap Berries, a Lure Module, and even more goodies — plus upgraded rewards and faster progression.",
      "GO Pass Deluxe + 6 Ranks (US$6.99): everything in Deluxe and automatically reach Rank 7.",
      "Rewards unlocked in the GO Pass expire Wednesday, July 22, 2026, at 8:00 p.m. local time."
    ] },
    { heading: "Pokémon GO Web Store", items: [
      "A GO Pass Deluxe purchased via the Web Store activates as soon as the GO Pass begins.",
      "Upgrading to GO Pass Deluxe or GO Pass Deluxe + 6 Ranks on the Web Store includes a gift of 10 Ultra Balls, 5 Max Revives, 1 Premium Battle Pass, and 5 Max Potions.",
      "Web-exclusive GO Pass Deluxe + 6 Ranks Ultra Box adds 20 Ultra Balls, 10 Max Revives, 10 Max Potions, 2 Premium Battle Passes, 1 Incubator, and 1 Super Incubator."
    ] },
    { heading: "Good to Know", items: [
      "Three rotating celebrations mean different costumed Pikachu across the week — check all three windows.",
      "Every costumed Pikachu (wild and raid) can be Shiny if you're lucky.",
      "All prices are in USD or the equivalent pricing tier in your local currency."
    ] }
  ] },
  { id: 25, date: "2026-06-18", published: "2026-06-18", title: "The Road of Legends — Five Days of Legendary Raids", tag: "News", url: "https://pokemongo.com/en/news/road-of-legends-2026", icon: "assets/pokemon-images/Raid-Eggs/5-star.png", body: "From July 6–10, the road to Pokémon GO Fest 2026: Global fills raids with Legendary Pokémon and Ultra Beasts. Monday features nearly the entire Legendary roster, then each day spotlights marquee bosses — White & Black Kyurem, the Necrozma fusions, Crowned Zacian & Zamazenta, Origin Dialga & Palkia — before Friday's Primal Kyogre and Primal Groudon. No Remote Raid Pass limit all event long.", fullBody: "The Road of Legends runs Monday, July 6, 2026, at 12:01 a.m. to Friday, July 10, 2026, at 11:59 p.m. local time — a five-day raid event leading into Pokémon GO Fest 2026: Global. Legendary Pokémon, Ultra Beasts, and other powerful Pokémon appear in raids throughout the event, with a different featured lineup each day and special Charged Attacks available through Elite TM for the first time.", sections: [
    { heading: "When (all times local)", items: [
      "Monday, July 6, 12:01 a.m. through Friday, July 10, 11:59 p.m.",
      "Leads directly into Pokémon GO Fest 2026: Global on July 11–12."
    ] },
    { heading: "Five-Star Raid Schedule", items: [
      "Monday, July 6 — Nearly every Legendary Pokémon and Ultra Beast (Articuno through Enamorus); Uxie, Mesprit, Azelf and several Ultra Beasts are region-exclusive.",
      "Tuesday, July 7 — White Kyurem, Zekrom, Dawn Wings Necrozma.",
      "Wednesday, July 8 — Black Kyurem, Reshiram, Dusk Mane Necrozma.",
      "Thursday, July 9 — Crowned Sword Zacian, Crowned Shield Zamazenta.",
      "Friday, July 10 — Origin Forme Dialga, Origin Forme Palkia."
    ] },
    { heading: "Mega & Primal Raids", items: [
      "Monday — Mega Salamence.",
      "Tuesday — Mega Tyranitar.",
      "Wednesday — Mega Gardevoir.",
      "Thursday — Mega Gengar.",
      "Friday — Primal Kyogre and Primal Groudon."
    ] },
    { heading: "Raid Hours (local time)", items: [
      "Five-star feature: 6:00–7:00 p.m. each day (Monday extended to 6:00–8:00 p.m.).",
      "Mega Raid Hour: 7:00–8:00 p.m. daily.",
      "Friday Primal Raid Hour: 7:00–8:00 p.m."
    ] },
    { heading: "Featured Charged Attacks (Elite TM)", items: [
      "Kyurem caught from raids may know Glaciate.",
      "Origin Forme Dialga may know Roar of Time; Origin Forme Palkia may know Spacial Rend.",
      "Roar of Time and Spacial Rend are available through Elite TM for the first time in Pokémon GO."
    ] },
    { heading: "Egg Hatches (all but Goomy can be Shiny)", items: [
      "2 km — Weedle, Pidgey, Abra, Gastly, Pinsir, Aerodactyl, Mareep, Larvitar, Treecko, Torchic, Mudkip, Ralts, Bagon, Beldum, Gible, Riolu, Snover, Audino, Larvesta.",
      "5 km — Bulbasaur, Charmander, and Squirtle wearing Pikachu visors, Heracross, Mime Jr., Larvesta.",
      "10 km — Tropius, Axew, Bouffalant, Goomy, Jangmo-o, Toxel, Frigibax, Larvesta."
    ] },
    { heading: "Event Bonuses", items: [
      "No Remote Raid Pass limit from Monday, July 6 to Sunday, July 12.",
      "Up to 2 free Raid Passes per day from spinning Gym Photo Discs.",
      "Trainers level 31 and up are guaranteed Candy XL for trading Pokémon.",
      "Premier Balls are more effective after raids; Party Power charges faster in raids.",
      "Special Backgrounds on Pokémon caught from five-star, Mega, and Primal raids July 6–12.",
      "Event stickers from spinning PokéStops and opening Gifts."
    ] },
    { heading: "GO Pass: Road of Legends (July 6–12)", items: [
      "Free track: team-hat Pikachu encounters, Legendary raid Candy, GO Fest Tee avatar item, Link Charges, Incubators, Incense, a Lucky Trinket at Rank 100, and a Link Charge cap raised to 2,000 through GO Fest: Global.",
      "Deluxe adds: Team Leader Pose, Gold Bottle Cap, Mewtwo with a Special Background, Candy XL, 2× Hatch Stardust and 2× Hatch Candy, an extra 5,000 XP from raids, bonus Candy and Candy XL from 5★+ raids, and 1/2 Egg Hatch Distance."
    ] },
    { heading: "Good to Know", items: [
      "Daily Discoveries will not take place during Road of Legends or GO Fest 2026: Global (July 6–12).",
      "Pikachu has a few playful surprises lined up starting July 13 at 10:00 a.m. local time."
    ] }
  ] },
  { id: 24, date: "2026-06-10", published: "2026-06-10", title: "Choose Your Path Timed Research & Daily Discoveries Update", tag: "Update", url: "https://pokemongo.com/news/choose-your-path-forever-forward-2026", body: "New Choose Your Path Timed Research lets you pick Explore, Catch, or Battle during non-event weeks — first up is Fossil Fun, June 17–21. Daily Discoveries also refresh starting June 15: Spotlight Hour moves to Thursdays 6–7 PM, with Rare Candy XL added to Max Monday and Wednesday Raid Hour.", fullBody: "During a Choose Your Path Timed Research week you choose one of three paths — Explore, Catch, or Battle — and your choice shapes that week's Timed Research tasks and bonuses. You may have different research opportunities than other Trainers. Alongside it, Forever Forward's Daily Discoveries are being updated starting Monday, June 15.", sections: [
    { heading: "Choose Your Path Timed Research", items: [
      "A new way to play during non-event weeks — pick one of three paths: Explore, Catch, or Battle.",
      "Your choice shapes the Timed Research tasks and bonuses throughout the week.",
      "You may have different research opportunities than other Trainers."
    ] },
    { heading: "Choose Your Path Schedule (all times local)", items: [
      "Fossil Fun — Wed, June 17, 10:00 AM to Sun, June 21, 8:00 PM",
      "Charged Embers — Tue, June 30, 10:00 AM to Fri, July 3, 8:00 PM",
      "Fairy Trail — Tue, July 28, 10:00 AM to Mon, August 3, 8:00 PM",
      "Venom and Vines — Tue, August 11, 10:00 AM to Mon, August 17, 8:00 PM"
    ] },
    { heading: "Updated Daily Discoveries (starting Monday, June 15)", items: [
      "Active 12:00 AM–11:59 PM local time unless otherwise stated.",
      "Max Monday — 1 Rare Candy XL per in-person Max Battle; Power Spots refresh more often with extra active spots; rotating Dynamax Pokémon.",
      "Showcase Tuesday — enter up to five PokéStop Showcases; more PokéStops may host Showcases.",
      "Wednesday Raid Hour — 1 Rare Candy XL per in-person Raid Battle.",
      "GO Battle Thursdays and Spotlight Hour — Spotlight Hour returns 6:00–7:00 PM local time.",
      "Friendship Friday — up to two additional Special Trades; trades require 20% less Stardust.",
      "Scenic Sunday — more wild Pokémon; more Pokémon on Routes with Incense attracting even more; reduced Buddy Candy distance on Routes; encounter Mateo up to three times daily."
    ] },
    { heading: "Upcoming Spotlight Hours (Thursdays, 6–7 PM local)", spotlight: true, note: "You may encounter a Shiny one — if you're lucky!", items: [
      { name: "Swinub", date: "Thu, Jun 18", bonus: "2× Candy", action: "for transferring", res: "candy" },
      { name: "Wingull", date: "Thu, Jun 25", bonus: "2× Stardust", action: "for catching", res: "stardust" },
      { name: "Pidgey", date: "Thu, Jul 2", bonus: "2× XP", action: "for evolving", res: "xp" },
      { name: "Zubat", date: "Thu, Jul 16", bonus: "2× XP", action: "for catching", res: "xp" },
      { name: "Eevee", date: "Thu, Jul 23", bonus: "2× Candy", action: "for catching", res: "candy" },
      { name: "Bidoof", date: "Thu, Jul 30", bonus: "2× Candy", action: "for transferring", res: "candy" }
    ] },
    { heading: "Good to Know", items: [
      "Daily Discoveries may change as Niantic/Scopely experiments to make the best experience.",
  ...
  { id: 23, date: "2026-06-04", published: "2026-06-04", title: "Skarmory Super Mega Raid Day — June 27", tag: "Update", url: "https://pokemongo.com/news/skarmory-super-mega-raid-day-2026", body: "Mega Skarmory debuts in Super Mega Raids on June 27, 2–5 PM local time! Up to 6 free Raid Passes, boosted Shiny Skarmory odds, Remote Raid limit raised to 20, free Timed Research with a Houndoom encounter, and a $4.99 ticket for 14 Raid Passes plus bonus XP and Stardust.", fullBody: "Skarmory Super Mega Raid Day runs Saturday, June 27, 2026, from 2:00 PM – 5:00 PM local time — stay sharp as Mega Skarmory takes flight in its Super Mega Raid debut:", sections: [{ heading: "Featured Pokémon", items: ["Mega Skarmory — Super Mega Raid debut ✨", "Increased chance of encountering Shiny Skarmory from Super Mega Raids"] }, { heading: "Free Bonuses", items: ["Up to 6 free Raid Passes from spinning Gym Photo Discs", "Remote Raid Pass limit increased to 20 (Fri, June 26, 5:00 PM – Sat, June 27, 8:00 PM PDT)", "Increased chance of encountering Shiny Skarmory from Super Mega Raids"] }, { heading: "Free Timed Research", items: ["Available during event hours — tasks must be completed and rewards claimed before the Timed Research expires", "Rewards: 1 Premium Battle Pass + 1 Houndoom encounter"] }, { heading: "Paid Ticket ($4.99)", items: ["Up to 14 Raid Passes from spinning Gym Photo Discs", "Increased chance to get Rare Candy XL from Raid Battles", "An additional 5,000 XP per Super Mega Raid Battle", "An additional 5,000 Stardust per Super Mega Raid Battle", "Bonuses effective Saturday, June 27, 2026, 2:00 PM – 5:00 PM local time", "Available in the in-game shop until Saturday, June 27 at 5:00 PM local time", "Giftable to Great Friends or higher", "Non-refundable; cannot be purchased with PokéCoins"] }, { heading: "Web Store Bundle", items: ["Skarmory Super Mega Raid Day Ultra Ticket Box ($4.99): Event ticket + 1 Premium Battle Pass at no additional cost"] }, { heading: "New Feature — Super Mega Raid Map", items: ["Use the new web-based map at pokemongo.com/map to locate Super Mega Raids (they only appear at certain Gyms)", "View nearby community meetups — including those hosted by Community Ambassadors — through the map", "Coordinate more easily with friends, family, and other Trainers in your community"] }, { heading: "Tips", items: ["Mega Skarmory (Steel/Flying) is 4× weak to Fire and also weak to Electric.", "Top counters (verified): Mega Charizard Y (Fire Spin / Blast Burn), Shadow Thundurus Therian (Volt Switch / Wildbolt Storm), Shadow Heatran (Fire Spin / Magma Storm), Shadow Raikou (Thunder Shock / Wild Charge), Regieleki (Lock-On / Thunder Cage), Shadow Magnezone (Volt Switch / Wild Charge).", "Super Mega Raids need a coordinated lobby (8+ Trainers minimum due to Shield phases; lobby cap 20).", "Stack a Premium Battle Pass before the event to maximize Raid Pass usage.", "Shiny Skarmory odds are boosted — check every encounter.", "Complete the free Timed Research before it expires for a free Premium Battle Pass and a Houndoom encounter.", "Ultra Ticket Box on the Web Store includes the event ticket + Premium Battle Pass — best value for ticketed players."] }] },
  { id: 21, date: "2026-05-27", published: "2026-05-27", updated: "2026-06-04", lastUpdated: "June 4, 2026", title: "June Content Update — Forever Forward", tag: "News", url: "https://pokemongo.com/news/june-2026-content-update", body: "June 2026's full content rundown — Featured Necrozma, Celesteela (Southern Hemisphere) and Kartana (Northern Hemisphere). Max Battles rotate Inkay → Electabuzz → Roggenrola → Hoothoot → Pidove. 5★ Raids run Reshiram → Zekrom → Necrozma → Celesteela/Kartana. Mega Raids feature Mega Audino, Mega Lopunny, Mega Scizor, and Mega Pidgeot. Shadow Dialga returns to Shadow Raids all month, plus Community Day, the Skarmory Super Mega Raid Day debut, Flying Taxi, and the Team Leader Quests.", fullBody: "Pokémon GO's June 2026 Content Update launches alongside the Forever Forward Season and the game's 10th Anniversary celebrations. Necrozma is the headline 5★ raid debut for the month, with Celesteela appearing in the Southern Hemisphere and Kartana appearing in the Northern Hemisphere for the closing week. Max Battles, Mega Raids, Raid Hours, Community Day, the Skarmory Super Mega Raid Day debut, the Flying Taxi event, and Shadow Dialga round out a packed schedule. All times are local unless otherwise specified.", sections: [
    ...
    { heading: "Events", icon: "📅", items: [
      ...
      "Skarmory Super Mega Raid Day — June 27 (Mega Skarmory ✨ debut in Super Mega Raids)"
    ] },
    ...
    { heading: "Tips", icon: "💡", items: [
      ...
      "Skarmory Super Mega Raid Day — Saturday, June 27, 2:00–5:00 PM local. Mega Skarmory debuts in Super Mega Raids; Steel/Flying so 4× weak to Fire — bring Mega Blaziken / Mega Charizard Y / Shadow Heatran. Free Timed Research rewards a Premium Battle Pass + Houndoom encounter. $4.99 ticket adds 14 Raid Passes, Rare Candy XL, +5,000 XP and +5,000 Stardust per Super Mega Raid Battle.",
...
const STORE_ITEM_VALUES = {
  ...
  "Skarmory Super Mega Raid Day Ticket": 4.99,
  "Pokémon Storage Upgrade": 1.99,
  "Item Bag Upgrade": 1.99,
  "PokéCoin": 0.007
};

const ESTIMATED_VALUE_ITEMS = new Set(["Ultra Ball", "GO Pass Deluxe", "Fashion Raid Day Ticket", "Shadow Entei Raid Day Ticket", "Falinks Super Mega Raid Day Ticket", "Skarmory Super Mega Raid Day Ticket", "PokéCoin"]);
...
const WEB_STORE_BOXES = [
  ...
  {
    name: "Skarmory Super Mega Raid Day Ultra Ticket Box",
    price: 4.99,
    category: "Event Bundle",
    limited: true,
    ...
    oneTime: true,
    items: [
      { name: "Skarmory Super Mega Raid Day Ticket", qty: 1 },
      { name: "Premium Battle Pass", qty: 1 }
    ],
    ticketBonuses: ["Up to 14 Raid Passes from spinning Gym Photo Discs", "Increased chance to get Rare Candy XL from Raid Battles", "+5,000 XP per Super Mega Raid Battle", "+5,000 Stardust per Super Mega Raid Battle"]
  },
...
function renderCounters(counters, th) {
  ...
  const tilesHTML = `<div style="display:flex;${!isMob ? "flex-wrap:wrap;gap:8px" : "flex-direction:column;gap:6px"}">${renderCounterTiles(counters, th, isMob)}</div>`;
  if (isMob) {
    ...
    return `<div style="border:1.5px solid ${th.border};border-radius:12px;overflow:hidden;background:${th.accentBgSubtle("#3498DB")}">
      <button class="acc-trigger" data-open="false" onclick="toggleAccordion(this)" aria-expanded="false" style="display:flex;align-items:center;gap:10px;padding:12px 14px;border:none;color:${th.text};width:100%;text-align:left;cursor:pointer;background:transparent">
        ...
        <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:2px">
          <span style="font-size:13px;font-weight:700;color:${th.text}">Top Counters</span>
          <span style="font-size:11px;color:${th.textMuted}">vs ${esc(counters.label)}</span>
        </div>
        <span style="font-size:10px;font-weight:700;color:#3498DB;background:${th.accentBg("#3498DB")};padding:3px 8px;border-radius:10px;letter-spacing:0.3px;text-transform:uppercase;white-space:nowrap">Tap to view</span>
        <span style="display:flex;align-items:center;color:${th.textMuted}">${chevronSVG}</span>
      </button>
      <div class="acc-content" data-open="false">
        <div style="padding:10px 12px 12px">${tilesHTML}</div>
      </div>
    </div>`;
  }
...
function renderCounterTiles(counters, th, isMob) {
  return counters.pokemon.map((c, i) => {
      ...
      if (breakpoint !== "mobile") {
        return `<div style="display:flex;flex-direction:column;align-items:center;padding:12px 8px;border-radius:12px;background:${th.accentBgSubtle("#3498DB")};border:1.5px solid ${th.border};font-size:13px;color:${th.textSecondary};line-height:1.4;text-align:center;flex:1;min-width:140px;max-width:200px;position:relative">
          ...
          <div style="margin-top:4px;font-size:11px;display:flex;flex-wrap:wrap;gap:3px;align-items:center;justify-content:center">
            <span style="color:${th.textSecondary}">${esc(c.fast)}${c.fastNote ? ` <span style="font-size:10px;font-weight:700;color:#E67E22;background:${th.accentBg("#E67E22")};padding:1px 6px;border-radius:10px;margin-left:2px">${esc(c.fastNote)}</span>` : ""}</span>
      ...
      return `<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;background:${th.accentBgSubtle("#3498DB")};font-size:13px;color:${th.textSecondary};line-height:1.4">
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="margin-top:2px;font-size:12px;display:flex;flex-wrap:wrap;gap:4px;align-items:center">
            ...
            ${c.fastNote ? `<span style="font-size:10px;font-weight:700;color:#E67E22;background:${th.accentBg("#E67E22")};padding:1px 6px;border-radius:10px">${esc(c.fastNote)}</span>` : ""}
            <span style="color:${th.textFaint}">/</span>
            <span style="color:${th.textSecondary}">${esc(c.charged)}</span>
...
function renderDetailSection(title, emoji, items, color, th, showImages, noSparkles, groupSize, countersData, collapsibleOnMobile) {
  ...
  const countersByBoss = (() => {
    ...
    list.forEach(c => {
      ...
      const names = Array.isArray(c.bosses) && c.bosses.length
        ? c.bosses
        : (c.label ? [c.label.split("(")[0].trim()] : []);
      names.forEach(n => { if (n) map[n] = c; });
    });
    return map;
  })();
  const findCountersForBoss = bossLabel => {
    ...
    const stripped = cleaned.replace(/\s*\((Southern|Northern)\s+Hemisphere\)\s*$/i, "").trim();
  ...
  const hasSourceMarkers = items.some(it =>
  ...
  if (untiered.length > 0) {
    const SOURCE_META = {
      SpecialResearch: { label: "Special Research", iconImg: "assets/pokemon-images/icons/QuestSpecialResearchStamp_00.png" },
      SuperMega: { label: "Super Mega Raids", iconImg: "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png" },
      Park:    { label: "Park Spawns",         iconEmoji: "🌿" },
      Wild:    { label: "Wild Spawns",         iconImg: "assets/pokemon-images/icons/ic_grass.png" },
      Incense: { label: "Incense Encounters",  iconImg: "assets/pokemon-images/Items/Incense_0.png" },
      Raid:    { label: "Raids",               iconImg: "assets/pokemon-images/Raid-Eggs/3-star.png" }
    };
    const sourceOf = (it) => {
    ...
    const bySource = { SpecialResearch: [], SuperMega: [], Park: [], Wild: [], Incense: [], Raid: [] };
    untiered.forEach(it => bySource[sourceOf(it)].push(it));
    const contentLayout = breakpoint !== "mobile" ? "flex-wrap:wrap;gap:8px" : "flex-direction:column;gap:5px";
    ["SpecialResearch", "SuperMega", "Park", "Wild", "Incense", "Raid"].forEach(src => {
      ...
      const itemsHTML = list.map(item => renderBossItem(item, color, th, breakpoint !== "mobile", noSparkles, groupSize)).join("");
      // Only the Super Mega Raids source bucket renders a counters block, matching how tiered groups do it.
      const srcCounters = src === "SuperMega"
        ? list.map(item => findCountersForBoss(item)).filter((c, i, arr) => c && arr.indexOf(c) === i)
        : [];
      const srcCountersHTML = srcCounters.length > 0
    ...
    <div style="background:${th.surface};border-radius:20px;border:1.5px solid ${th.border};overflow:hidden;box-shadow:${th.shadowLg}">
      ...
      <div style="padding:20px 20px 24px;display:flex;flex-direction:column;gap:42px">
        ...
        ${event.details.rocketLeaders ? (() => {
          ...
          const isMob = breakpoint === "mobile";
          const portraitSize = isMob ? 64 : 84;
          const portraits = rl.leaders.map(l => `<div style="display:flex;flex-direction:column;align-items:center;gap:6px">
            <div style="position:relative;width:${portraitSize}px;height:${portraitSize}px;border-radius:50%;background:${th.accentBg(event.color)};border:2.5px solid ${event.color};overflow:hidden;box-shadow:0 2px 8px ${event.color}40">
              <img src="${l.img}" alt="${esc(l.name)}" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover" onerror="this.style.display='none'" />
            </div>
            <div style="font-size:${isMob ? 12 : 13}px;font-weight:700;color:${th.text};letter-spacing:0.3px">${esc(l.name)}</div>
          </div>`).join("");
          return `<div>
            <h4 style="margin:0 0 10px 0;font-size:13px;font-weight:700;color:${th.text};display:flex;align-items:center;gap:8px"><img src="assets/pokemon-images/icons/teamrocket_r_full.png" alt="" style="width:18px;height:18px;object-fit:contain;flex-shrink:0" /> ${esc(rl.title || "Team GO Rocket Leaders")}</h4>
            <div style="border:1.5px solid ${th.countdownBorder(event.color)};border-radius:14px;padding:${isMob ? "16px 14px" : "20px 18px"};background:${th.accentBgSubtle(event.color)};display:flex;flex-direction:column;align-items:center;gap:${isMob ? 12 : 16}px">
              <div style="display:flex;justify-content:center;align-items:flex-start;gap:${isMob ? 18 : 32}px;flex-wrap:wrap">${portraits}</div>
              ${rl.subtitle ? `<p style="margin:0;font-size:${isMob ? 13 : 13.5}px;color:${th.textSecondary};line-height:1.55;text-align:center;max-width:560px">${esc(rl.subtitle)}</p>` : ""}
            </div>
          </div>`;
        })() : ""}
        ${(event.details.saturdayRaids || event.details.sundayRaids || event.details.dailyRaids) ? (() => {
          const isMob = breakpoint === "mobile";
          const chevronSVG = `<svg class="acc-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>`;
          const renderTierAcc = (tierLabel, items, tierColor, tierKey, iconOverride) => {
            const eggUrl = TIER_EGGS[tierKey];
            const eggImg = iconOverride
              ? `<img src="${iconOverride}" style="width:26px;height:26px;object-fit:contain;flex-shrink:0" />`
              : (eggUrl ? `<img src="${eggUrl}" style="width:26px;height:26px;object-fit:contain;flex-shrink:0" />` : "");
            const stars = iconOverride ? "" : renderRaidHeads(tierKey);
          ...
          const renderDayAcc = (sr, dayLabel, accent) => {
            ...
            const totalCount = (sr.superMegaRaid ? sr.superMegaRaid.length : 0) + (sr.megaRaids ? sr.megaRaids.length : 0) + (sr.fiveStarRaids ? sr.fiveStarRaids.length : 0) + (sr.primalRaids ? sr.primalRaids.length : 0);
            return `<div style="border:1.5px solid ${th.countdownBorder(accent)};border-radius:14px;overflow:hidden;background:${th.accentBgSubtle(accent)}">
              ...
              <div class="acc-content" data-open="false">
                <div style="padding:0 12px 12px;display:flex;flex-direction:column;gap:10px">
                  ${sr.superMegaRaid ? renderTierAcc(`${dayName} Super Mega Raid`, sr.superMegaRaid, "#FF6348", "Super Mega Raid", "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png") : ""}
                  ...
                  ${sr.primalRaids ? renderTierAcc(`${dayName} Primal Raids`, sr.primalRaids, "#E74C3C", "Primal Raids") : ""}
          ...
          const headerLabel = event.details.dailyRaids ? "Daily Raid Schedule" : "GO Fest Raid Bosses";
          return `<div style="display:flex;flex-direction:column;gap:14px">
            <h4 style="margin:0;font-size:13px;font-weight:700;color:${th.text};display:flex;align-items:center;gap:8px"><span>\uD83C\uDFAF</span> ${headerLabel} <span style="font-size:11px;font-weight:500;color:${th.textMuted}">(tap to expand)</span></h4>
            ${event.details.dailyRaids ? event.details.dailyRaids.map(d => renderDayAcc(d, d.label, d.accent || "#8E44AD")).join("") : `${renderDayAcc(event.details.saturdayRaids, "Saturday \u00B7 July 11", "#6C5CE7")}${renderDayAcc(event.details.sundayRaids, "Sunday \u00B7 July 12", "#E84393")}`}
          </div>`;
        })() : ""}
        ${event.details.parkRaids ? (() => {
          const isMob = breakpoint === "mobile";
          const chevronSVG = `<svg class="acc-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>`;
          const pr = event.details.parkRaids;
          const renderTierAcc = (tierLabel, items, tierColor, tierKey, iconOverride) => {
            const eggUrl = TIER_EGGS[tierKey];
            const isShadowEgg = eggUrl && eggUrl.endsWith("shadow.png");
            const eggImg = iconOverride
              ? `<img src="${iconOverride}" style="width:26px;height:26px;object-fit:contain;flex-shrink:0" />`
              : (eggUrl ? (isShadowEgg
                ? `<img src="${eggUrl}" style="width:48px;height:32px;object-fit:contain;flex-shrink:0;margin:-2px -10px" />`
                : `<img src="${eggUrl}" style="width:26px;height:26px;object-fit:contain;flex-shrink:0" />`) : "");
            const stars = iconOverride ? "" : renderRaidHeads(tierKey);
            const bossesHTML = items.map(item => renderBossItem(item, tierColor, th, !isMob)).join("");
            return `<div style="border:1.5px solid ${th.border};border-radius:14px;overflow:hidden;background:${th.surface}">
              <button class="acc-trigger" data-open="false" onclick="toggleAccordion(this)" aria-expanded="false" style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:${th.accentBgSubtle(tierColor)};border:none;border-bottom:1.5px solid ${th.border};color:${th.text};width:100%;text-align:left;cursor:pointer">
                ${eggImg}
                <span style="font-size:12px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase">${esc(tierLabel)}</span>
                <span style="font-size:11px;font-weight:600;color:${th.textMuted};margin-left:4px">${items.length}</span>
                <span style="margin-left:auto;display:flex;align-items:center;gap:8px">${stars}${chevronSVG}</span>
          ...
          return `<div style="display:flex;flex-direction:column;gap:10px">
            ...
            ${pr.superMegaRaid ? renderTierAcc("Super Mega Raid", pr.superMegaRaid, "#FF6348", "Super Mega Raid", "assets/pokemon-images/icons/MEGA_EVOLUTION_REINSTATED.png") : ""}
            ${pr.fiveStarRaids ? renderTierAcc("5\u2605 Raids", pr.fiveStarRaids, "#8E44AD", "5-Star Raids") : ""}
            ${pr.megaRaids ? renderTierAcc("Mega Raids", pr.megaRaids, "#E67E22", "Mega Raids") : ""}
            ${pr.primalRaids ? renderTierAcc("Primal Raids", pr.primalRaids, "#E74C3C", "Primal Raids") : ""}
            ${pr.shadowRaids ? renderTierAcc("5\u2605 Shadow Raids", pr.shadowRaids, "#2C3E50", "Shadow 5-Star Raids") : ""}
            ${pr.threeStarRaids ? renderTierAcc("3\u2605 Raids", pr.threeStarRaids, "#F39C12", "3-Star Raids") : ""}
            ${pr.note ? `<div style="padding:10px 14px;border-radius:10px;background:${th.tipBg};border:1px solid ${th.tipBorder};font-size:13px;color:${th.tipText};line-height:1.5;display:flex;align-items:center;gap:8px"><span style="font-size:14px">\uD83D\uDCA1</span>${esc(pr.note)}</div>` : ""}
          </div>`;
        })() : ""}
        ${event.details.specialResearch ? (() => {
          const sr = event.details.specialResearch;
          const accent = "#F1C40F";
          const isMob = breakpoint === "mobile";
          const chevronSVG = `<svg class="acc-chevron" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>`;
          const stampSrc = "assets/pokemon-images/icons/QuestSpecialResearchStamp_00.png";
          const ITEM_BASE = "https://db.pokemongohub.net/images/items/";
          const ITEM_ICONS = {
            "XP": ITEM_BASE + "xp.png",
            "Pok\u00E9 Ball": ITEM_BASE + "poke-ball.png",
            "Great Ball": ITEM_BASE + "greatball.png",
            "Ultra Ball": ITEM_BASE + "ultraball_sprite.png",
            "Stardust": ITEM_BASE + "stardust.png",
            "Star Piece": ITEM_BASE + "starpiece.png",
            "Lucky Egg": ITEM_BASE + "lucky-egg.png",
            "Magnetic Lure Module": ITEM_BASE + "magnetic-lure.png",
            "Lure Module": ITEM_BASE + "lure.png",
            "Pinap Berry": ITEM_BASE + "pinap-berry.png",
            "Premium Raid Pass": ITEM_BASE + "free-pass.png",
            "Max Revive": ITEM_BASE + "max-revive.png",
            "Max Potion": ITEM_BASE + "max-potion.png",
            "Super Incubator": ITEM_BASE + "super-incubator.png",
            "Rare Candy": ITEM_BASE + "rare-candy.png",
            "Rare Candy XL": ITEM_BASE + "rare-xl-candy.png",
            ...
            "GO Fest 2026 Glasses avatar item": "assets/pokemon-images/clothing-items/glasses/go-fest-glasses.png"
          };
          const resolveRewardImg = (label) => {
            if (ITEM_ICONS[label]) return { url: ITEM_ICONS[label], kind: "item" };
            const trimmed = label.replace(/\s*encounter\s*$/i, "").trim();
            const pk = getPokemonImg(trimmed);
            if (pk && pk.url) return { url: pk.url, kind: "pokemon" };
            return null;
          };
          const renderRewardChip = (text, opts = {}) => {
            const big = opts.big === true;
            const m = text.match(/^([\d,]+)\s*[\u00D7x]\s*(.+)$/);
            const count = m ? m[1] : null;
            const label = m ? m[2].trim() : text.trim();
            const img = resolveRewardImg(label);
            const itemSize = big ? 32 : 22;
            const pkmnSize = big ? 48 : 34;
            const size = img && img.kind === "pokemon" ? pkmnSize : itemSize;
            const imgEl = img
              ? `<img src="${escAttr(img.url)}" alt="${escAttr(label)}" title="Tap to enlarge" onclick="event.stopPropagation();showFormModal(${escAttr(JSON.stringify(img.url))},${escAttr(JSON.stringify(label))})" style="width:${size}px;height:${size}px;object-fit:contain;flex-shrink:0;cursor:zoom-in" onerror="this.style.display='none'" />`
              : "";
            const countEl = count ? `<span style="font-weight:800;color:${accent};font-size:${big ? 13 : 12.5}px;flex-shrink:0">${esc(count)} \u00D7</span>` : "";
            const labelEl = `<span style="font-size:${big ? 12.5 : 12}px;color:${th.textSecondary};font-weight:600;line-height:1.3">${esc(label)}</span>`;
            return `<div style="display:inline-flex;align-items:center;gap:8px;padding:${big ? "8px 12px" : "6px 10px"};border-radius:10px;background:${th.accentBgSubtle(accent)};border:1px solid ${th.countdownBorder(accent)}">${countEl}${imgEl}${labelEl}</div>`;
          };
          const stageHTML = sr.stages.map(stage => {
            const tasksRows = stage.tasks.map(t => `<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:9px;background:${th.surface};border:1px solid ${th.border};font-size:13px;color:${th.textSecondary};line-height:1.45">
              <span style="flex:1;min-width:0">${esc(t.task)}</span>
              <span style="flex-shrink:0">${renderRewardChip(t.reward)}</span>
            </div>`).join("");
            const rewardsHTML = stage.rewards.map(r => renderRewardChip(r, { big: true })).join("");
            const venueTag = stage.venueSpecific ? `<span style="font-size:9.5px;font-weight:700;color:#fff;background:${accent};padding:2px 8px;border-radius:6px;letter-spacing:0.5px;text-transform:uppercase;flex-shrink:0">Venue-specific</span>` : "";
            return `<div style="border:1.5px solid ${th.border};border-radius:14px;overflow:hidden;background:${th.surface}">
              <button class="acc-trigger" data-open="false" onclick="toggleAccordion(this)" aria-expanded="false" style="display:flex;align-items:center;gap:10px;padding:12px 14px;background:${th.accentBgSubtle(accent)};border:none;border-bottom:1.5px solid ${th.border};color:${th.text};width:100%;text-align:left;cursor:pointer">
                <img src="${stampSrc}" alt="Research stamp" style="width:28px;height:28px;object-fit:contain;flex-shrink:0" onerror="this.style.display='none'" />
                <span style="font-size:12px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase">${esc(stage.label)}</span>
                ${venueTag}
              ...
              <div class="acc-content" data-open="false">
                <div style="padding:12px;display:flex;flex-direction:column;gap:12px">
                  <div style="display:flex;flex-direction:column;gap:6px">
                    <div style="font-size:10.5px;font-weight:700;color:${th.textMuted};text-transform:uppercase;letter-spacing:0.6px">Tasks</div>
                    ${tasksRows}
                  </div>
                  <div style="display:flex;flex-direction:column;gap:6px">
                    <div style="font-size:10.5px;font-weight:700;color:${th.textMuted};text-transform:uppercase;letter-spacing:0.6px">Stage Rewards</div>
                    <div style="display:flex;flex-wrap:wrap;gap:8px">${rewardsHTML}</div>
                  </div>
                </div>
              </div>
            </div>`;
          }).join("");
          const sourceLink = sr.source ? `<a href="${esc(sr.source.url)}" target="_blank" rel="noopener noreferrer" style="font-size:11.5px;font-weight:600;color:${accent};text-decoration:none">Source: ${esc(sr.source.label)} \u2197</a>` : "";
          return `<div style="border:1.5px solid ${th.countdownBorder(accent)};border-radius:14px;overflow:hidden;background:${th.accentBgSubtle(accent)}">
            ...
            <div class="acc-content" data-open="false">
              <div style="padding:0 14px 14px;display:flex;flex-direction:column;gap:10px">
                ${sr.note ? `<div style="padding:10px 14px;border-radius:10px;background:${th.tipBg};border:1px solid ${th.tipBorder};font-size:12.5px;color:${th.tipText};line-height:1.5;display:flex;align-items:center;gap:8px"><span style="font-size:14px">\uD83D\uDCA1</span>${esc(sr.note)}</div>` : ""}
                ${stageHTML}
                ${sourceLink ? `<div style="text-align:right">${sourceLink}</div>` : ""}
              </div>
            </div>
          </div>`;
        })() : ""}
        ${event.type === "Community Day" && event.iconImg ? (() => {
          ...
          cdNames.forEach(cdName => {
            ...
            if (!family || family.length === 0) {
              family = EVO_CHAIN_FALLBACK[cdDex] || [{ name: baseName, dexNum: cdDex }];
            }
        ...
        ${event.details.paidResearch ? `<div><h4 style="margin:0 0 10px 0;font-size:13px;font-weight:700;color:${th.text};display:flex;align-items:center;gap:8px"><span>\uD83D\uDCB0</span> Paid Timed Research — ${esc(event.details.paidResearch.price)}</h4>
          <div style="display:flex;flex-direction:column;gap:6px">
            ${event.details.paidResearch.rewards.map(r => { const _rt = (r && typeof r === "object") ? r.text : r; const _rs = (r && typeof r === "object" && r.imgSize) || 26; const _ri = (r && typeof r === "object" && r.img) ? `<div style="position:relative;width:34px;height:34px;flex-shrink:0"><img src="${r.img}" style="position:absolute;top:50%;left:50%;transform:translate(-50%,calc(-50% - ${r.imgNudgeY||0}px));width:${_rs}px;height:${_rs}px;object-fit:contain" onerror="this.style.display='none'" /></div>` : (r && typeof r === "object" && r.glyph) ? `<div style="width:34px;text-align:center;flex-shrink:0;color:#E67E22;font-weight:800;font-size:20px;line-height:1">${esc(r.glyph)}</div>` : `<span style="color:#E67E22;font-weight:700">•</span>`; return `<div style="padding:10px 14px;border-radius:10px;background:${th.accentBgSubtle("#E67E22")};border:1px solid ${th.countdownBorder("#E67E22")};font-size:13.5px;color:${th.textSecondary};line-height:1.5;display:flex;align-items:center;gap:8px">${_ri} ${esc(_rt)}</div>`; }).join("")}
        ...
        ${event.details.milestones ? `<div><h4 style="margin:0 0 10px 0;font-size:13px;font-weight:700;color:${th.text};display:flex;align-items:center;gap:8px"><span>\uD83C\uDFC6</span> Major Milestone Bonuses</h4>
          <div style="display:flex;flex-wrap:wrap;gap:8px">${event.details.milestones.map(m => `<div style="flex:1;min-width:200px;border:1.5px solid ${th.border};border-radius:14px;overflow:hidden">
            ...
            <div style="padding:12px 14px;font-size:14px;font-weight:500;color:${th.textSecondary};line-height:1.5;display:flex;flex-direction:column;gap:6px">${(() => { const mLead = txt => { const ic = rewardIcon(txt); return ic ? `<img src="${ic}" alt="" style="width:20px;height:20px;object-fit:contain;flex-shrink:0;margin-top:1px" onerror="this.style.display='none'" />` : `<span style="flex-shrink:0">•</span>`; }; return `<div style="display:flex;align-items:flex-start;gap:8px">${mLead(m.bonus)}<span>${esc(m.bonus)}</span></div>${m.deluxe ? `<div style="display:flex;align-items:flex-start;gap:8px">${mLead(m.deluxe)}<span>GO Pass Deluxe: ${esc(m.deluxe)}</span></div>` : ""}`; })()}</div>
          </div>`).join("")}</div></div>` : ""}
        ${event.details.goPass ? (() => {
          ...
          const renderTier = (title, emoji, color, items) => `<div style="border:1.5px solid ${th.border};border-radius:14px;overflow:hidden;flex:1;min-width:200px">
            ...
            <div style="padding:10px;display:flex;flex-direction:column;gap:5px">${items.map(r => { const ic = rewardIcon(r); const lead = ic ? `<img src="${ic}" alt="" style="width:22px;height:22px;object-fit:contain;flex-shrink:0" onerror="this.style.display='none'" />` : `<span style="color:${color};font-weight:700;flex-shrink:0">\u2022</span>`; return `<div style="padding:8px 12px;border-radius:9px;background:${th.accentBgSubtle(color)};font-size:13px;color:${th.textSecondary};line-height:1.45;display:flex;align-items:center;gap:8px">${lead} ${esc(r)}</div>`; }).join("")}</div>
          </div>`;
              ...
              <div style="display:inline-flex;flex-direction:column;align-items:center;gap:10px;padding:12px 18px;border-radius:14px;border:1.5px solid ${th.border};background:${th.accentBgSubtle(event.color || "#3498DB")}">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:18px;align-items:flex-end;justify-content:center">${gp.featuredRewards.map(r => `<div style="display:flex;flex-direction:column;align-items:center;gap:6px">
                  <div style="position:relative;width:120px;height:120px;display:flex;align-items:center;justify-content:center;border-radius:16px;background:${th.surface};border:1.5px solid ${th.border}">
                    ...
                    ${r.shiny ? `<div title="Shiny available" style="position:absolute;top:6%;right:10%;z-index:2;font-size:22px;pointer-events:none">✨</div>` : ""}
                  </div>
        ...
        ${(event.details.tips && event.details.tips.length > 0) || event.type === "Community Day" ? `<div><h4 style="margin:0 0 10px 0;font-size:13px;font-weight:700;color:${th.text};display:flex;align-items:center;gap:8px"><span>\uD83D\uDCA1</span> Trainer Tips</h4>
          <div style="display:flex;flex-direction:column;gap:6px">${event.type === "Community Day" ? (() => {
            const _m = /^Community Day:\s*(.+)$/.exec(event.title || "");
            const _raw = _m ? _m[1].trim() : "";
            const _feat = (_raw && !/\d/.test(_raw)) ? _raw : "Community Day Pok\u00E9mon";
            return `<div style="display:flex;align-items:center;gap:10px;padding:12px 16px;border-radius:12px;background:${th.accentBgSubtle("#E84393")};border:2px solid #E84393"><img src="assets/pokemon-images/Items/lure-module.png" alt="Regular Lure Module" style="width:24px;height:24px;flex-shrink:0" onerror="this.style.display='none'"><span style="font-size:13px;font-weight:600;color:${th.tipText};line-height:1.45"><strong style="color:#E84393">Use regular Lure Modules only.</strong> Only regular Lures reliably draw the featured ${esc(_feat)} \u2014 Glacial, Mossy, Magnetic, and Rainy Lures pull their own themed species and attract <em>far fewer</em> of the featured Community Day Pok\u00E9mon.</span></div>`;
      ...
      <div style="padding:20px 20px 24px;display:flex;flex-direction:column;gap:20px">
        ...
        ${announcement.sections ? announcement.sections.map(s => {
          ...
          if (s.spotlight) {
            ...
            const isMobile = breakpoint === "mobile";
            const RES_ICON = { candy: "assets/pokemon-images/Items/pokemon_details_candy.png", stardust: "assets/pokemon-images/Items/stardust_painted.png", xp: "assets/pokemon-images/Items/luckyegg.png" };
            const spSize = isMobile ? 88 : 116;
            const spCards = s.items.map(it => {
              const pkmn = getPokemonImg(it.name);
              let imgEl = pokemonImgHTML(pkmn, spSize);
              // Every Spotlight Hour Pokémon is Shiny-eligible — force the sparkle for all.
              if (imgEl) imgEl = wrapShinySparkles(imgEl, it.name + " ✨", spSize);
              const resIcon = RES_ICON[it.res] ? `<img src="${RES_ICON[it.res]}" style="width:20px;height:20px;object-fit:contain;flex-shrink:0" onerror="this.style.display='none'" />` : "";
              return `<div style="border-radius:14px;background:${th.accentBgSubtle(tagColor)};border:1.5px solid ${th.border};flex:1 1 150px;min-width:140px;max-width:200px;display:flex;flex-direction:column;align-items:center;padding:14px 10px 12px;text-align:center">
                <div style="font-size:11px;font-weight:800;letter-spacing:0.4px;color:${tagColor}">${esc(it.date)}</div>
                <div style="margin:6px 0 0">${imgEl || ""}</div>
                <div style="margin-top:6px;font-weight:800;color:${th.text};font-size:15px">${esc(it.name)}</div>
                <div style="margin-top:8px;display:inline-flex;align-items:center;gap:7px;padding:5px 12px;border-radius:20px;background:${th.tagBg(announcement.tag)}">${resIcon}<span style="font-weight:800;color:${th.text};font-size:13px">${esc(it.bonus)}</span></div>
                <div style="margin-top:5px;font-size:11px;color:${th.textSecondary};font-style:italic">${esc(it.action)}</div>
              </div>`;
            }).join("");
            const spNote = s.note ? `<div style="margin-top:12px;text-align:center;font-size:12.5px;color:${th.textSecondary};font-style:italic">✨ ${esc(s.note)}</div>` : "";
            return `<div>${headingHTML}
              <div style="display:flex;flex-wrap:wrap;gap:10px">${spCards}</div>
              ${spNote}
            </div>`;
          }
          if (s.showImages) {
            ...
            const cardsHTML = s.items.map(it => {
              ...
              const isRaidTier = /\(\d★\s*(?:Raid|Shadow Raid)?\)|\(Mega\)|\(Primal Raid\)|\([^)]*Super Mega Raid[^)]*\)|\(\d★\s*Max Battle|\(Shadow Raid\)/i.test(itemName);
...
function render() {
  ...
  if (state.selectedEvent) {
    ...
    if (state.tab === "tools") {
      toolsTabHTML = `<div style="display:flex;flex-direction:column;gap:${isMobile ? 16 : 20}px">
        ...
        <a href="https://pokemongo.com/map" target="_blank" rel="noopener noreferrer" style="display:flex;${isMobile ? "flex-direction:column" : "flex-direction:row;align-items:center"};gap:${isMobile ? 16 : 20}px;padding:${isMobile ? "20px 18px" : "24px 28px"};background:${th.surface};border:1.5px solid ${th.border};border-radius:${isMobile ? 18 : 20}px;text-decoration:none;transition:all 0.25s ease;box-shadow:${th.shadow}" onmouseenter="this.style.borderColor='#16A085';this.style.transform='translateY(-3px)';this.style.boxShadow='0 8px 25px rgba(22,160,133,0.15)'" onmouseleave="this.style.borderColor='${th.border}';this.style.transform='translateY(0)';this.style.boxShadow='${th.shadow}'">
          <div style="width:${isMobile ? "100%" : "80px"};display:flex;${isMobile ? "justify-content:center" : "justify-content:center;flex-shrink:0"}">
            <div style="width:${isMobile ? 64 : 72}px;height:${isMobile ? 64 : 72}px;border-radius:18px;background:linear-gradient(135deg,#16A085,#1ABC9C);display:flex;align-items:center;justify-content:center;font-size:${isMobile ? 40 : 48}px;box-shadow:0 4px 12px rgba(22,160,133,0.25)">🗺️</div>
          </div>
          <div style="flex:1;${isMobile ? "text-align:center" : ""}">
            <div style="display:flex;align-items:center;gap:8px;${isMobile ? "justify-content:center" : ""}">
              <h3 style="margin:0;font-size:${isMobile ? 18 : 22}px;font-weight:800;color:${th.text}">Official Pokémon GO Map</h3>
              <span style="font-size:10px;font-weight:700;color:#16A085;background:${th.accentBg("#16A085")};padding:2px 8px;border-radius:20px">OFFICIAL</span>
            </div>
            <p style="margin:8px 0 0 0;font-size:${isMobile ? 13 : 14}px;color:${th.textSecondary};line-height:1.6">Niantic’s official live web map for Pokémon GO. Find Super Mega Raids, nearby raids, Routes, and community meetups happening around you — straight from the source.</p>
            <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px;${isMobile ? "justify-content:center" : ""}">
              ${["Live Map","Super Mega Raids","Routes","Meetups"].map(tag =>
                `<span style="font-size:10px;font-weight:700;color:${th.textMuted};background:${th.accentBgSubtle("#16A085")};border:1px solid ${th.countdownBorder("#16A085")};padding:3px 10px;border-radius:20px">${tag}</span>`
              ).join("")}
            </div>
            <div style="margin-top:14px;display:inline-flex;align-items:center;gap:6px;font-size:13px;font-weight:600;color:#16A085">Open pokemongo.com/map →</div>
          </div>
        </a>
    ...
    const mapBannerHTML = state.tab === "home" ? `<a href="https://pokemongo.com/map" target="_blank" rel="noopener noreferrer" style="display:flex;align-items:center;gap:12px;padding:${isMobile ? "12px 16px" : "14px 20px"};background:${th.surface};border:1.5px solid ${th.border};border-radius:${isMobile ? 14 : 16}px;text-decoration:none;box-shadow:${th.shadow};transition:all 0.2s ease" onmouseenter="this.style.borderColor='#16A085';this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 18px rgba(22,160,133,0.15)'" onmouseleave="this.style.borderColor='${th.border}';this.style.transform='translateY(0)';this.style.boxShadow='${th.shadow}'">
      <span style="width:${isMobile ? 34 : 38}px;height:${isMobile ? 34 : 38}px;border-radius:11px;background:linear-gradient(135deg,#16A085,#1ABC9C);display:flex;align-items:center;justify-content:center;font-size:${isMobile ? 18 : 20}px;flex-shrink:0">🗺️</span>
      <div style="flex:1;min-width:0">
        <div style="font-size:${isMobile ? 14 : 15}px;font-weight:800;color:${th.text}">Official Pokémon GO Map</div>
        <div style="font-size:${isMobile ? 11 : 12}px;color:${th.textMuted};font-weight:500">Find Super Mega Raids, Routes &amp; meetups near you</div>
      </div>
      <span style="font-size:14px;font-weight:700;color:#16A085;flex-shrink:0;white-space:nowrap">Open →</span>
    </a>` : "";
  ...
  appEl.innerHTML = `${sidebarHTML}<div style="min-height:100vh;display:flex;flex-direction:column;background:${th.bg};font-family:'Outfit','DM Sans',-apple-system,BlinkMacSystemFont,sans-serif;color:${th.text};width:100%">
    ${headerHTML}${/* tickerHTML — top scrolling ticker bar disabled, see line ~8440 for source */""}<div style="flex:1">${content}</div>${footerHTML}
