...
// Raids rotate at 10am on the day AFTER endDate (when the next boss takes over).
...
function renderBossItem(item, color, th, cardLayout, noSparkles, groupSize) {
  ...
  const isRaidTier = /\(\d★\s*(?:Raid|Shadow Raid)?\)|\(Mega\)|\(\d★\s*Max Battle|\(Shadow Raid\)|\(weekends?\)|\(final week.*?\)/i.test(item);
  ...
  const cleanedItemName = cleanRaidLabel(item);
  ...
  if (cardLayout && pkmn) {
    const frontHTML = `<div style="display:flex;flex-direction:column;align-items:center;padding:12px 8px;font-size:13px;color:${th.textSecondary};line-height:1.45;text-align:center">
      ...
      ${typesHTML}${cpHTML}${groupSizeHTML}
    </div>`;
    if (!hasBack) {
  ...
  const frontRowHTML = `<div style="display:flex;align-items:center;gap:10px;padding:${pkmn ? "4px" : "7px"} 12px;font-size:13.5px;color:${th.textSecondary};line-height:1.45">${imgEl
    || `<div style="width:5px;height:5px;border-radius:50%;background:${color};flex-shrink:0"></div>`}<div style="flex:1;min-width:0"><div>${esc(cleanRaidLabel(item))}</div>${typesHTML}${cpHTML}${groupSizeHTML}</div></div>`;
  if (!hasBack) {
...
// Shows individual cells for 1..(optimalMin-1) then a final "optimalMin+" cell
// representing the comfortable range. Colors: red below minimum, orange at
// minimum, green at optimal+.
function renderGroupSizeCompact(gs, th, centered) {
  if (!gs) return "";
  const RED = "#C0392B", ORANGE = "#E67E22", GREEN = "#27AE60";
  const greenAt = gs.greenAt != null ? gs.greenAt : gs.optimalMin;
  const colorFor = n => n < gs.minimum ? RED : (n < greenAt ? ORANGE : GREEN);
  const cells = [];
  for (let n = 1; n < greenAt; n++) {
    cells.push(`<div style="display:flex;align-items:center;justify-content:center;min-width:26px;height:24px;padding:0 8px;border-radius:7px;background:${colorFor(n)};color:#fff;font-weight:800;font-size:12px">${n}</div>`);
  }
  ...
  return `<div style="animation:fadeSlideIn 0.3s ease">
    ...
    <div style="background:${th.surface};border-radius:20px;border:1.5px solid ${th.border};overflow:hidden;box-shadow:${th.shadowLg}">
      ...
      <div style="padding:20px 20px 24px;display:flex;flex-direction:column;gap:42px">
        ...
        ${event.details.bosses ? renderDetailSection(event.type === "Community Day" ? "Featured Move(s)" : "Featured Encounters", "\uD83C\uDFAF", event.details.bosses, event.color, th, true, event.type === "Community Day", event.details.groupSize) : ""}
