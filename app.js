    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">
      <div style="display:flex;align-items:center;gap:10px;flex:1;min-width:0">
        ${(() => {
          ...
          if (event.iconImg) {
            const shadowLayer = event.shadowBg ? `<img src="assets/pokemon-images/icons/shadow_icon.png" style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;opacity:${darkMode ? 0.9 : 0.75};z-index:0" />` : "";
            return `<div style="width:48px;height:48px;border-radius:12px;background:${th.accentBg(event.color)};display:flex;align-items:center;justify-content:center;flex-shrink:0;position:relative;overflow:hidden">${shadowLayer}<img src="${event.iconImg}" style="width:44px;height:44px;object-fit:contain;position:relative;z-index:1" onerror="this.parentElement.innerHTML='${event.icon}'" />${shinySparkleCard}</div>`;
          }
